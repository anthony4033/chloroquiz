...
async function loadPlantnetKey(){
  ...
  try{
    const hdr = {'apikey':window.SUPA_KEY,'Authorization':'Bearer '+window.SUPA_KEY};
    const r = await fetch(window.SUPA_URL+'/rest/v1/floralab_config?key=eq.plantnet_api_key&select=value&limit=1', {headers:hdr});
    if(!r.ok) return;
    const data = await r.json();
    if(data && data.length && data[0].value) plantnetKey = data[0].value;
  }catch(e){ /* silencieux */ }
}

// ── Sauvegarde clé PlantNet vers Supabase (admin uniquement) ──
async function savePlantnetKeyToCloud(key){
  if(!cloudOk || !window.SUPA_URL || !window.SUPA_KEY) return false;
  try{
    const hdr = {
    ...
    const r = await fetch(window.SUPA_URL+'/rest/v1/floralab_config', {
      method:'POST', headers:hdr,
      body: JSON.stringify({key:'plantnet_api_key', value:key})
    });
