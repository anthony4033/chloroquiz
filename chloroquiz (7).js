...
async function loadPlantnetKey(){
  ...
  try{
    ...
    if(rows && rows.length && rows[0].value) plantnetKey = rows[0].value;
  }catch(e){ /* silencieux */ }
}
...
async function _pnetAdminSave(){
  ...
  if(ok){
    plantnetKey = key;
    inp.value = '';
    if(msg){ msg.style.color='#4ab870'; msg.textContent='✅ Clé enregistrée — active immédiatement'; }
    setTimeout(function(){ renderPlantnetTab(); }, 1200);
  } else {
    if(msg){ msg.style.color='#ffb347'; msg.textContent='❌ Erreur — vérifiez la connexion cloud'; }
