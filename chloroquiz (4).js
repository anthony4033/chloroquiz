...
let plantnetKey = localStorage.getItem('cq_plantnet_key') || '';
...
function openPlantnetModal(){
  ...
  div.innerHTML = ''
    ...
    +     '<summary style="font-size:11px;color:rgba(200,223,204,.3);cursor:pointer;list-style:none">🔑 Clé API Pl@ntNet</summary>'
    +     '<div style="margin-top:8px;display:flex;gap:8px">'
    +       '<input id="pnet-key-input" value="' + (plantnetKey||'') + '" placeholder="Votre clé (my.plantnet.org)"'
    +         ' style="flex:1;padding:9px 12px;background:rgba(19,48,29,.8);border:1px solid rgba(74,184,112,.2);border-radius:10px;color:var(--cr);font-size:13px;outline:none"/>'
    +       '<button onclick="_pnetSaveKey()" style="padding:9px 14px;background:rgba(74,184,112,.15);border:1px solid rgba(74,184,112,.3);border-radius:10px;color:#4ab870;font-size:13px;cursor:pointer">💾</button>'
    +     '</div>'
    +     '<div style="font-size:10px;color:rgba(200,223,204,.25);margin-top:6px">Clé gratuite sur <strong>my.plantnet.org</strong> · Usage éducatif non commercial</div>'
    +   '</details>'
    + '</div>';
...
function _pnetSaveKey(){
  const inp = document.getElementById('pnet-key-input');
  plantnetKey = inp ? inp.value.trim() : '';
  localStorage.setItem('cq_plantnet_key', plantnetKey);
  const btn = document.querySelector('#pnet-overlay details button');
