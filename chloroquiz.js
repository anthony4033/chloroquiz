  ...
  const sessionsListHTML=sessionsSorted.map(s=>{
    ...
    return `<div class="collective-row" style="flex-direction:column;align-items:flex-start;gap:5px;padding:10px 12px">
      <div style="display:flex;width:100%;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px">
        ...
        <div style="display:flex;gap:5px;align-items:center;flex-wrap:wrap">${statusBadge}<span style="font-size:11px;color:rgba(200,223,204,.4)">${dateStr} ${timeStr}</span></div>
      </div>
    ...
    <div class="form-bg" onclick="if(event.target===this)closeForm()">
      <div class="form-card">
        ...
        <div class="form-grid">
          ...
          <div class="fg full">
            <label class="fl">📷 Photos (1 à 4) — compressées automatiquement en WebP</label>
            <div class="photo-grid" style="grid-template-columns:1fr 1fr">
              ${[1,2,3,4].map(n=>{
                ...
                const isReq=n===1?' *':'';
                return `<div>
                  <div style="font-size:11px;color:rgba(200,223,204,.35);margin-bottom:6px;letter-spacing:1px">PHOTO ${n}${isReq}</div>
                  <div class="photo-zone ${val?'has-photo':''}" id="photo-zone${sfx}" onclick="document.getElementById('f-photo-file${sfx}').click()">
                    ${val?`<img src="${val}" class="photo-preview" id="photo-preview${sfx}"/>`:`<span class="photo-zone-icon" id="photo-zone-icon${sfx}">📷</span>`}
