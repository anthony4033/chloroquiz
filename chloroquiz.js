...
function buildPhotoRound(){
  ...
  // Exclure les plantes sans aucune photo (impossible d'identifier sans image)
  const withPhoto = qPlants.filter(p=>p.photo||p.photo2||p.photo3||p.photo4);
  const safePool  = withPhoto.length >= 4 ? withPhoto : qPlants; // fallback si trop peu de photos
  let pool = safePool.filter(p=>!qUsed.includes(p.id));
  if(!pool.length){ qUsed=[]; pool=[...safePool]; }
  const correct = rand(pool);
  const wrongs = shuffle(safePool.filter(p=>p.id!==correct.id)).slice(0,3);
...
function buildPhotoQuestion(p, pool){
  // Seules les plantes avec au moins une photo peuvent être proposées comme options
  const photoPool = pool.filter(x=>x.photo||x.photo2||x.photo3||x.photo4);
  const wrongs = shuffle(photoPool.filter(x=>x.id!==p.id)).slice(0,3);
