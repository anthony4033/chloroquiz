...
async function confirmCSVImport(rows){
  ...
  // ── Traitement 100% local d'abord ──
  for(const {plant,action} of toProcess){
    if(action==='add'){
      // ID unique basé sur le max actuel + compteur
  ...
  try{
    ...
    cqErr('CSV import save error:',e);
    alert('⚠️ Import effectué localement mais erreur de sauvegarde cloud : '+e.message);
  }

  renderAdminList();
  alert('✅ Import terminé !\n✨ '+added+' ajoutée(s)  ✏️ '+updated+' mise(s) à jour\n📦 Total catalogue : '+plants.length+' végétaux');
}
