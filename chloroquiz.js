...
function launchComparison(){
  ...
  document.getElementById('overlay-root').innerHTML =
    ...
    +'<button onclick="exportComparaisonPDF('+id1+','+id2+')" style="padding:7px 14px;background:rgba(232,160,32,.12);border:1px solid rgba(232,160,32,.35);border-radius:9px;color:var(--am2);font-family:\'DM Sans\',sans-serif;font-size:12px;font-weight:600;cursor:pointer;white-space:nowrap">📄 Imprimer / PDF</button>'
    +'<button class="modal-close" onclick="closeModal()">✕</button>'
    +'</div>'
    +'</div>'
    +legendHTML
      ...
      <div class="modal-bg" onclick="if(event.target===this)closeModal()">
        <div class="modal">
          ...
          ${(()=>{
            ...
            if(_ph.length===1) return `<img src="${_ph[0]}" class="modal-img" onerror="this.style.display='none'" loading="lazy"/>`;
            ...
            return `<div style="display:grid;grid-template-columns:${cols};gap:8px;margin:12px 0 16px">`
              +_ph.map(ph=>`<img src="${ph}" class="modal-img" style="margin:0;height:220px" onerror="this.style.display='none'" loading="lazy"/>`).join('')
              +'</div>';
          })()}
...
function checkLatinField(el){
  ...
  const tokens=(el.value||'').trim().split(/\s+/);
  if(el.value.trim()&&tokens.length<2){
    errDiv.style.display='block';
    errDiv.textContent='⚠️ Doit contenir au moins le genre et l\'épithète. Ex : Lavandula angustifolia';
...
function openForm(id){
  ...
  document.getElementById('overlay-root').innerHTML=glossOv+
    ...
    +' onblur="checkLatinField(this)" oninput="this.value=normalizeLatin(this.value)"/>'
    ...
    +' onblur="checkFamilleField(this)" oninput="this.value=normalizeFamille(this.value)"/>'
    ...
    +[['Annuelle','annuelle'],['Bisannuelle','annuelle'],['Vivace herbacée','vivace'],['Bulbeuse',''],['Graminée ornementale','graminee'],
    ...
    +'<span class="type-chip '+(!v('typeInflorescence')?'selected':'')+'" data-val="" onclick="event.stopPropagation();document.getElementById(\'f-inflo-det\').style.display=\'none\';document.getElementById(\'f-typeInflorescence\').value=\'\';document.getElementById(\'inflo-toggle-chip\').classList.remove(\'selected\');this.classList.add(\'selected\')">Fleurs simples</span>'
    +'</div>'
    ...
    +renderTempPicker('rusticite', v('rusticite'))+'</div>'
    +'<div class="fg"><label class="fl">Résistance sécheresse</label>'
    +'<input type="hidden" id="f-resistanceSech" value="'+v('resistanceSech')+'"/>'
    +'<div class="type-chips" id="sech-chips">'
    +['Faible','Modérée','Élevée','Très élevée'].map(function(t){return '<span class="type-chip '+cs1('resistanceSech',t)+'" data-val="'+t+'" onclick="setSFChip(this,\'f-resistanceSech\',\'sech-chips\',\'resistanceSech\')">'+t+'</span>';}).join('')
    +'</div>'
    +'<div class="type-display" id="resistanceSech">'+(v('resistanceSech')||'—')+'</div>'
    +'</div>'
    ...
    +[['Sec',''],['Frais','sol_frais'],['Humide',''],['Détrempé / Aquatique','sol_detrempe']].map(function(item){
      return '<span class="type-chip '+cs1('humidite',item[0])+'" data-val="'+item[0]+'" onclick="setSFChip(this,\'f-humidite\',\'humid-chips\',\'humidite\')">'+item[0]+(item[1]?' '+G(item[1]):'')+'</span>';
    }).join('')+'</div>'
    +'<div class="type-display" id="humidite">'+(v('humidite')||'—')+'</div>'
    +'</div>'
    ...
    +[['Acidophile (4 – 5,5)','ph_acide'],['Légèrement acide (5,5 – 6,5)','ph_lacide'],['Neutre (6,5 – 7,2)','ph_neutre'],
      ...
      return '<span class="type-chip '+cs1('ph',item[0])+'" data-val="'+item[0]+'" onclick="setSFChip(this,\'f-ph\',\'ph-chips\',\'ph\')">'+item[0]+(item[1]?' '+G(item[1]):'')+'</span>';
    }).join('')+'</div>'
    +'<div class="type-display" id="ph">'+(v('ph')||'—')+'</div>'
    +'</div>'
    ...
    +[['Drainant','drainant'],['Équilibré','equilibre'],['Lourd / Argileux','lourd'],['Indifférent','']].map(function(item){
      return '<span class="type-chip '+cs1('structureSol',item[0])+'" data-val="'+item[0]+'" onclick="setSFChip(this,\'f-structureSol\',\'sol-chips\',\'structureSol\')">'+item[0]+(item[1]?' '+G(item[1]):'')+'</span>';
    }).join('')+'</div>'
    +'<div class="type-display" id="structureSol">'+(v('structureSol')||'—')+'</div>'
    +'</div>'
    ...
    +[['Aucune (port naturel)',''],['Taille de formation','taille_formation'],['Taille de maintien','taille_maintien'],['Recépage','recepage'],['Haie / Topiaire','topiaire']].map(function(item){
      return '<span class="type-chip '+cs1('typeTaille',item[0])+'" data-val="'+item[0]+'" onclick="setSFChip(this,\'f-typeTaille\',\'taille-chips\',\'typeTaille\')">'+item[0]+(item[1]?' '+G(item[1]):'')+'</span>';
    }).join('')+'</div>'
    +'<div class="type-display" id="typeTaille">'+(v('typeTaille')||'—')+'</div>'
    +'</div>'
    ...
    +["Mellifère","Médicinale","Feuillage aromatique","Comestible / Fruitière","Plante hôte","Fixatrice d'azote","Résistante à la pollution","Toxique","Épineuse / Défensive"].map(function(t){
