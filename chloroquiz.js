  ...
  if(cloudOk){
    try{
      ...
      for(let i=0;i<updates.length;i+=20){
        await sbPost('floralab_mastery', updates.slice(i,i+20), 'resolution=merge-duplicates');
      }
...
async function renderStats(){
  ...
  if(cloudOk){
    const [resScores, resAbandoned, resPlayers, resSessions] = await Promise.allSettled([
      sbGet('floralab_scores','select=*&order=pct.desc,ok.desc'),
      sbGet('floralab_abandoned','select=*&order=date.desc&limit=200'),
      sbGet('floralab_session_players','select=*&order=updated_at.desc'),
      sbGet('floralab_sessions','select=code,level,created_at,status&order=created_at.desc&limit=100')
    ]);
