...
async function renderScores(){
  ...
  const now=Date.now();
  if(cloudOk && (!window._scoresLastLoad || now-window._scoresLastLoad > 180000)){ // 3 min
...
function openForm(id){
  ...
  const photoHTML=[1,2,3,4].map(n=>{
    ...
    return '<div>'
      ...
      +'</div>'
      +'<input class="fi" id="f-photo-url'+sfx+'" placeholder="https://..." value="'+(val&&!val.startsWith('data:')?val:'')+'" style="margin-top:6px;font-size:12px" oninput="previewUrl(this.value,'+n+')" onblur="previewUrl(this.value,'+n+')"/>'
      +'</div>';
  }).join('');
  ...
  document.getElementById('overlay-root').innerHTML=glossOv+
    ...
    +'</div></div></div>';
  // Déclencher la prévisualisation des photos URL déjà renseignées (modification d'une fiche existante)
  setTimeout(function(){
    for(var pn=1;pn<=4;pn++){
      var sfx=pn===1?'':String(pn);
      var el=document.getElementById('f-photo-url'+sfx);
      if(el&&el.value.trim()) previewUrl(el.value.trim(),pn);
    }
  },80);
}
...
function buildPlantFromForm(){
  ...
  return {
    ...
    photo:(()=>{
      // Priorité 1 : nouvelle photo uploadée (b64)
      if(formPhotoB64) return formPhotoB64;
      // Priorité 2 : URL saisie dans le champ
      const el=document.getElementById('f-photo-url');
      const url=el?el.value.trim():'';
      if(url) return url;
      // Priorité 3 : photo base64 déjà affichée (non visible dans le champ URL)
      const prev=document.getElementById('photo-preview');
      return (prev&&prev.src&&prev.src.startsWith('data:'))?prev.src:'';
    })(),
    photo2:(()=>{
      if(formPhoto2B64) return formPhoto2B64;
      const url=(document.getElementById('f-photo-url2')||{}).value?.trim()||'';
      if(url) return url;
    ...
    photo3:(()=>{
      ...
      const url=(document.getElementById('f-photo-url3')||{}).value?.trim()||'';
      if(url) return url;
    ...
    photo4:(()=>{
      ...
      const url=(document.getElementById('f-photo-url4')||{}).value?.trim()||'';
      if(url) return url;
...
async function savePlantForm(){
  ...
  // Récupérer aussi les base64 affichées dans les previews (non visibles dans le champ URL)
  const prevSrc  = (document.getElementById('photo-preview') ||{}).src||'';
  const prevSrc2 = (document.getElementById('photo-preview2')||{}).src||'';
  const prevSrc3 = (document.getElementById('photo-preview3')||{}).src||'';
  const prevSrc4 = (document.getElementById('photo-preview4')||{}).src||'';
  const existing  = (editingId ? (plants.find(x=>x.id==editingId)||{}) : {});
  const photo     = formPhotoB64  || urlField  || (prevSrc.startsWith('data:') ?prevSrc :'') || existing.photo  || '';
  const photo2    = formPhoto2B64 || urlField2 || (prevSrc2.startsWith('data:')?prevSrc2:'') || existing.photo2 || '';
  const photo3    = formPhoto3B64 || urlField3 || (prevSrc3.startsWith('data:')?prevSrc3:'') || existing.photo3 || '';
  const photo4    = formPhoto4B64 || urlField4 || (prevSrc4.startsWith('data:')?prevSrc4:'') || existing.photo4 || '';
