    ...
    [['photo',''],['photo2','2'],['photo3','3'],['photo4','4']].forEach(function(pair){
      ...
      if(prev){
        prev.onerror = function(){
          // URL cassée → nettoyer la zone
          this.removeAttribute('src');
          this.style.display='none';
          if(zone) zone.classList.remove('has-photo');
          if(txt) txt.textContent='Uploader une photo';
          // Effacer aussi la valeur invalide dans plants[]
          var pid = id;
          var mp = plants.find(function(x){return x.id===pid;});
          if(mp){
            var key = pair[0];
            mp[key]='';
            console.log('[CQ] URL cassée supprimée pour '+mp.latin+' ('+key+')');
          }
        };
        prev.src=src;
      }
