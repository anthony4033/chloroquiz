...
async function renderSessionMonitor(code){
  ...
  multiPollInterval = setInterval(()=>pollMonitor(code), 10000);
  pollMonitor(code);
}
...
async function renderSessionEnd(code){
  ...
  // S'assurer que done=true est bien enregistré dans Supabase
  try{
    await sbPatch(
      'floralab_session_players',
      'session_code=eq.'+code+'&player_name=eq.'+encodeURIComponent(sessionPlayerName),
      {score:sessionScore.ok, total:sessionScore.total, current_q:sessionQIdx, done:true, updated_at:new Date().toISOString()}
    );
  } catch(e){ cqErr('renderSessionEnd patch done failed:', e); }
  const c = document.getElementById('quiz-container');
