...
async function diagPhotos(){
  ...
  // 2. Lire floralab_photos si dispo
  let cloudPhotos = [];
  if(cloudOk){
  ...
  // 3. Analyser
  const total = localPlants.length;
  const localWithPhoto  = localPlants.filter(p=>p.photo||p.photo2||p.photo3||p.photo4);
  const cloudWithPhoto  = cloudPhotos.filter(r=>r.photo||r.photo2||r.photo3||r.photo4);
  const memWithPhoto    = plants.filter(p=>p.photo||p.photo2||p.photo3||p.photo4);

  // Lister les plantes sans photo nulle part
  const missingAll = plants.filter(p=>{
    const lp = localPlants.find(x=>x.id===p.id);
    const cp = cloudPhotos.find(x=>x.plant_id===p.id);
    const hasLocal = lp && (lp.photo||lp.photo2||lp.photo3||lp.photo4);
    const hasCloud = cp && (cp.photo||cp.photo2||cp.photo3||cp.photo4);
    const hasMem   = p.photo||p.photo2||p.photo3||p.photo4;
    return !hasLocal && !hasCloud && !hasMem;
  });

  var msg = '\u{1F4CA} DIAGNOSTIC PHOTOS\n\n';
  msg += '\u{1F33F} Total plantes : '+total+'\n';
  msg += '\u{1F4F1} localStorage : '+localWithPhoto.length+' avec photos\n';
  msg += 'floralab_photos (cloud) : '+cloudWithPhoto.length+' avec photos\n';
  msg += 'Memoire actuelle : '+memWithPhoto.length+' avec photos\n';
  msg += '\nSans photo nulle part : '+missingAll.length+'\n';
  if(missingAll.length && missingAll.length <= 20){
    msg += missingAll.map(function(p){ return '\n  - '+p.latin; }).join('');
  }
  alert(msg);
  console.table(localPlants.map(p=>({
    id:p.id, latin:p.latin,
    local: !!(p.photo||p.photo2||p.photo3||p.photo4),
    cloud: !!(cloudPhotos.find(c=>c.plant_id===p.id&&(c.photo||c.photo2||c.photo3||c.photo4)))
  })));
}

// ── Upload photo vers Supabase Storage (CDN gratuit) ──
...
async function renderScores(){
  ...
  // Cache scores : ne recharger que si les données ont plus de 60s
  const now=Date.now();
  // Recharger si : pas encore chargé, cache > 3min, ou tableau vide
  if(cloudOk && (!window._scoresLastLoad || now-window._scoresLastLoad > 180000 || !scores.length)){
    try{ scores=await sbGet('floralab_scores','select=*&order=pct.desc,ok.desc'); window._scoresLastLoad=now; }catch(e){}
  }
  let data=[...scores];
...
function buildPlantFromForm(){
  ...
  return {
    ...
    photo:(()=>{
      ...
      return (prev&&prev.src&&prev.src!==window.location.href)?prev.src:'';
    })(),
    photo2:(()=>{
...
async function savePlantForm(){
  ...
  // Lecture des photos : nouvelle upload → preview affichée → existante en base
  const _ps  = (s)=> { const e=document.getElementById('photo-preview'+s); return (e&&e.src&&e.src!==window.location.href)?e.src:''; };
  const existing  = (editingId ? (plants.find(x=>x.id==editingId)||{}) : {});
  const photo     = formPhotoB64  || _ps('')  || existing.photo  || '';
  const photo2    = formPhoto2B64 || _ps('2') || existing.photo2 || '';
  const photo3    = formPhoto3B64 || _ps('3') || existing.photo3 || '';
  const photo4    = formPhoto4B64 || _ps('4') || existing.photo4 || '';
