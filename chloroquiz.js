...
function openFilterCat(field){
  ...
  if(field==='feuillage'){
    ...
    const dynClasse=[...new Set(plants.map(p=>p.classe).filter(Boolean))].sort();
    // Fallback sur les 4 valeurs officielles si peu de données encore renseignées
    vals=dynClasse.length>=2?dynClasse:['Bryophyte','Dicotylédone','Gymnosperme','Monocotylédone'];
