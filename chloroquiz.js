...
function _noAcc(s){ return (s||'').normalize('NFD').replace(/[\u0300-\u036f]/g,''); }
function _fmtNom(s){ return _noAcc(s||'').toUpperCase().replace(/[^A-Z0-9\s\-']/g,'').trim(); }
function _fmtPre(s){
  return _noAcc(s||'').toLowerCase()
    .replace(/(?:^|[\s\-])[a-z]/g, m => m.toUpperCase()).trim();
}
function getPlayerName(){
  if(currentUser) return _fmtPre(currentUser.prenom||'')+(currentUser.nom?' '+_fmtNom(currentUser.nom).substring(0,2)+'.':'');
  return '';
}
...
var COULEURS_LIMBE=[
  // Verts (Vert bleuté supprimé ≈ Vert glauque, Vert bronzé supprimé ≈ Bronze/Cuivré)
  {nom:'Vert clair',css:'#a8d878'},{nom:'Vert moyen',css:'#5a9e3a'},{nom:'Vert foncé',css:'#2d6020'},
  {nom:'Vert glauque',css:'#7aad8c'},{nom:'Vert olive',css:'#7a8830'},
  {nom:'Vert luisant',css:'#3a8a20'},{nom:'Jaune-vert',css:'#c8d840'},
  ...
  {nom:'Brun',css:'#7a4820'},
  {nom:'Panaché vert/crème',css:'linear-gradient(135deg,#5a9e3a 50%,#f5f0d8 50%)'},
  {nom:'Panaché vert/jaune',css:'linear-gradient(135deg,#5a9e3a 50%,#d8e060 50%)'},
];
...
var COULEURS_FLEURS = [
  // Blancs & Crèmes (Ivoire supprimé ≈ Crème)
  {nom:'Blanc',             css:'#ffffff'},
  {nom:'Blanc crème',       css:'#f5e6c8'},
  // Jaunes & Verts (Jaune pâle supprimé ≈ Jaune)
  {nom:'Jaune',             css:'#f5e020'},
  {nom:'Jaune vif',         css:'#f0d000'},
  {nom:'Jaune-vert',        css:'linear-gradient(135deg,#c8e020,#6abf30)'},
  {nom:'Vert',              css:'#4caf50'},
  // Oranges (Saumon supprimé ≈ Abricot, Abricot → Orange pâle)
  {nom:'Orange pâle',       css:'#ffb347'},
  {nom:'Orange',            css:'#ff8c00'},
  {nom:'Jaune orangé',      css:'#ffa500'},
  // Roses (Rose vif supprimé ≈ Rose, Magenta supprimé ≈ Rose vif)
  {nom:'Rose pâle',         css:'#ffb6c1'},
  {nom:'Rose',              css:'#ff69b4'},
  // Rouges (Rouge vif supprimé ≈ Rouge)
  {nom:'Rouge',             css:'#e02020'},
  {nom:'Carmin',            css:'#960018'},
  {nom:'Bordeaux',          css:'#722f37'},
  // Violets & Bleus (Lilas supprimé ≈ Mauve, Violet foncé supprimé ≈ Violet, Indigo supprimé ≈ Bleu foncé)
  ...
  // Bruns & Spéciaux (Bronze supprimé ≈ Brun)
  {nom:'Brun',              css:'#8b4513'},
  {nom:'Brun-rouge',        css:'#8b3020'},
  {nom:'Noir',              css:'#1a1a2e'},
  // Bicolores (Rose & Blanc supprimé = doublon Blanc & Rose, Violet & Blanc supprimé = doublon Blanc & Violet)
...
const PW_DEFAULTS = {user:'flora2024', admin:'admin2024'}; // valeur initiale uniquement — à changer dans Admin > Accès
// ── Nom botanique toujours en italique (règle nomenclature) ──
function italLatin(name){
  if(!name) return '';
  // Le × des hybrides et les cultivars entre '' ne sont pas en italique
  return name.replace(/^(.+?)$/,function(m){
    // Split: genre/espèce en italic, × non italique, cultivar '' non italique
    return m.replace(/([A-Z][a-zàâäéèêëîïôùûüæœ×][\w×àâäéèêëîïôùûüæœ\.\-]*(?:\s+[a-z×àâäéèêëîïôùûüæœ\-\.][\w×àâäéèêëîïôùûüæœ\.\-]*)*)/g,
      '<em>$1</em>');
  }).replace(/<em>(×)<\/em>/g,'×').replace(/<em>('[^']*')<\/em>/g,'$1');
}

// ── Noms communs multiples (séparés par ' / ') ──
function firstNom(nom){ return nom?(nom.split(' / ')[0].trim()||nom):nom; }
function allNoms(nom){ return nom?nom.split(' / ').map(s=>s.trim()).filter(Boolean):[]; }
...
const SB_DEFAULT_URL = 'https://api.chloroquiz.fr';
const SB_DEFAULT_KEY = 'local';
let plantnetKey = ''; // chargée depuis Supabase au login — jamais exposée côté apprenti
...
-- Table pour les fiches partagées entre apprentis (et vers admin)
create table if not exists floralab_plant_shares (
  id bigserial primary key,
  sender_name text not null,
  sender_share_code text,
...
create policy "public_all_shares" on floralab_plant_shares
  for all using (true) with check (true);`;

// ── NIVEAU DÉCOUVERTE (0) : critères supplémentaires optionnels ──
// Questions posées avec le nom commun (pas le nom botanique)
const KFIELDS_N0 = [
  {key:'exposition',       label:'Exposition',            q:n=>`Quelle est l'exposition idéale pour ${n} ?`},
  {key:'type',             label:'Type de végétal',       q:n=>`Quel est le type de végétal de ${n} ?`},
  {key:'feuillage',        label:'Feuillage',             q:n=>`Quel est le comportement foliaire de ${n} ?`},
  {key:'periodeFloraison', label:'Période de floraison',  q:n=>`Quelle est la période de floraison de ${n} ?`},
  {key:'couleurFleurs',    label:'Couleur de floraison',  q:n=>`Quelle est la couleur des fleurs de ${n} ?`},
];

// ── CHIPS par niveau : chaque chip active des types de questions ──
// keys = champs activés dans qEnabledChips quand la chip est cochée
const CHIPS_BY_LEVEL = {
  0:[
    {key:'exposition',        label:'☀️ Exposition',            keys:['exposition']},
    {key:'type',              label:'🌿 Type de végétal',       keys:['type']},
    {key:'feuillage',         label:'🍃 Feuillage',             keys:['feuillage']},
    {key:'periodeFloraison',  label:'🌸 Floraison',             keys:['periodeFloraison']},
    {key:'couleurFleurs',     label:'🎨 Couleur fleurs',        keys:['couleurFleurs']},
  ],
  1:[
    {key:'exposition',        label:'☀️ Exposition',            keys:['exposition']},
    {key:'type_rusticite',    label:'🌿 Type & Rusticité',      keys:['type','rusticite']},
    {key:'feuillage',         label:'🍃 Feuillage',             keys:['feuillage']},
    {key:'periodeFloraison',  label:'🌸 Floraison',             keys:['periodeFloraison']},
    {key:'couleurFleurs',     label:'🎨 Couleur fleurs',        keys:['couleurFleurs']},
  ],
  2:[
    ...
    {key:'periodeFloraison',  label:'🌸 Floraison',             keys:['periodeFloraison']},
    {key:'couleurFleurs',     label:'🎨 Couleur fleurs',        keys:['couleurFleurs']},
    {key:'ph',                label:'🧪 pH sol',                keys:['ph']},
    {key:'humidite',          label:'💧 Humidité sol',          keys:['humidite']},
    {key:'structureSol',      label:'🌱 Structure sol',         keys:['structureSol']},
    {key:'port',              label:'🌳 Port',                  keys:['port']},
    {key:'vitesseCroissance', label:'⚡ Vitesse croissance',    keys:['vitesseCroissance']},
    {key:'typeTaille',        label:'✂️ Type de taille',        keys:['typeTaille']},
    {key:'frequenceTaille',   label:'📅 Fréquence taille',      keys:['frequenceTaille']},
    {key:'interetOrnemental', label:'✨ Intérêt ornemental',    keys:['interetOrnemental']},
    {key:'autresInterets',    label:'🌿 Autres intérêts',       keys:['autresInterets']},
    {key:'usageAmenagement',  label:'🏡 Usage',                 keys:['usageAmenagement']},
    {key:'reproduction',      label:'🔄 Reproduction',          keys:['reproduction']},
    {key:'pollinisation',     label:'🐝 Pollinisation',         keys:['pollinisation']},
    {key:'biodiversite',      label:'🦋 Biodiversité',          keys:['biodiversite']},
    {key:'particularites',    label:'⚠️ Particularités',        keys:['particularites']},
  ],
  3:[
    {key:'exposition',        label:'☀️ Exposition',            keys:['exposition']},
    {key:'type_rusticite',    label:'🌿 Type & Rusticité',      keys:['type','rusticite']},
    ...
    {key:'biodiversite',      label:'🦋 Biodiversité',          keys:['biodiversite']},
    {key:'particularites',    label:'⚠️ Particularités',        keys:['particularites']},
    {key:'famille',           label:'🔬 Famille',               keys:['famille']},
    {key:'classe',            label:'🧬 Classe',                keys:['classe']},
    {key:'couleurLimbe',      label:'🍀 Couleur feuillage',     keys:['couleurLimbe']},
    {key:'formeLimbe',        label:'🍃 Forme feuilles',        keys:['formeLimbe']},
    {key:'texture',           label:'✋ Texture feuillage',     keys:['texture']},
  ],
};
...
const FILLERS = {
  ...
  couleurFleurs:['Rouge','Bleu','Orange','Blanc crème','Rose','Violet','Jaune','Mauve','Rose pâle','Bordeaux','Bleu ciel'],
...
let currentUser = null; // {id, nom, prenom, mail, code, role, actif}
let lbFilter = 'all';
...
let qEnabledChips=new Set(); // clés de champs activés par les chips (vide = quiz de base uniquement)
let qSelectedPlants=[]; // plantes sélectionnées par l'apprenti (vide = toutes)
...
let _photoReady = false;      // true quand la photo principale est chargée
let _timerPendingCb = null;   // callback onExpire en attente du chargement
let _timerPendingDur = null;  // durée en attente
let _photoRetryCount = 0;     // nombre de rechargements tentés
...
let qActiveFields=new Set(); // champs actifs pour les questions (vide = tous)
...
const SK_MASTERY='cq_mastery_v1'; // maîtrise par végétal et par apprenti
const SK_DRAFTS='cq_drafts_v1'; // fiches admin préparées, non publiées
let draftPlants=[]; // fiches en attente de publication
// ── Mes végétaux (fiches personnelles apprenti) ──
const SK_MY_PLANTS   = 'cq_myplants_v1';   // clé base — suffixée par le nom de l'apprenti
const SK_SHARE_CODE  = 'cq_share_code';    // code de partage unique 6 chars
let myPlants = [];         // plantes perso de l'apprenti connecté
let myPlantsOwner = null;  // nom de l'apprenti propriétaire
...
const SK_USER       = 'cq_current_user';
const SK_USERS_CACHE= 'cq_users_cache';
...
//  DATA LAYER — plants
// ══════════════════════════════════════════════════════
async function loadPlants(forceRefresh){
  if(cloudOk){
    // Cache session : ne recharger Supabase qu'une fois par session sauf si forcé
    if(!forceRefresh && window._plantsSessionLoaded){
      plantsFromLocal(); // utiliser le cache local
      return;
    }
    try{
      const rows = await sbGet('floralab_data','key=eq.plants&select=value');
      if(rows.length){
        // Fusionner : photos base64 du localStorage + données texte de Supabase
        const cloudPlants = rows[0].value.map(p=>({...p,famille:normalizeFamille(p.famille||'')||p.famille}));
        let localPlants = [];
        try{ const r=localStorage.getItem(SK_PLANTS); localPlants=r?JSON.parse(r):[]; }catch(e){}
        // Restaurer les photos depuis localStorage si Supabase n'en a pas (URL ou base64)
        let photosRestored = 0;
        plants = cloudPlants.map(cp=>{
          const lp = localPlants.find(p=>p.id===cp.id);
          if(lp){
            if(!cp.photo  && lp.photo)  { cp.photo  = lp.photo;  photosRestored++; }
            if(!cp.photo2 && lp.photo2) { cp.photo2 = lp.photo2; photosRestored++; }
            if(!cp.photo3 && lp.photo3) { cp.photo3 = lp.photo3; photosRestored++; }
            if(!cp.photo4 && lp.photo4) { cp.photo4 = lp.photo4; photosRestored++; }
          }
          return cp;
        });
        // Si des photos ont été restaurées depuis le local, sauvegarder vers Supabase
        if(photosRestored > 0){
          console.log('[CQ] '+photosRestored+' photo(s) restaurées → envoi vers floralab_photos');
          setTimeout(async function(){
            try{ await savePhotosCloud(); } catch(e){ console.warn('Photo repair failed',e); }
          }, 2000);
        }
      } else plants = JSON.parse(JSON.stringify(DEFAULT_PLANTS));
      // Charger les photos depuis floralab_photos
      await loadPhotosCloud();
      // Nettoyer les anciennes URLs externes (ni base64 ni CDN) des plantes
      var storB = (sbConf) ? sbConf.url+'/storage/v1/object/public/chloroquiz-photos/' : '';
      plants.forEach(function(p){
        ['photo','photo2','photo3','photo4'].forEach(function(k){
          var v = p[k]||'';
          if(v && !v.startsWith('data:') && storB && !v.startsWith(storB)){
            console.log('[CQ] URL externe nettoyée: '+p.latin+' ('+k+')');
            p[k] = '';
          }
        });
      });
      window._plantsSessionLoaded = true;
      localSavePlants(); // mettre en cache local (avec photos) pour usage offline
...
async function savePlants(){
  localSavePlants(); // toujours sauvegarder localement en premier
  if(cloudOk){
    try{
      // Sauvegarder les données texte SANS photos (léger)
      const plantsNoPhotos = plants.map(p=>({
        ...p, photo:'', photo2:'', photo3:'', photo4:''
      }));
      await sbPost('floralab_data',{key:'plants',value:plantsNoPhotos,updated_at:new Date().toISOString()},'resolution=merge-duplicates');
      // Sauvegarder les photos séparément (table floralab_photos)
      await savePhotosCloud();
      flashSaved();
    } catch(e){ console.warn('Supabase save plants fail',e); }
  }
}

// ── Sauvegarde des photos dans floralab_photos (table dédiée) ──
async function savePhotosCloud(){
  if(!cloudOk) return;
  // Collecter toutes les plantes avec au moins une photo
  // Ne sauvegarder que les plantes avec au moins une photo
  // et exclure les photos vides pour réduire la taille du payload
  const photosToSave = plants
    .filter(p=>p.photo||p.photo2||p.photo3||p.photo4)
    .map(p=>({
      plant_id: p.id,
      plant_latin: p.latin||'',
      photo:  p.photo  ||'',
  ...
  // Upsert par batch de 5 (photos WebP ~100Ko chacune, max ~2Mo par requête)
  for(let i=0;i<photosToSave.length;i+=5){
    const batch = photosToSave.slice(i,i+5);
    try{
      await sbPost('floralab_photos', batch, 'resolution=merge-duplicates');
    } catch(e){ console.warn('Photo batch save fail (batch '+i+'):', e); }
  }
}

// ── Chargement des photos depuis floralab_photos ──
async function loadPhotosCloud(forceRefresh){
  if(!cloudOk) return;
  // Cache session : ne recharger les photos qu'une fois par session
  if(!forceRefresh && window._photosSessionLoaded) return;
  try{
    const rows = await sbGet('floralab_photos','select=plant_id,photo,photo2,photo3,photo4');
    rows.forEach(r=>{
      const p = plants.find(x=>x.id===r.plant_id);
      if(p){
        if(r.photo)  p.photo  = r.photo;
        if(r.photo2) p.photo2 = r.photo2;
        if(r.photo3) p.photo3 = r.photo3;
        if(r.photo4) p.photo4 = r.photo4;
      }
    });
    window._photosSessionLoaded = true;
    localSavePlants(); // mettre en cache avec photos
  } catch(e){ console.warn('loadPhotosCloud fail:', e); }
}
...
// ── Diagnostic complet des photos ──
async function diagPhotos(){
  ...
  let cloudPhotos = [];
  if(cloudOk){
    try{ cloudPhotos = await sbGet('floralab_photos','select=plant_id,photo,photo2,photo3,photo4'); }catch(e){}
  }
  const total = plants.length;
  // Catégoriser chaque plante
  const onCDN = plants.filter(p=>
    [p.photo,p.photo2,p.photo3,p.photo4].some(x=>x&&storageBase&&x.startsWith(storageBase))
  );
  const onLocalB64 = plants.filter(p=>{
    if(onCDN.find(x=>x.id===p.id)) return false;
    const lp=localPlants.find(x=>x.id===p.id);
    return lp&&[lp.photo,lp.photo2,lp.photo3,lp.photo4].some(x=>x&&x.startsWith('data:'));
  });
  const onCloudPhoto = cloudPhotos.filter(r=>r.photo||r.photo2||r.photo3||r.photo4);
  const missing = plants.filter(p=>{
    if(onCDN.find(x=>x.id===p.id)) return false;
    if(onLocalB64.find(x=>x.id===p.id)) return false;
    const lp=localPlants.find(x=>x.id===p.id);
    if(lp&&[lp.photo,lp.photo2,lp.photo3,lp.photo4].some(x=>x&&x.trim())) return false;
    const cp=cloudPhotos.find(x=>x.plant_id===p.id);
    if(cp&&(cp.photo||cp.photo2||cp.photo3||cp.photo4)) return false;
    return ![p.photo,p.photo2,p.photo3,p.photo4].some(x=>x&&x.trim());
  });
  var msg = 'DIAGNOSTIC PHOTOS — '+total+' plantes\n\n';
  msg += '✅ Sur Storage CDN : '+onCDN.length+'\n';
  msg += '💾 Base64 locale (a migrer) : '+onLocalB64.length+'\n';
  msg += 'floralab_photos : '+onCloudPhoto.length+'\n';
  msg += '\nSans photo nulle part : '+missing.length;
  if(missing.length && missing.length<=30){
    msg += missing.map(function(p){return '\n  - '+p.latin;}).join('');
  }
  alert(msg);
  console.table(plants.map(p=>({
    id:p.id, latin:p.latin,
    cdn:  !!(onCDN.find(x=>x.id===p.id)),
    local:!!(onLocalB64.find(x=>x.id===p.id)),
    cloud:!!(cloudPhotos.find(c=>c.plant_id===p.id&&(c.photo||c.photo2||c.photo3||c.photo4))),
    mem:  !!(p.photo||p.photo2||p.photo3||p.photo4)
  })));
}

async function uploadPhotoToStorage(b64, plantLatin, slot){
  if(!cloudOk || !b64 || !b64.startsWith('data:')) return b64; // si pas cloud → garder base64 local
  try{
    // Convertir base64 → Blob
    const parts = b64.split(',');
    const mime = parts[0].match(/:(.*?);/)[1]; // image/webp ou image/jpeg
    ...
    // Nom de fichier : latin_slot.ext (ex: rosa_canina_1.webp)
    const safeLatin = (plantLatin||'unknown').replace(/[^a-z0-9]/gi,'_').toLowerCase();
    const path = safeLatin+'_'+slot+'.'+ext;

    // Upload vers Supabase Storage bucket "chloroquiz-photos"
    const uploadUrl = sbConf.url+'/storage/v1/object/chloroquiz-photos/'+path;
    ...
    const publicUrl = sbConf.url+'/storage/v1/object/public/chloroquiz-photos/'+path;
    return publicUrl;
  } catch(e){
    console.warn('[CQ] uploadPhotoToStorage failed:', e);
    return b64; // fallback sur base64 si erreur
  }
}

// ── Migrer les photos base64 locales vers Supabase Storage ──
async function migratePhotosToStorage(){
  ...
  prog.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:#0d2b1a;border:1px solid rgba(74,184,112,.3);border-radius:14px;padding:24px 32px;z-index:99999;color:#4ab870;font-size:15px;font-weight:700;text-align:center;min-width:280px';
  prog.id = 'storage-prog';
  ...
  // Sauvegarder avec les nouvelles URLs
  localSavePlants();
  await savePhotosCloud();
  await savePlants();
  renderAdmin();
  alert('✅ Migration terminée !\n\n☁️ '+done+' photos migrées vers Storage CDN\n'+(errors?'⚠️ '+errors+' erreur(s)\n':'')+'\nL\'egress Supabase est maintenant quasi nul !');
}

// ── Réparer les photos cloud après bug strip-URL ──
async function repairPhotosCloud(){
  if(!cloudOk){ alert('Non connecté à Supabase.'); return; }
  let localPlants = [];
  try{ localPlants = JSON.parse(localStorage.getItem(SK_PLANTS)||'[]'); } catch(e){}
  if(!localPlants.length){ alert('❌ Aucune donnée locale trouvée.'); return; }

  const withPhotos = localPlants.filter(p=>p.photo||p.photo2||p.photo3||p.photo4);
  if(!confirm('🔧 Envoyer les photos vers Supabase (table floralab_photos) ?\n\n'
    +withPhotos.length+' plante(s) avec photos trouvées en local.\nEnvoi par batch de 5 pour éviter les timeouts.\n\nContinuer ?')) return;

  // Fusionner photos locales → plants en mémoire
  plants = plants.map(cp=>{
    const lp = localPlants.find(p=>p.id===cp.id);
    if(lp){
      if(!cp.photo  && lp.photo)  cp.photo  = lp.photo;
      if(!cp.photo2 && lp.photo2) cp.photo2 = lp.photo2;
      if(!cp.photo3 && lp.photo3) cp.photo3 = lp.photo3;
      if(!cp.photo4 && lp.photo4) cp.photo4 = lp.photo4;
    }
    return cp;
  });
  ...
  try{
    ...
    localSavePlants();
    renderAdmin();
    const recovered = plants.filter(p=>p.photo||p.photo2||p.photo3||p.photo4).length;
    alert('✅ Photos envoyées dans floralab_photos !\n\n📸 '+recovered+' plante(s) avec photos\nLes apprentis verront les photos au prochain chargement.');
  } catch(e){
...
async function loadScores(forceRefresh){
  if(cloudOk){
    const now=Date.now();
    if(!forceRefresh && window._scoresLoaded && now-window._scoresLoaded < 300000){
      scoresFromLocal(); return; // cache 5 min
    }
    try{
      const fromCloud = await sbGet('floralab_scores','select=*&order=pct.desc,ok.desc');
      window._scoresLoaded=now;
      // Fusionner avec localStorage pour ne jamais perdre les anciens scores
      let fromLocal = [];
      try{ fromLocal = JSON.parse(localStorage.getItem(SK_SCORES)||'[]'); }catch(e){}
      // Dedoublonner : cle = nom+date+pct
      const cloudKeys = new Set(fromCloud.map(s=>s.name+'|'+s.date+'|'+s.pct));
      const onlyLocal = fromLocal.filter(s=>!cloudKeys.has((s.name||'')+'|'+(s.date||0)+'|'+(s.pct||0)));
      scores = [...fromCloud, ...onlyLocal].sort((a,b)=>b.pct-a.pct||b.ok-a.ok);
      // Sauvegarder en localStorage pour le mode hors-ligne et le cache
...
async function addScore(s, sLocal){
  // sLocal = objet complet (avec startTime) pour localStorage
  // s = objet allégé pour Supabase (colonnes exactes de la table)
  const forLocal = sLocal || s;
  if(cloudOk){
    try{
      await sbPost('floralab_scores',s,'return=minimal');
      scoresLocalAdd(forLocal); // aussi en local pour renderEnd immédiat
      window._scoresLoaded = null; // invalider cache pour prochain affichage classement
      syncOfflineQueue();
    } catch(e){
      console.warn('Supabase add score fail',e);
      scoresLocalAdd(forLocal);
      addToOfflineQueue(s);
      showOfflineBanner(1);
    }
  } else {
...
async function forceSyncScores(){
  // Synchroniser la file offline + récupérer les scores du localStorage
  let q=[];
  try{ q=JSON.parse(localStorage.getItem(SK_OFFLINE_QUEUE)||'[]'); }catch(e){}
  // Aussi récupérer les scores locaux non envoyés
  let localScores=[];
  try{ localScores=JSON.parse(localStorage.getItem(SK_SCORES)||'[]'); }catch(e){}
  // Dédoublonner avec ce qui est déjà dans Supabase
  const allToSync=[...q,...localScores];
  if(!allToSync.length){ alert('Aucun score local à synchroniser.'); return; }
  if(!cloudOk){ alert('Connexion Supabase requise.'); return; }
  let ok=0, fail=0;
  for(const s of allToSync){
    const sCloud={name:s.name,level:s.level||1,ok:s.ok||0,total:s.total||20,pct:s.pct||0,timeouts:s.timeouts||0,date:s.date||Date.now()};
    try{ await sbPost('floralab_scores',sCloud,'return=minimal'); ok++; }
    catch(e){ fail++; }
  }
  // Vider la queue si tout ok
  if(fail===0) try{ localStorage.setItem(SK_OFFLINE_QUEUE,'[]'); }catch(e){}
  // Recharger
  window._scoresLoaded=null;
  await loadScores(true);
  alert('Synchronisation : '+ok+' score(s) envoyes'+(fail?' - '+fail+' erreur(s)':'')+'\n\nClassement mis a jour.');
  renderAdmin();
}
...
async function loadPasswords(){
  // Essayer cloud avec retry x3
  if(cloudOk){
    for(var _r=0; _r<3; _r++){
      try{
        const rows = await sbGet('floralab_data','key=eq.passwords&select=value');
        if(rows.length){
          PW_USER=rows[0].value.user;
          PW_ADMIN=rows[0].value.admin;
          // Mettre en cache local pour les prochains lancements
          try{ localStorage.setItem(SK_PASSWORDS,JSON.stringify({user:PW_USER,admin:PW_ADMIN})); }catch(e){}
          return;
        }
        break; // Requête ok mais aucune ligne → première installation
      } catch(e){
        if(_r<2) await new Promise(function(res){setTimeout(res,800);});
      }
    }
  }
  // Fallback localStorage (contient le dernier mot de passe connu)
  try{
    const saved = JSON.parse(localStorage.getItem(SK_PASSWORDS));
    if(saved && saved.user && saved.admin){
      PW_USER=saved.user;
      PW_ADMIN=saved.admin;
      return; // Ne jamais tomber sur les défauts si localStorage est valide
    }
  } catch(e){}
  // Défauts uniquement si aucun mot de passe nulle part (première installation)
  if(!PW_USER)  PW_USER  = PW_DEFAULTS.user;
  if(!PW_ADMIN) PW_ADMIN = PW_DEFAULTS.admin;
}
...
function loadSbConf(){
  ...
  // Exposer globalement pour les fonctions user management
  window.SUPA_URL = sbConf.url;
  window.SUPA_KEY = sbConf.key;
}
...
//  AUTH
// ══════════════════════════════════════════════════════
// Note: la touche Entrée est gérée via onkeydown sur #login-code dans le HTML
async function checkLogin(){
  ...
  if(btn){ btn.disabled=true; btn.innerHTML='<span class="spinner" style="width:16px;height:16px;border-width:2px;vertical-align:middle"></span> Connexion…'; }
  if(errEl) errEl.textContent='';
  try{
    if(!PW_USER || !PW_ADMIN) await loadPasswords();
    let user=null;
    // Chercher dans Supabase
    if(sbConf && window.SUPA_URL && window.SUPA_KEY){
      try{
        const hdr={'apikey':window.SUPA_KEY,'Authorization':'Bearer '+window.SUPA_KEY};
        const res=await fetch(window.SUPA_URL+'/rest/v1/floralab_users?code=eq.'+encodeURIComponent(code)+'&select=*&limit=1',{headers:hdr});
        if(res.ok){ const rows=await res.json(); if(rows&&rows.length) user=rows[0]; }
      }catch(e){ cqWarn('Supabase users lookup failed',e); }
    }
    // Fallback cache localStorage
    if(!user){
      try{ const c=JSON.parse(localStorage.getItem(SK_USERS_CACHE)||'[]'); user=c.find(u=>u.code===code)||null; }catch(e){}
    }
    // Fallback legacy passwords
    if(!user){
      if(PW_ADMIN && code===PW_ADMIN)
        user={id:'legacy-admin',nom:'ADMIN',prenom:'Admin',code:PW_ADMIN,role:'admin',actif:true};
      else if(PW_USER && code===PW_USER)
        user={id:'legacy-user',nom:'',prenom:'Visiteur',code:PW_USER,role:'apprenti',actif:true};
    }
    if(!user){ if(errEl) errEl.textContent='❌ Code introuvable'; }
    else if(!user.actif){ if(errEl) errEl.textContent='❌ Compte désactivé — contactez votre formateur'; }
    else{ await login(user); }
  }catch(e){
    cqErr('checkLogin',e);
    if(errEl) errEl.textContent='❌ Erreur de connexion';
  }
  if(btn){ btn.disabled=false; btn.innerHTML='Accéder à ChloroQuiz →'; }
}
// Alias pour compatibilité
function checkPassword(){ checkLogin(); }
...
function saveSession(){
  try{
    localStorage.setItem(SK_SESSION, JSON.stringify({role, userId:currentUser?.id||null, ts:Date.now()}));
...
function clearSession(){
  try{
    ...
    localStorage.removeItem(SK_QUIZ_STATE);
...
async function tryRestoreSession(){
  ...
  try{
    ...
    // Restaurer currentUser depuis SK_USER
    try{ const u=localStorage.getItem(SK_USER); if(u) currentUser=JSON.parse(u); }catch(e){}
    // Si pas de currentUser reconstruit un objet legacy
    if(!currentUser){
      if(sess.role==='admin') currentUser={id:'legacy-admin',nom:'ADMIN',prenom:'Admin',code:'',role:'admin',actif:true};
      else currentUser={id:'legacy-user',nom:'',prenom:'Visiteur',code:'',role:'apprenti',actif:true};
    }
    ...
    if(qRaw){
      ...
      ['home','quiz','fiches','scores','admin','stats','myplants'].forEach(id=>{
...
// ─────────────────────────────────────────────────────
// _applyRoleUI : affiche/masque les éléments selon le rôle
// ─────────────────────────────────────────────────────
function _applyRoleUI(){
  const r = role||'';
  // Éléments admin (admin uniquement)
  const isAdmin = r==='admin';
  // Éléments formateur ou admin
  const isFormateurPlus = r==='formateur' || isAdmin;
  // Éléments apprenti, formateur ou admin (pas amateur)
  const isApprentiPlus = r==='apprenti' || isFormateurPlus;

  // Nav items
  _showEl('nb-admin',    isFormateurPlus);
  _showEl('nb-stats',    isFormateurPlus);
  _showEl('nb-myplants', isApprentiPlus);
  // Home cards
  _showEl('home-admin-card', isFormateurPlus);
  _showEl('home-peer-card',  isApprentiPlus);
  // Leaderboard reset
  _showEl('lb-reset-btn', isAdmin);
  // Bottom nav admin
  const bnAdm=document.getElementById('bn-admin-mob');
  if(bnAdm) bnAdm.style.display = isFormateurPlus ? '' : 'none';
  // Role badge
  const badge=document.getElementById('role-badge');
  if(badge){
    badge.classList.remove('admin');
    if(isAdmin){ badge.textContent='👑 Administrateur'; badge.classList.add('admin'); }
    else if(r==='formateur'){ badge.textContent='🎓 Formateur'; badge.classList.add('admin'); }
    else if(r==='apprenti'){ badge.textContent='🌱 Apprenti'; }
    else if(r==='amateur'){ badge.textContent='🌿 Amateur'; }
    else { badge.textContent='👤 Visiteur'; }
  }
  // Admin sidebar : onglet Utilisateurs visible admin seulement
  const atUsers=document.getElementById('atab-users');
  if(atUsers) atUsers.style.display = isAdmin ? '' : 'none';
  // Admin sidebar : onglet Multi visible formateur+
  const atMulti=document.getElementById('atab-multi');
  if(atMulti) atMulti.style.display = isFormateurPlus ? '' : 'none';
  // Admin sidebar : cloud/mdp/plantnet ADMIN SEULEMENT (pas formateur)
  ['cloud','mdp','plantnet'].forEach(function(t){
    var el=document.getElementById('atab-'+t); if(el) el.style.display=isAdmin?'':'none';
  });
  // Fiches : boutons créer/modifier visibles formateur+
  document.querySelectorAll('.fiche-admin-btn').forEach(el=>{ el.style.display=isFormateurPlus?'':'none'; });
  // Reset classement : formateur+ (pas amateur)
  _showEl('lb-reset-btn', isFormateurPlus);
  // Onglet messages visible si connecté (pas visiteur legacy)
  const hasAccount = currentUser && currentUser.id && !currentUser.id.startsWith('legacy');
  _showEl('nb-messages', hasAccount || isAdmin || isFormateurPlus || isApprentiPlus);
  const bnMsg=document.getElementById('bn-messages'); if(bnMsg) bnMsg.style.display=(hasAccount||isAdmin||isFormateurPlus||isApprentiPlus)?'':'none';
  // Badge notifs demandes en attente (admin)
  if(isAdmin) _checkPendingRequests();
  // Lancer poll messages si connecté
  if(currentUser && currentUser.id) _startMsgPoll();
}
...
async function _checkPendingRequests(){
  if(!cloudOk || !window.SUPA_URL || !window.SUPA_KEY) return;
  try{
    const hdr={'apikey':window.SUPA_KEY,'Authorization':'Bearer '+window.SUPA_KEY,'Prefer':'count=exact','Range':'0-0'};
    const res=await fetch(window.SUPA_URL+'/rest/v1/floralab_requests?statut=eq.pending&select=id',{headers:hdr});
    const n=parseInt((res.headers.get('content-range')||'').split('/')[1])||0;
    // Badge dans la sidebar admin
    const badge=document.getElementById('sidebar-badge-users');
    if(badge){ badge.textContent=n||''; badge.style.display=n?'':'none'; }
  }catch(e){}
}

async function login(userObj, silent=false){
  // Accepte soit un objet user, soit une string legacy ('admin'/'user')
  if(typeof userObj==='string'){
    userObj = userObj==='admin'
      ? {id:'legacy-admin',nom:'ADMIN',prenom:'Admin',code:'',role:'admin',actif:true}
      : {id:'legacy-user',nom:'',prenom:'Visiteur',code:'',role:'apprenti',actif:true};
  }
  currentUser = userObj;
  role = userObj.role||'apprenti';
  loadSbConf();
  saveSession();
  ...
  updateCloudDot();
  if(sbConf){
    try{ await testConnection(sbConf.url, sbConf.key); cloudOk=true; }
...
function stopAllPolls(){
  ...
  if(typeof sessionPollI!=='undefined' && sessionPollI){ clearInterval(sessionPollI); sessionPollI=null; }
  if(typeof msgPollInterval!=='undefined' && msgPollInterval){ clearInterval(msgPollInterval); msgPollInterval=null; }
}

function logout(){
  stopAllPolls(); role=null; currentUser=null; cloudOk=false;
  window._plantsSessionLoaded = false;
  window._photosSessionLoaded = false;
  window._scoresLoaded = null;
  window._scoresLastLoad = null;
  clearSession();
  document.getElementById('app-screen').style.display='none';
  document.getElementById('pw-screen').style.display='flex';
  // Reset champs login
  ['login-nom','login-prenom','login-code'].forEach(id=>{ const el=document.getElementById(id); if(el) el.value=''; });
  const errEl=document.getElementById('pw-err'); if(errEl) errEl.textContent='';
  // Reset UI rôle
  ['nb-admin','nb-stats','home-admin-card','home-peer-card'].forEach(id=>{ const el=document.getElementById(id); if(el)el.style.display='none'; });
  _showEl('lb-reset-btn',false);
  const badge=document.getElementById('role-badge');
  if(badge){ badge.textContent='👤 Visiteur'; badge.classList.remove('admin'); }
...
function showPage(p){
  stopTimer();
  ['home','quiz','fiches','scores','admin','stats','myplants','messages'].forEach(id=>{
    const pg=document.getElementById('page-'+id); if(pg) pg.style.display='none';
    const nb=document.getElementById('nb-'+id); if(nb) nb.classList.remove('active');
  });
  ...
  // Sync bottom nav
  ['home','quiz','fiches','scores','myplants','messages'].forEach(id=>{
  ...
  if(p==='myplants') renderMyPlants();
  if(p==='messages') renderMessages();
  if(p==='stats'){
    if(role!=='admin' && role!=='formateur'){ showPage('home'); return; }
    renderStats();
...
function startTimer(onExpire, duration){
  // Si la photo principale n'est pas encore chargée : mettre en attente
  if(!_photoReady){
    _timerPendingCb  = onExpire;
    _timerPendingDur = duration || TIMER_SEC;
    return;
  }
  _timerPendingCb = null;
...
// ── Callbacks chargement photo principale du quiz ──
function onMainPhotoLoaded(){
  _photoReady = true;
  _photoRetryCount = 0;
  // Masquer le spinner si présent
  var spin = document.getElementById('photo-loading-spinner');
  if(spin) spin.style.display = 'none';
  // Lancer le timer si en attente
  if(_timerPendingCb){
    var cb = _timerPendingCb;
    var dur = _timerPendingDur;
    _timerPendingCb = null;
    startTimer(cb, dur);
  }
}

function onMainPhotoError(img){
  var spin = document.getElementById('photo-loading-spinner');
  if(_photoRetryCount < 2){
    // Retry automatique jusqu'à 2 fois (cache-buster)
    _photoRetryCount++;
    if(spin){ spin.style.display='flex'; spin.innerHTML='<div style="font-size:13px;color:rgba(200,223,204,.5)">🔄 Chargement… ('+_photoRetryCount+'/2)</div>'; }
    setTimeout(function(){
      var src = img.src.split('?')[0];
      img.src = src + '?r=' + _photoRetryCount;
    }, 1500);
  } else {
    // Échec définitif : afficher message + permettre de continuer
    if(spin){ spin.style.display='flex'; spin.innerHTML='<div style="text-align:center"><div style="font-size:28px;margin-bottom:8px">📵</div><div style="font-size:13px;color:rgba(200,223,204,.6);margin-bottom:12px">Photo indisponible<br><span style=\\"font-size:11px;color:rgba(200,223,204,.35)\\">Problème réseau ou image manquante</span></div><button onclick=\\"retryPhotoLoad()\\" style=\\"padding:8px 18px;background:rgba(74,184,112,.15);border:1px solid rgba(74,184,112,.35);border-radius:10px;color:#4ab870;font-size:13px;cursor:pointer\\">🔄 Réessayer</button></div>'; }
    img.style.display = 'none';
    // Le timer ne démarre pas — l'apprenti doit réessayer
  }
}

function retryPhotoLoad(){
  _photoRetryCount = 0;
  var img = document.getElementById('qphoto-main-0');
  if(!img) return;
  var spin = document.getElementById('photo-loading-spinner');
  if(spin){ spin.style.display='flex'; spin.innerHTML='<div style="font-size:13px;color:rgba(200,223,204,.5)">🔄 Rechargement…</div>'; }
  img.style.display = '';
  var src = img.src.split('?')[0];
  img.src = src + '?r=retry' + Date.now();
}
...
function renderNamePrompt(c){
  // Pré-remplir depuis currentUser si disponible
  const prefillName = currentUser ? _fmtPre(currentUser.prenom||'') : '';
  const _pre = currentUser ? _fmtPre(currentUser.prenom||'') : '';
  const _nom = currentUser ? _fmtNom(currentUser.nom||'') : '';
  const prefillInit = (_pre.charAt(0) + _nom.charAt(0)).toUpperCase();
  ...
  c.innerHTML=`
    <div class="name-prompt">
      <span class="quiz-start-icon">🌿</span>
      <div class="quiz-start-title">Prêt pour le Quiz ?</div>
      <div class="quiz-start-subtitle">Entrez votre prénom puis choisissez votre niveau</div>
      <div class="quiz-inputs-row">
        <input class="quiz-input-name" id="player-name-input" placeholder="Prénom…" autocomplete="off" maxlength="30" value="${prefillName}"/>
        <input class="quiz-input-init" id="player-initial-input" placeholder="AB" autocomplete="off" maxlength="2" value="${prefillInit}"/>
      </div>
      <div class="quiz-initials-hint">
        📝 2 premières lettres de ton nom de famille (ex : <strong style="color:rgba(200,223,204,.95)">Dupont → DU</strong>)
      </div>
      <div class="quiz-level-label">Choisissez votre niveau</div>
      <div class="level-cards" style="grid-template-columns:1fr 1fr">
        <button class="level-card" id="lv0-card" onclick="selectLevel(0)"
          style="border:1px solid rgba(100,200,255,.3);background:rgba(100,200,255,.05);grid-column:1/-1">
          <span class="level-badge-tag" style="background:rgba(100,200,255,.2);color:#7ab8f5">Découverte</span>
          <span class="level-icon">👀</span>
          <div class="level-title" style="color:#7ab8f5">Niveau 0</div>
          <div class="level-desc">Photo → choisir le <strong style="color:rgba(200,223,204,.7)">nom commun</strong> parmi 4 propositions · idéal pour débuter</div>
        </button>
      ...
      <div class="filter-section" style="margin-top:14px">
        <div class="filter-title">&#128269; Révision ciblée — filtrer les plantes</div>
        <div style="font-size:11px;color:rgba(200,223,204,.5);margin-bottom:8px">Sélectionne un filtre pour limiter le quiz à un sous-ensemble</div>
        <div id="filter-cats" style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:0"></div>
            </div>

      <div class="extra-chips-section" style="margin-top:12px">
        <div style="font-size:10px;text-transform:uppercase;letter-spacing:1.5px;color:rgba(200,223,204,.4);margin-bottom:8px">
          ➕ Critères supplémentaires <span style="font-size:9px;color:rgba(200,223,204,.25);text-transform:none;letter-spacing:0">(optionnel — enrichit le quiz)</span>
        </div>
        <div id="extra-chips-wrap" style="display:flex;gap:6px;flex-wrap:wrap"></div>
        <div class="extra-chips-hint" style="font-size:10px;color:rgba(200,223,204,.3);margin-top:5px">
          Aucune sélection = reconnaissance du nom botanique uniquement
        </div>
      </div>
      ...
      <div id="smart-quiz-bar" style="margin-top:10px">
        <button id="smart-quiz-btn" onclick="toggleSmartMode()"
          style="width:100%;padding:13px;background:rgba(123,184,245,.08);border:1px solid rgba(123,184,245,.25);
          border-radius:14px;color:#7ab8f5;font-family:'DM Sans',sans-serif;font-size:14px;font-weight:700;
          cursor:pointer;transition:all .2s;display:flex;align-items:center;justify-content:center;gap:8px">
          <span style="font-size:18px">🧠</span>
          <span id="smart-quiz-label">Quiz Intelligent — activé</span>
          <span id="smart-quiz-badge" style="font-size:10px;background:rgba(123,184,245,.15);
          padding:2px 8px;border-radius:10px;margin-left:4px"></span>
        </button>
        <div id="smart-mastery-bar" style="display:flex;gap:6px;margin-top:6px;font-size:9px;
          text-transform:uppercase;letter-spacing:1px;color:rgba(200,223,204,.35)"></div>
      </div>
      <button class="quiz-start-btn" style="width:100%;padding:15px;background:linear-gradient(135deg,#1e5030,#2d7a4a);border:none;border-radius:16px;color:white;font-family:'DM Sans',sans-serif;font-size:17px;font-weight:700;cursor:pointer;transition:all .2s;margin-top:10px" onclick="startQuiz(false)">🚀 Démarrer — 30s par question</button>
  ...
  // Initialiser les chips pour le niveau par défaut (N1)
  renderExtraChips(qLevel||1);
  buildFilterCats();
  ...
  document.getElementById('player-name-input').addEventListener('input',e=>{
    ...
    if(myErrors.length>=2){
      ...
      hint.textContent=myErrors.length+' vegetaux a retravailler — dont : '+top+'...';
...
// Construit dynamiquement les boutons de filtres révision ciblée (évite les backticks imbriqués)
function buildFilterCats(){
  ...
  const cats=[
    {key:'feuillage', label:'🍃 Feuillage',
     getVals:()=>[...new Set(plants.map(p=>p.feuillage).filter(Boolean))].sort()},
    {key:'exposition', label:'☀️ Exposition',
     getVals:()=>[...new Set(plants.map(p=>p.exposition).filter(Boolean))].sort()},
    {key:'type', label:'🌿 Type',
     getVals:()=>[...new Set(plants.flatMap(p=>(p.type||'').split(',').map(s=>s.trim())).filter(Boolean))].sort()},
    {key:'couleur', label:'🎨 Couleur fleurs',
     getVals:()=>[...new Set(plants.flatMap(p=>(p.couleurFleurs||'').split(/,\s*/).map(s=>s.replace(/\s*\(.*\)|\s*&.*/,'').trim())).filter(s=>s&&s!=='Non significatives'&&s.length>1))].sort()},
    {key:'rusticite', label:'🌡️ Rusticité',
     getVals:()=>['Très rustique (< -25°C)→-100→-25','Rustique (-25 à -15°C)→-25→-15','Assez rustique (-15 à -10°C)→-15→-10','Peu rustique (-10 à -5°C)→-10→-5','Sensible au gel (> -5°C)→-5→0']},
    {key:'taille', label:'📏 Dimensions',
     getVals:()=>['0–0.5 m→0→0.5','0.5–1 m→0.5→1','1–2 m→1→2','2–5 m→2→5','5–15 m→5→15','> 15 m→15→100']},
    {key:'famille', label:'🔬 Famille',
     getVals:()=>[...new Set(plants.map(p=>p.famille).filter(Boolean))].sort()},
    {key:'classe', label:'🧬 Classe',
     getVals:()=>{const d=[...new Set(plants.map(p=>p.classe).filter(Boolean))].sort();return d.length>=2?d:['Bryophyte','Dicotylédone','Gymnosperme','Monocotylédone'];}},
    {key:'usage', label:'🏡 Usage',
     getVals:()=>[...new Set(plants.flatMap(p=>(p.usageAmenagement||'').split(',').map(s=>s.trim())).filter(Boolean))].sort()},
  ];

  wrap.innerHTML='';
  cats.forEach(cat=>{
    const active=qFilter[cat.key]||'';
    const vals=cat.getVals();

    // Groupe conteneur
    const grp=document.createElement('div');
    grp.className='fcat-group';

    // Bouton principal
    const btn=document.createElement('button');
    btn.id='fcat-'+cat.key;
    btn.className='fcat-btn'+(active?' fcat-active':'');
    btn.textContent=cat.label;
    if(active){
      const isRange=cat.key==='taille'||cat.key==='rusticite';
      const display=isRange?active.split('→')[0]:active.split(',')[0].trim();
      const badge=document.createElement('span');
      badge.className='fcat-badge';
      badge.style.cssText='background:var(--g4);color:#0b1e12;border-radius:10px;padding:1px 7px;font-size:10px;margin-left:4px';
      badge.textContent=display;
      btn.appendChild(badge);
    }
    btn.addEventListener('click',()=>toggleFcatDropdown(cat.key));
    grp.appendChild(btn);

    // Dropdown
    const drop=document.createElement('div');
    drop.id='fcatdrop-'+cat.key;
    drop.className='fcat-dropdown';
    drop.style.display='none';

    // Chip "Tous"
    const allChip=document.createElement('span');
    allChip.className='fchip all-chip'+(active?'':' active');
    allChip.textContent='Tous';
    allChip.addEventListener('click',(e)=>setFilter(cat.key,'',e));
    drop.appendChild(allChip);

    vals.forEach(v=>{
      const isRange=cat.key==='taille'||cat.key==='rusticite';
      const stored=isRange?v.split('→').slice(1).join('→'):v;
      const display=isRange?v.split('→')[0]:v;
      const chip=document.createElement('span');
      chip.className='fchip'+(active===stored?' active':'');
      chip.textContent=display;
      chip.addEventListener('click',(e)=>setFilter(cat.key,stored,e));
  ...
  if(Object.values(qFilter).some(v=>v)){
    const resetBtn=document.createElement('button');
    resetBtn.textContent='✕ Réinitialiser';
    resetBtn.style.cssText='padding:6px 13px;background:rgba(255,100,100,.08);border:1px solid rgba(255,100,100,.2);border-radius:20px;color:#ff9999;font-family:DM Sans,sans-serif;font-size:11px;font-weight:600;cursor:pointer;transition:all .2s';
    resetBtn.setAttribute('onmouseover',"this.style.background='rgba(255,100,100,.18)'");
    resetBtn.setAttribute('onmouseout',"this.style.background='rgba(255,100,100,.08)'");
    resetBtn.addEventListener('click',clearAllFilters);
    wrap.appendChild(resetBtn);
  }
}

// Nouveau système de filtres : dropdown inline intégré dans le bouton
function toggleFcatDropdown(field){
  const drop=document.getElementById('fcatdrop-'+field);
  const btn=document.getElementById('fcat-'+field);
  if(!drop||!btn) return;
  const isOpen=drop.style.display!=='none';
  ['feuillage','exposition','type','famille','classe','usage','couleur','taille','rusticite'].forEach(k=>{
    const d=document.getElementById('fcatdrop-'+k);
    const b=document.getElementById('fcat-'+k);
    if(d&&k!==field){ d.style.display='none'; }
    if(b&&k!==field) b.classList.remove('fcat-open');
  });
  if(isOpen){
    drop.style.display='none';
    btn.classList.remove('fcat-open');
  } else {
    drop.style.display='flex';
    btn.classList.add('fcat-open');
  }
}

function setFilter(field, val, event){
...
function selectLevel(n){
  qLevel=n;
  qEnabledChips=new Set(); // réinitialiser les chips au changement de niveau
  [1,2,3,4].forEach(i=>{
    const b=document.getElementById('lc-'+i);
    if(b) b.classList.toggle('selected',i===n);
  });
  // N0: highlight via style
  const lv0=document.getElementById('lv0-card');
  if(lv0){
    lv0.style.background=n===0?'rgba(100,200,255,.15)':'rgba(100,200,255,.05)';
    lv0.style.borderColor=n===0?'rgba(100,200,255,.6)':'rgba(100,200,255,.3)';
  }
  // Fermer tous les dropdowns filtres ouverts
  const allCats=['feuillage','exposition','type','famille','classe','usage','couleur','taille','rusticite'];
  allCats.forEach(k=>{
    const d=document.getElementById('fcatdrop-'+k);
    const b=document.getElementById('fcat-'+k);
    if(d) d.style.display='none';
    if(b) b.classList.remove('fcat-open');
  });

  // N0 : cacher la section filtres révision ciblée
  const filterSection=document.querySelector('.filter-section');
  if(filterSection) filterSection.style.display=n===0?'none':'';
  if(n===0){ clearAllFilters(); }

  // Dictée : visible pour tous les niveaux sauf N4
  const dictToggle=document.querySelector('.dictee-toggle-wrap');
  if(dictToggle) dictToggle.style.display=n===4?'none':'';
  // Mettre à jour le label dictée selon le niveau
  const dictLabel=document.querySelector('.dictee-toggle-label');
  if(dictLabel){
    dictLabel.textContent = n===0
      ? '✏️ Mode dictée — 1 question sur 4 : écrire le nom commun'
      : '✏️ Mode dictée — 1 question sur 4 en dictée botanique';
  }

  // N1+: afficher/cacher les filtres révision ciblée selon le niveau
  if(n>0){
    const catsN1=['feuillage','exposition','type','couleur','taille','rusticite'];
    const catsN2=[...catsN1,'usage'];
    const catsN3=[...catsN2,'famille','classe'];
    const visible = n===1?catsN1 : n===2?catsN2 : catsN3;
    allCats.forEach(k=>{
      const b=document.getElementById('fcat-'+k);
      if(b){
        ...
        if(!show && qFilter[k]){
          qFilter[k]='';
          b.querySelectorAll('.fcat-badge').forEach(el=>el.remove());
        }
      }
    });
    if(panel && panel.dataset.openField && !['feuillage','exposition','type','couleur','taille','rusticite','usage','famille','classe'].includes(panel.dataset.openField)){
      panel.style.display='none'; panel.dataset.openField='';
    }
  }

  // Rendu des chips supplémentaires selon le niveau
  renderExtraChips(n);
}

// ── Rendu des chips "critères supplémentaires" selon le niveau ──
function renderExtraChips(n){
  const wrap=document.getElementById('extra-chips-wrap');
  if(!wrap) return;
  const chips = CHIPS_BY_LEVEL[n];
  if(!chips || n===4){
  ...
  const hint = n===0
    ? 'Aucune sélection = quiz nom commun uniquement'
    : 'Aucune sélection = reconnaissance du nom botanique uniquement';
  wrap.parentElement.querySelector('.extra-chips-hint').textContent=hint;

  wrap.innerHTML = chips.map(c=>`
    <button id="echip-${c.key}" data-keys="${c.keys.join(',')}"
      onclick="toggleExtraChip(this)"
      style="padding:6px 13px;background:rgba(74,184,112,.05);border:1px solid rgba(74,184,112,.2);
      border-radius:20px;color:rgba(200,223,204,.45);font-family:'DM Sans',sans-serif;font-size:12px;
      font-weight:600;cursor:pointer;transition:all .2s;white-space:nowrap"
      onmouseover="this.style.background='rgba(74,184,112,.15)'"
      onmouseout="if(!this.classList.contains('echip-on'))this.style.background='rgba(74,184,112,.05)'"
    >${c.label}</button>`).join('')
  + `<button id="echip-selection" onclick="openPlantSelection()"
      style="padding:6px 13px;background:rgba(123,184,245,.05);border:1px solid rgba(123,184,245,.2);
      border-radius:20px;color:rgba(123,184,245,.5);font-family:'DM Sans',sans-serif;font-size:12px;
      font-weight:600;cursor:pointer;transition:all .2s;white-space:nowrap"
      onmouseover="this.style.background='rgba(123,184,245,.15)'"
      onmouseout="this.style.background='rgba(123,184,245,.05)'"
    >🌿 Sélection de plantes<span id="echip-sel-count"></span></button>`;
}

// ── Activer / désactiver une chip supplémentaire ──
function toggleExtraChip(btn){
  const keys = btn.dataset.keys.split(',');
  const isOn = btn.classList.contains('echip-on');
  if(isOn){
    btn.classList.remove('echip-on');
    btn.style.background='rgba(74,184,112,.05)';
    btn.style.borderColor='rgba(74,184,112,.2)';
    btn.style.color='rgba(200,223,204,.45)';
    keys.forEach(k=>qEnabledChips.delete(k));
  } else {
    btn.classList.add('echip-on');
    btn.style.background='rgba(74,184,112,.18)';
    btn.style.borderColor='rgba(74,184,112,.5)';
    btn.style.color='var(--g4)';
    keys.forEach(k=>qEnabledChips.add(k));
  }
}

// ── Sélection de plantes : modal ──
function openPlantSelection(){
  // Récupérer le nom depuis les inputs (qName pas encore défini au stade de la sélection)
  const nameEl = document.getElementById('player-name-input');
  const initEl = document.getElementById('player-initial-input');
  const nameRaw = nameEl ? nameEl.value.trim() : '';
  const initRaw = initEl ? initEl.value.trim().toUpperCase().replace(/[^A-Z]/g,'') : '';
  const playerName = nameRaw
    ? (nameRaw.charAt(0).toUpperCase()+nameRaw.slice(1).toLowerCase())+(initRaw?' '+initRaw+'.':'')
    : (qName||'');
  const myVeg = (()=>{ try{ const raw=localStorage.getItem('cq_myveg_'+playerName); return raw?JSON.parse(raw):[]; }catch(e){return [];} })();
  const allP = [...plants, ...myVeg.map(p=>({...p,_isPerso:true}))];
  const selectedIds = new Set(qSelectedPlants.map(p=>String(p.id)+'_'+(p._isPerso?'perso':'db')));

  const modal=document.createElement('div');
  modal.id='plant-sel-modal';
  ...
  modal.innerHTML=`
    <div style="background:#0f2d1a;border:1px solid rgba(74,184,112,.3);border-radius:20px;width:100%;max-width:520px;max-height:80vh;display:flex;flex-direction:column;overflow:hidden">
      <div style="padding:16px 20px;border-bottom:1px solid rgba(74,184,112,.15);display:flex;align-items:center;justify-content:space-between">
        <div>
          <div style="font-family:var(--disp);font-size:18px;font-weight:800;color:var(--cr)">🌿 Sélection de plantes</div>
          <div style="font-size:11px;color:rgba(200,223,204,.4);margin-top:2px">Aucune sélection = toutes les plantes</div>
        </div>
        <button onclick="closePlantSelection()" style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:10px;color:rgba(200,223,204,.6);padding:6px 12px;cursor:pointer;font-size:13px">✕</button>
      </div>
      <div style="padding:10px 16px;border-bottom:1px solid rgba(74,184,112,.1)">
        <input id="plant-sel-search" placeholder="🔍 Rechercher…" oninput="filterPlantSelModal(this.value)"
          style="width:100%;padding:8px 14px;background:rgba(19,48,29,.7);border:1px solid rgba(74,184,112,.2);border-radius:10px;color:var(--cr);font-family:'DM Sans',sans-serif;font-size:13px;outline:none;box-sizing:border-box"/>
      </div>
      <div style="overflow-y:auto;flex:1;padding:10px 16px" id="plant-sel-list">
        ${allP.map(p=>{
          const uid=String(p.id)+'_'+(p._isPerso?'perso':'db');
          const checked=selectedIds.has(uid)?'checked':'';
          return `<label style="display:flex;align-items:center;gap:10px;padding:8px 4px;cursor:pointer;border-bottom:1px solid rgba(74,184,112,.06)"
            data-latin="${(p.latin||'').toLowerCase()}" data-nom="${(p.nom||'').toLowerCase()}">
            <input type="checkbox" data-uid="${uid}" data-id="${p.id}" data-perso="${p._isPerso?'1':'0'}"
              ${checked} style="accent-color:var(--g4);width:16px;height:16px;flex-shrink:0"/>
            <div>
              <div style="font-style:italic;font-size:13px;color:var(--cr)">${p.latin||''}${p._isPerso?' <span style="font-size:10px;background:rgba(123,184,245,.15);color:#7ab8f5;padding:1px 6px;border-radius:8px;font-style:normal">Mes végétaux</span>':''}</div>
              <div style="font-size:11px;color:rgba(200,223,204,.5)">${p.nom||''}</div>
            </div>
          </label>`;
        }).join('')}
      </div>
      <div style="padding:12px 16px;border-top:1px solid rgba(74,184,112,.15);display:flex;gap:8px">
        <button onclick="clearPlantSelection()" style="flex:1;padding:10px;background:rgba(255,100,100,.08);border:1px solid rgba(255,100,100,.2);border-radius:12px;color:#ff9999;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:600;cursor:pointer">✕ Tout désélectionner</button>
        <button onclick="confirmPlantSelection()" style="flex:2;padding:10px;background:linear-gradient(135deg,#1e5030,#2d7a4a);border:none;border-radius:12px;color:white;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:700;cursor:pointer">✓ Confirmer</button>
      </div>
    </div>`;
  document.body.appendChild(modal);
}
function filterPlantSelModal(q){
  const s=q.toLowerCase().trim();
  document.querySelectorAll('#plant-sel-list label').forEach(el=>{
    const match=!s||el.dataset.latin.includes(s)||el.dataset.nom.includes(s);
    el.style.display=match?'':'none';
  });
}
function clearPlantSelection(){
  document.querySelectorAll('#plant-sel-list input[type=checkbox]').forEach(cb=>cb.checked=false);
}
function confirmPlantSelection(){
  ...
  const myVeg = (()=>{ try{ const raw=localStorage.getItem('cq_myveg_'+playerName); return raw?JSON.parse(raw):[]; }catch(e){return [];} })();
  const allP = [...plants, ...myVeg.map(p=>({...p,_isPerso:true}))];
  const checked=[...document.querySelectorAll('#plant-sel-list input[type=checkbox]:checked')];
  if(!checked.length){
    qSelectedPlants=[];
  } else {
    qSelectedPlants=checked.map(cb=>{
      const id=parseInt(cb.dataset.id);
      const perso=cb.dataset.perso==='1';
      return allP.find(p=>p.id===id&&(!!p._isPerso)===perso)||null;
    }).filter(Boolean);
  }
  closePlantSelection();
  // Mettre à jour le badge sur le chip
  const cnt=document.getElementById('echip-sel-count');
  const selBtn=document.getElementById('echip-selection');
  if(cnt) cnt.textContent=qSelectedPlants.length?` (${qSelectedPlants.length})`:'';
  if(selBtn){
    const hasSelection=qSelectedPlants.length>0;
    selBtn.style.background=hasSelection?'rgba(123,184,245,.18)':'rgba(123,184,245,.05)';
    selBtn.style.borderColor=hasSelection?'rgba(123,184,245,.5)':'rgba(123,184,245,.2)';
    selBtn.style.color=hasSelection?'#7ab8f5':'rgba(123,184,245,.5)';
  }
}
function closePlantSelection(){
  const m=document.getElementById('plant-sel-modal');
  if(m) m.remove();
}


// ══════════════════════════════════════════════════════════════════════
//  QUIZ INTELLIGENT — moteur de maîtrise par végétal
// ══════════════════════════════════════════════════════════════════════

// Charger la maîtrise depuis localStorage (clé: player_name)
function loadMastery(playerName){
  try{
    const raw=localStorage.getItem(SK_MASTERY+'_'+playerName);
    return raw ? JSON.parse(raw) : {};
  }catch(e){ return {}; }
}

// Sauvegarder la maîtrise localement
function saveMasteryLocal(playerName, mastery){
  try{ localStorage.setItem(SK_MASTERY+'_'+playerName, JSON.stringify(mastery)); }catch(e){}
}

// Score de maîtrise 0–100 pour un végétal
function masteryScore(entry){
  if(!entry || entry.total===0) return -1; // jamais vu
  return Math.round(entry.ok / entry.total * 100);
}

// Construire le pool intelligent 60/30/10
function buildSmartPool(basePool, playerName){
  const mastery = loadMastery(playerName);
  const now = Date.now();

  // Classer chaque plante
  const withScore = basePool.map(p=>{
    const key = String(p.id);
    const entry = mastery[key] || null;
    const score = masteryScore(entry);
    const lastSeen = entry ? entry.last : 0;
    // Pénalité temporelle : si vu récemment (< 3 quiz), légère pression pour laisser place aux autres
    const recentPenalty = (now - lastSeen) < 1000*60*5 ? 10 : 0;
    return { p, score, lastSeen, recentPenalty };
  });

  // 3 catégories
  const errors   = withScore.filter(x=>x.score>=0 && x.score<50);  // vus + en erreur
  const unseen   = withScore.filter(x=>x.score===-1);               // jamais vus
  const mastered = withScore.filter(x=>x.score>=50);                // bien connus

  // Ratios cibles pour 20 questions
  const total = 20;
  const nErrors   = Math.round(total * 0.60); // 12
  const nUnseen   = Math.round(total * 0.30); // 6
  const nMastered = total - nErrors - nUnseen; // 2

  // Trier par priorité
  // Erreurs : priorité = score le plus bas, puis le plus ancien vu en dernier
  errors.sort((a,b)=>a.score-b.score || a.lastSeen-b.lastSeen);
  // Jamais vus : aléatoire pour découvrir progressivement
  shuffle(unseen);
  // Maîtrisés : priorité = le moins revu récemment
  mastered.sort((a,b)=>a.lastSeen-b.lastSeen);

  function pick(arr, n){
    return arr.slice(0, n).map(x=>x.p);
  }

  let pool = [];

  // Si pas assez dans une catégorie, compenser avec les autres
  const errSlice   = pick(errors,   Math.min(nErrors,   errors.length));
  const unSlice    = pick(unseen,   Math.min(nUnseen,   unseen.length));
  const mastSlice  = pick(mastered, Math.min(nMastered, mastered.length));

  pool = [...errSlice, ...unSlice, ...mastSlice];

  // Compléter si moins de 20
  if(pool.length < total){
    const usedIds = new Set(pool.map(p=>p.id));
    const remaining = withScore
      .filter(x=>!usedIds.has(x.p.id))
      .sort((a,b)=>a.lastSeen-b.lastSeen)
      .map(x=>x.p);
    pool = [...pool, ...remaining].slice(0, total);
  }

  // Garantir minimum 4 plantes pour avoir des distracteurs
  if(pool.length < 4) pool = basePool.slice();

  return shuffle(pool);
}

// Mettre à jour la maîtrise après un quiz
async function updateMasteryAfterQuiz(playerName, history){
  if(!playerName || !history || !history.length) return;
  const mastery = loadMastery(playerName);
  const now = Date.now();

  history.forEach(h=>{
    const p = plants.find(x=>x.latin===h.latin);
    if(!p) return;
    const key = String(p.id);
    if(!mastery[key]) mastery[key]={ok:0,total:0,last:0};
    // Compter étape 1 (photo/orthographe)
    mastery[key].total++;
    if(h.step1Ok===true) mastery[key].ok++;
    // Compter étape 2 si elle a eu lieu
    if(h.step2Ok!==null){
      mastery[key].total++;
      if(h.step2Ok===true) mastery[key].ok++;
    }
    mastery[key].last = now;
  });

  saveMasteryLocal(playerName, mastery);

  // Sync Supabase si connecté
  if(cloudOk){
    try{
      const updates = Object.entries(mastery).map(([plantId, m])=>{
        const p = plants.find(x=>String(x.id)===plantId);
        return {
          player_name: playerName,
          plant_id: parseInt(plantId),
          plant_latin: p?p.latin:'',
          ok: m.ok,
          total: m.total,
          last_seen: m.last
        };
      });
      // Upsert en un seul appel (pas de batch) — réduit l'egress
      if(updates.length) await sbPost('floralab_mastery', updates, 'resolution=merge-duplicates');
    }catch(e){ console.warn('Mastery sync failed:', e); }
  }
}

// Statistiques de maîtrise pour un apprenti (pour affichage)
function getMasteryStats(playerName, pool){
  const mastery = loadMastery(playerName);
  let unseen=0, learning=0, mastered=0;
  pool.forEach(p=>{
    const entry=mastery[String(p.id)];
    const s=masteryScore(entry);
    if(s===-1) unseen++;
    else if(s<50) learning++;
    else mastered++;
  });
  return {unseen, learning, mastered, total:pool.length};
}


// ── Quiz Intelligent UI ──
window.qSmartMode = true; // activé par défaut

function toggleSmartMode(){
  window.qSmartMode = !window.qSmartMode;
  const btn = document.getElementById('smart-quiz-btn');
  const lbl = document.getElementById('smart-quiz-label');
  if(!btn || !lbl) return;
  if(window.qSmartMode){
    btn.style.background='rgba(123,184,245,.18)';
    btn.style.borderColor='rgba(123,184,245,.5)';
    btn.style.color='#7ab8f5';
    lbl.textContent='Quiz Intelligent — activé';
  } else {
    btn.style.background='rgba(200,223,204,.04)';
    btn.style.borderColor='rgba(200,223,204,.1)';
    btn.style.color='rgba(200,223,204,.3)';
    lbl.textContent='Quiz Intelligent — désactivé';
  }
  updateMasteryDisplay();
}
...
function updateMasteryDisplay(){
  ...
  const init=(initEl.value.trim().toUpperCase().replace(/[^A-Z]/g,''));
  if(!name||init.length<2||!window.qSmartMode){
    const bar=document.getElementById('smart-mastery-bar');
    const badge=document.getElementById('smart-quiz-badge');
    if(bar) bar.innerHTML='';
    if(badge) badge.textContent='';
    return;
  }
  const playerName=(name.charAt(0).toUpperCase()+name.slice(1).toLowerCase())+' '+init+'.';
  const stats = getMasteryStats(playerName, plants);
  const bar=document.getElementById('smart-mastery-bar');
  const badge=document.getElementById('smart-quiz-badge');
  if(badge) badge.textContent=stats.mastered+'/'+stats.total+' maîtrisés';
  if(bar){
    const pctMastered=Math.round(stats.mastered/stats.total*100)||0;
    const pctLearning=Math.round(stats.learning/stats.total*100)||0;
    const pctUnseen=Math.round(stats.unseen/stats.total*100)||0;
    bar.innerHTML=`
      <span style="color:rgba(200,223,204,.5)">🔴 ${stats.learning} à retravailler</span>
      <span style="color:rgba(200,223,204,.35)">·</span>
      <span style="color:rgba(200,223,204,.5)">⚪ ${stats.unseen} à découvrir</span>
      <span style="color:rgba(200,223,204,.35)">·</span>
      <span style="color:rgba(74,184,112,.6)">🟢 ${stats.mastered} maîtrisés</span>
    `;
  }
}
...
function startQuiz(priorityMode=false){
  ...
  if(qFilter.couleur)    pool=pool.filter(p=>{
    const fleurs=(p.couleurFleurs||'').toLowerCase();
    const q=qFilter.couleur.toLowerCase();
    // Match si la couleur apparaît seule ou en début de valeur (ex: 'blanc' dans 'Blanc, Rose')
    return fleurs.split(/,\s*/).some(s=>s.trim().startsWith(q)||s.trim()===q||s.includes(q));
  });
  if(qFilter.taille)     pool=pool.filter(p=>{
    const h=parseDimensionValue(p.hauteurAdulte)||0;
    const parts=qFilter.taille.split('\u2192').map(Number); // stored as 'lo→hi'
    const lo=parts[0], hi=parts[1];
    return !isNaN(lo)&&!isNaN(hi)&&h>=lo&&h<=hi;
  });
  ...
  // Sélection de plantes par l'apprenti (chip Sélection de plantes)
  if(qSelectedPlants.length>=2){
    // Fusionner plantes sélectionnées (BDD + fiches perso) avec le pool filtré
    const dbSel = qSelectedPlants.filter(p=>!p._isPerso);
    const persoSel = qSelectedPlants.filter(p=>p._isPerso);
    const dbPool = pool.filter(p=>dbSel.some(s=>s.id===p.id));
    const merged = [...(dbPool.length>=1?dbPool:dbSel), ...persoSel];
    if(merged.length>=2) pool=merged;
  }

  if(pool.length<2) pool=[...plants];

  // Exclure les plantes sans aucune photo du pool quiz
  const poolWithPhoto = pool.filter(p=>p.photo||p.photo2||p.photo3||p.photo4);
  if(poolWithPhoto.length < 4){
    alert('⚠️ Pas assez de plantes avec photos pour lancer le quiz.\n\nIl faut au moins 4 plantes avec au moins 1 photo.\n\nActuellement : '+poolWithPhoto.length+' plante(s) avec photo.');
    return;
  }
  qPlants = poolWithPhoto;

  qDicteeMode = document.getElementById('dictee-toggle')?.checked || false;

  // Mode Quiz Intelligent : recomposer le pool selon la maîtrise
  if(window.qSmartMode && qNameNorm){
    qPlants = buildSmartPool(qPlants, qNameNorm);
  }
...
function buildPhotoRound(){
  ...
  // qPlants est déjà filtré (au moins 1 photo) depuis startQuiz
  ...
  _photoReady = false; _timerPendingCb = null; _timerPendingDur = null; _photoRetryCount = 0;

  // Déterminer le type de question étape 1
  if(qLevel===0){
    // N0 : identification nom commun uniquement (+ dictée nom commun si mode dictée)
    if(qDicteeMode && Math.random() < 0.25){
      qPhotoType = 'dictee'; // dictée sur nom commun
    } else {
      qPhotoType = 'identify';
    }
  } else {
    // N1+ : dictée botanique (1/4) → spelling botanique (50%) → identify
    if(qDicteeMode && Math.random() < 0.25){
      qPhotoType = 'dictee';
    } else {
...
// ══════════════════════════════════════════════════════
//  NIVEAU 0 — DÉCOUVERTE : photo → nom commun
// ══════════════════════════════════════════════════════
function buildDiscoveryQuestion(p, pool){
  if(!p.nom) return null;
  const others=shuffle(pool.filter(x=>x.id!==p.id&&x.nom&&x.nom!==p.nom)).slice(0,3);
  ...
  return {
    ...
    plantLatin:p.latin,
    plantNom:p.nom,
    plantPhoto:p.photo||'', plantPhoto2:p.photo2||'', plantPhoto3:p.photo3||'', plantPhoto4:p.photo4||'',
    plantPhoto3:p.photo3||'', plantPhoto4:p.photo4||'',
    question:`<span style="font-size:13px;letter-spacing:1px;text-transform:uppercase;color:rgba(200,223,204,.4)">👀 Quelle est cette plante ?</span>`,
    correct:p.nom,
    options:shuffle([p.nom,...others.map(x=>x.nom)]),
...
function buildKQ(){
  ...
  // ── Niveau 0 : questions sur le nom commun avec critères optionnels ──
  if(qLevel===0){
    // Seuls les champs activés via chips sont éligibles
    if(qEnabledChips.size===0) return null; // pas de chips → pas d'étape 2 en N0
    const avail0=KFIELDS_N0.filter(f=>qEnabledChips.has(f.key)&&p[f.key]&&String(p[f.key]).trim());
    if(!avail0.length) return null;
    const field=rand(avail0);
    // Question posée avec le NOM COMMUN (pas le nom botanique)
    const nomRef = p.nom || p.latin;
    let correct=p[field.key];
    const MULTIVALUE_KEYS0=['couleurFleurs','type'];
    let allValidVals0=[correct];
    if(MULTIVALUE_KEYS0.includes(field.key)){
      const vals=correct.split(',').map(s=>s.trim()).filter(Boolean);
      correct=rand(vals)||correct;
      allValidVals0=vals; // exclure TOUTES les valeurs valides des mauvaises réponses
    }
    const others=[...new Set(qPlants.filter(x=>x.id!==p.id&&x[field.key]).map(x=>{
      if(MULTIVALUE_KEYS0.includes(field.key)){
        const vals=(x[field.key]||'').split(',').map(s=>s.trim()).filter(v=>v&&!allValidVals0.includes(v));
        return vals.length?rand(vals):null;
      }
      return allValidVals0.includes(x[field.key]) ? null : x[field.key];
    }).filter(Boolean))];
    let wrongs=shuffle(others).slice(0,3);
    const fil=shuffle((FILLERS[field.key]||[]).filter(f=>!allValidVals0.includes(f)));
    for(const f of fil){ if(wrongs.length>=3)break; if(!allValidVals0.includes(f)&&!wrongs.includes(f))wrongs.push(f); }
    const NUMERIC_KEYS0=['exposition','rusticite'];
    if(NUMERIC_KEYS0.includes(field.key)&&field.key==='rusticite'){
      const iv=buildNumericIntervalOptions(correct,field.key);
      if(iv) return{field,question:field.q(nomRef),correct:iv.correct,options:iv.options};
    }
    return{field,question:field.q(nomRef),correct,options:shuffle([correct,...wrongs.slice(0,3)])};
  }

  // ── Niveaux 1-3 : champs selon KFIELDS + chips activées ──
  ...
  // Avec chips : on ne garde que les champs activés par les chips (+ champs de base du niveau)
  // Sans chips (qEnabledChips vide) : quiz de base = on utilise tous les champs du niveau
  const avail=fields.filter(f=>{
    if(!p[f.key]||!String(p[f.key]).trim()) return false;
    if(qEnabledChips.size===0) return true; // pas de chips → tous les champs du niveau
    return qEnabledChips.has(f.key);
  });
  if(!avail.length) return null;
  const field=rand(avail);
  // Pour les champs multi-valeurs, on prend une valeur au hasard parmi celles de la plante
  let correct=p[field.key];
  // Toutes les valeurs valides de la plante pour ce champ (à exclure des mauvaises réponses)
  let allValidVals=[correct];
  if(MULTIVALUE_KEYS.includes(field.key)){
    const vals=correct.split(',').map(s=>s.trim()).filter(Boolean);
    correct=rand(vals)||correct;
    allValidVals=vals; // exclure TOUTES les valeurs valides des mauvaises réponses
  }
  const others=[...new Set(qPlants.filter(x=>x.id!==p.id&&x[field.key]).map(x=>{
    if(MULTIVALUE_KEYS.includes(field.key)){
      // Exclure toute valeur qui est une valeur valide de la plante correcte
      const vals=(x[field.key]||'').split(',').map(s=>s.trim()).filter(v=>v&&!allValidVals.includes(v));
      return vals.length?rand(vals):null;
    }
    return x[field.key]===correct||allValidVals.includes(x[field.key]) ? null : x[field.key];
  }).filter(Boolean))];
  let wrongs=shuffle(others).slice(0,3);
  const fil=field.key==='famille'?shuffle(FILLERS_FAMILLE.filter(f=>f!==correct&&!allValidVals.includes(f))):shuffle((FILLERS[field.key]||[]).filter(f=>f!==correct&&!allValidVals.includes(f)));
  for(const f of fil){ if(wrongs.length>=3)break; if(!allValidVals.includes(f)&&!wrongs.includes(f))wrongs.push(f); }
  // Champs numériques → intervalles (cohérence avec buildKnowledgeQuestion)
  const NUMERIC_KEYS_SOLO=['hauteurAdulte','largeurAdulte','rusticite'];
  if(NUMERIC_KEYS_SOLO.includes(field.key)){
    const iv=buildNumericIntervalOptions(correct,field.key);
    if(iv) return{field,question:field.q(p.latin),correct:iv.correct,options:iv.options};
  }
...
async function executeStopQuiz(){
  ...
  if(qScore.total > 0){
    ...
    // ── Sauvegarde locale uniquement (egress Supabase optimisé) ──
    if(false && cloudOk){ // désactivé pour réduire l'egress
      try{
...
// ── Batterie : stopper les polls quand l'appli passe en arrière-plan ──
document.addEventListener('visibilitychange', function(){
  if(document.hidden){
    // Arrêt immédiat de tous les intervals réseau
    if(statsPollInterval){ clearInterval(statsPollInterval); }
    if(multiPollInterval){  clearInterval(multiPollInterval);  }
    if(typeof sessionPollI!=='undefined' && sessionPollI){ clearInterval(sessionPollI); }
  } else {
    // Retour au premier plan : relancer uniquement ce qui était actif
    const page = document.querySelector('.page[style*="flex"]');
    const pageId = page ? page.id : '';
    if(pageId === 'page-stats' && (role === 'admin'||role==='formateur') && cloudOk){
      if(statsPollInterval) clearInterval(statsPollInterval);
      statsPollInterval = setInterval(function(){ if(cloudOk) renderStats(); }, 10000);
    }
    // Les polls multi sont relancés automatiquement par leurs propres fonctions
    // si une session est en cours — pas besoin de les relancer manuellement ici
  }
});
...
function photoBlockHTML(p){
  ...
  if(!allPhotos.length){
    // Pas de photo : considérer comme chargée immédiatement
    _photoReady = true;
    return '<div class="qphoto-slide"><div class="qphoto-fallback">'+LEAF_SVG+'</div></div>';
  }
  const slides = allPhotos.map(function(ph, i){
    var loadCb = i===0 ? 'onMainPhotoLoaded()' : '';
    var errCb  = i===0 ? 'onMainPhotoError(this)' : "this.style.display='none'";
    return '<div class="qphoto-slide"><img src="'+ph+'" class="qphoto" id="qphoto-main-'+i+'" onload="'+loadCb+'" onerror="'+errCb+'"/></div>';
  }).join('');
  const dots = allPhotos.length > 1
    ? '<div class="qphoto-dots">'+allPhotos.map(function(_,i){ return '<span class="qphoto-dot'+(i===0?' active':'')+'" onclick="goSlide('+i+')"></span>'; }).join('')+'</div>'
    : '';
  const id = 'qpc-'+Date.now();
  const spinner = '<div id="photo-loading-spinner" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(7,22,16,.85);border-radius:14px;z-index:5;pointer-events:none">'
    + '<div style="font-size:13px;color:rgba(200,223,204,.45)">📷 Chargement…</div>'
    + '</div>';
  return '<div class="qphotos" style="position:relative"><div class="qphoto-carousel" id="'+id+'">'
    +'<div class="qphoto-track">'+slides+'</div>'
    +'</div>'+dots+spinner+'</div>';
}
...
function renderQuiz(c){
  ...
  if(qPhase==='photo'){
    if(qPhotoType==='spelling'){
      ...
      const opts=qSQ.options.map(opt=>{
        ...
        return `<button class="${cls}" ${dis} onclick="pickPhotoSpelling(this)" style="font-style:italic;font-family:Arial,Helvetica,sans-serif;font-size:16px;letter-spacing:0.5px;font-weight:500">${opt}</button>`;
      }).join('');
      ...
      // ── Étape 1 : Dictée ──
      const isN0dictee = qLevel===0;
      let dictFb='', dictNext='';
      if(qPhotoSel){
        let ok;
        if(isN0dictee){
          ok = qPhotoSel.trim().toLowerCase() === (p.nom||'').toLowerCase();
          dictFb=`<div class="k-feedback ${ok?'ok':'ko'}">${ok
            ?`✅ Exact ! <strong>${p.nom}</strong>`
            :`❌ La réponse était : <strong style="color:var(--g4)">${p.nom}</strong>`
          }</div>`;
        } else {
          ok = qPhotoSel.trim().toLowerCase() === p.latin.toLowerCase();
          dictFb=`<div class="k-feedback ${ok?'ok':'ko'}">${ok
            ?`✅ Exact ! <em>${italLatin(p.latin)}</em>`
            :`❌ La réponse était : <em style="color:var(--g4)">${italLatin(p.latin)}</em>`
          }</div>`;
        }
        if(qDicteeFormatMsg) dictFb+=`<div class="k-feedback" style="font-size:12px;color:var(--am2);margin-top:6px">${qDicteeFormatMsg}</div>`;
        // N0 : pas d'étape 2 → bouton suivant direct
        dictNext=isN0dictee
          ? `<button class="next-btn" onclick="afterPhotoDictee(${ok})">Question suivante →</button>`
          : `<button class="next-btn" onclick="afterPhotoDictee(${ok})">Étape 2 →</button>`;
      }
      const dictLabel = isN0dictee
        ? '<div class="phase-label">✏️ Dictée — Nom commun</div><div class="q-text">Quel est le nom commun de ce végétal ?</div>'
        : '<div class="phase-label">✏️ Étape 1 — Dictée botanique</div><div class="q-text">Quel est le nom latin de ce végétal ?</div>';
      const dictPlaceholder = isN0dictee ? 'Nom commun…' : 'Genre espèce…';
      const dictStyle = isN0dictee ? '' : 'font-style:italic;font-family:Arial,Helvetica,sans-serif;letter-spacing:0.5px';
      c.innerHTML=topbarHTML()+`<div class="qcard">${photoBlock}<div class="qbody">`
        +`${!qPhotoSel?timerHTML(qNum,20):''}`
        +dictLabel
        +`${!qPhotoSel
          ? `<div class="dictee-wrap">`
            +`<input id="dictee-input" class="dictee-input" type="text" placeholder="${dictPlaceholder}"
      ...
      const opts=qRound.options.map(opt=>{
        ...
        // N0 : afficher le nom commun, pas le nom botanique
        const label = isN0
          ? `<span style="font-family:'Nunito',sans-serif;font-size:16px">${`<span style='display:block;font-family:Arial,sans-serif;font-size:18px;font-weight:700;color:var(--cr);letter-spacing:0.2px'>${firstNom(opt.nom)||opt.latin}</span><span style='display:block;font-family:\'Playfair Display\',serif;font-size:11px;font-style:italic;color:rgba(200,223,204,.5);margin-top:2px'>${italLatin(opt.latin)}</span>`}</span>`
          : `<span class="opt-latin">${italLatin(opt.latin)}</span>`;
        return `<button class="${cls}" ${dis} onclick="pickPhoto(this,'${opt.latin.replace(/'/g,"\\'")}','${opt.id}')">${label}</button>`;
      }).join('');
      const phaseLabel = isN0
        ? '<div class="phase-label">👀 Découverte — Quel est le nom de cette plante ?</div>'
        : '<div class="phase-label">📸 Étape 1 — Identification</div><div class="q-text">Quel est ce végétal ?</div>';
      c.innerHTML=topbarHTML()+`<div class="qcard">${photoBlock}<div class="qbody">${timerHTML(qNum, 20)}${phaseLabel}<div class="q-opts">${opts}</div></div></div>`;
  ...
  if(qPhase==='knowledge'||qPhase==='k-done'){
    ...
    if(qPhase==='k-done'){
      ...
      feedback=`<div class="k-feedback ${ok?'ok':'ko'}">${ok?`✅ Bonne réponse ! <strong>${italLatin(p.latin)}</strong> — ${qKQ.field.label} : ${qKQ.correct}`:`❌ La bonne réponse était : <strong>${qKQ.correct}</strong><br><span style="font-size:12px;opacity:.7">📖 Entraîne-toi sur les fiches pour progresser !</span>`}</div>`;
...
function showCelebration(isOk,isTimeout){
  ...
  if(isOk){
    ...
    root.innerHTML=confettiHTML()+`<div class="overlay"><div class="cel-card good"><div class="cel-glow"></div><span class="cel-emoji">🎉</span><div class="cel-title">Félicitations ${qName} !<br>Bonne réponse !</div>${fmtMsgOk}${qLevel===0
  ? `<div class="cel-common" style="font-size:22px;font-weight:800;color:var(--g4);margin-bottom:4px">${firstNom(p.nom)}</div><div class="cel-latin" style="font-size:14px;opacity:.6">${italLatin(p.latin)}</div>`
  : `<div class="cel-latin">${italLatin(p.latin)}</div><div class="cel-common">${firstNom(p.nom)}</div>`}<button class="cel-btn" onclick="closeCelebration()">Continuer →</button></div></div>`;
    qDicteeFormatMsg = null;
  } else if(isTimeout){
    root.innerHTML=`<div class="overlay"><div class="cel-card timeout"><span class="cel-emoji">⏰</span><div class="cel-title">Temps écoulé !</div><p style="color:rgba(200,223,204,.45);font-size:13px;margin-bottom:16px">Pas de réponse en 30 secondes</p><div class="wrong-box"><div class="wrong-box-label">La bonne réponse était</div>${qLevel===0
  ? `<div class="wrong-box-common" style="font-size:20px;font-weight:800;color:var(--cr);margin-bottom:3px">${firstNom(p.nom)}</div><div class="wrong-box-latin">${italLatin(p.latin)}</div>`
  : `<div class="wrong-box-latin">${italLatin(p.latin)}</div><div class="wrong-box-common">${firstNom(p.nom)}</div>`}</div><button class="cel-btn" onclick="closeCelebration()">Continuer →</button></div></div>`;
  } else {
    ...
    root.innerHTML=`<div class="overlay"><div class="cel-card bad"><span class="cel-emoji">😕</span><div class="cel-title">Pas cette fois…</div><p style="color:rgba(200,223,204,.45);font-size:13px;margin-bottom:12px">Tu avais répondu : <strong style="color:#ff9999">${qPhotoSel}</strong></p>${fmtMsg}<div class="wrong-box"><div class="wrong-box-label">La bonne réponse était</div>${qLevel===0
  ? `<div class="wrong-box-common" style="font-size:20px;font-weight:800;color:var(--cr);margin-bottom:3px">${firstNom(p.nom)}</div><div class="wrong-box-latin">${italLatin(p.latin)}</div>`
  : `<div class="wrong-box-latin">${italLatin(p.latin)}</div><div class="wrong-box-common">${firstNom(p.nom)}</div>`}</div><button class="cel-btn" onclick="closeCelebration()">Continuer →</button></div></div>`;
    qDicteeFormatMsg = null;
  }
...
function closeCelebration(){
  document.getElementById('overlay-root').innerHTML='';
  // N0 : étape 2 seulement si des chips supplémentaires sont activées
  if(qLevel===0){
    if(qEnabledChips.size>0){
...
function submitDictee(){
  ...
  // N0 : dictée sur le nom commun (vérification insensible à la casse)
  if(qLevel===0){
    const expected = qRound.correct.nom||'';
    ...
    qDicteeFormatMsg=null;
    qHistory.push({latin:qRound.correct.latin, nom:qRound.correct.nom,
      step1Type:'dictée nom commun', step1Ok:ok, step1Correct:expected,
      step2Label:'—', step2Ok:null, step2Correct:'—'});
    renderQuiz(document.getElementById('quiz-container'));
  ...
  // N1-N4 : dictée sur le nom botanique (règles d'écriture botaniques)
...
function afterPhotoDictee(step1Ok){
  if(step1Ok){
    ...
    // N0 : étape 2 seulement si des chips sont activées
    if(qLevel===0){
      if(qEnabledChips.size>0){
        qKQ=buildKQ(); qKSel=null; qPhase='knowledge';
        renderQuiz(document.getElementById('quiz-container'));
...
async function renderEnd(c){
  ...
  // Pour Supabase : on n'envoie que les colonnes existantes (startTime n'est pas dans la table)
  const sCloud={name:s.name,level:s.level,ok:s.ok,total:s.total,pct:s.pct,timeouts:s.timeouts,date:s.date};
  c.innerHTML=`<div class="quiz-end"><span class="end-trophy">🏆</span><div class="end-player-name">Enregistrement du score…</div></div>`;
  await addScore(sCloud, s);

  // Sauvegarder les erreurs dans localStorage par prénom
  const errKey=SK_ERRORS;
  // Mettre à jour la maîtrise par végétal (quiz intelligent)
  updateMasteryAfterQuiz(qName, qHistory);
  let allErrors={};
...
function setLbFilter(f){
  lbFilter=f;
  ['all','lv0','lv1','lv2','lv3','lv4'].forEach(k=>{
    const b=document.getElementById('lb-tab-'+k); if(b) b.classList.toggle('active',k===f);
  });
  renderScores();
}
...
async function renderScores(){
  ...
  if(!scores.length){
    table.innerHTML='<div style="text-align:center;padding:30px;color:rgba(200,223,204,.4)">⏳ Chargement des scores…</div>';
  }
  // Recharger depuis Supabase si vide ou cache expiré
  if(!scores.length || (cloudOk && (!window._scoresLoaded || Date.now()-window._scoresLoaded > 180000))){
    try{ await loadScores(true); }
    catch(e){ console.error('[CQ] renderScores loadScores fail:',e); }
  }
  let data=[...scores];
  if(lbFilter==='lv0') data=data.filter(s=>(s.level||1)===0);
  else if(lbFilter==='lv1') data=data.filter(s=>(s.level||1)===1);
  ...
  const tabLabels={'all':'Tous les niveaux','lv0':'Niveau 0','lv1':'Niveau 1','lv2':'Niveau 2','lv3':'Niveau 3','lv4':'Niveau 4'};
  document.getElementById('lb-sub').textContent=`${data.length} score(s) · ${tabLabels[lbFilter]||''} · ${cloudOk?'☁️ En ligne':'💻 Local'}`;

  if(!data.length){
    const src = cloudOk ? '(Supabase connecté)' : '(mode local)';
    table.innerHTML=`<div class="lb-empty"><span class="lb-empty-icon">🌱</span><p>Aucun score enregistré.<br>Lancez le quiz pour commencer le challenge !<br><span style="font-size:10px;opacity:.4">${src} · ${scores.length} score(s) en mémoire</span></p></div>`;
    return;
  }
...
async function renderStats(){
  ...
  if(!c) return;
  // Charger les scores avant de rendre (sinon byName serait vide)
  if(!scores.length || !window._scoresLoaded){
    await loadScores(true);
  }

  // Recharger scores + données stats en parallèle (1 seul aller-retour réseau)
  let allErrors={};
  try{ allErrors=JSON.parse(localStorage.getItem(SK_ERRORS)||'{}'); }catch(e){}

  // ── Filtre formateur : seulement les apprentis qui ont renseigné son nom ──
  let filteredScores = scores;
  let formateurApprentisNames = null; // null = pas de filtre (admin)
  if(role==='formateur' && currentUser && cloudOk && window.SUPA_URL && window.SUPA_KEY){
    try{
      const hdr={'apikey':window.SUPA_KEY,'Authorization':'Bearer '+window.SUPA_KEY};
      // Chercher les apprentis ayant indiqué ce formateur (nom insensible casse)
      const nomF = (currentUser.nom||'').toLowerCase();
      const preF = (currentUser.prenom||'').toLowerCase();
      const res=await fetch(window.SUPA_URL+'/rest/v1/floralab_users?role=eq.apprenti&select=nom,prenom,formateur_nom',{headers:hdr});
      if(res.ok){
        const users=await res.json();
        formateurApprentisNames=new Set();
        users.forEach(u=>{
          const fn=(u.formateur_nom||'').toLowerCase();
          if(fn && (fn.includes(nomF) || fn.includes(preF))){
            // Le nom dans les scores est typiquement "Prénom" ou "Prénom AB"
            const pre=_fmtPre(u.prenom||'').toLowerCase();
            formateurApprentisNames.add(pre);
          }
        });
        filteredScores=scores.filter(s=>formateurApprentisNames.has((s.name||'').split(' ')[0].toLowerCase()));
      }
    }catch(e){ cqWarn('renderStats filter formateur',e); }
  }
  ...
  // Scores par apprenti — enrichis (filtrés pour formateur)
  const byName={};
  ...
  const allPcts=filteredScores.map(s=>s.pct||0);
  ...
  const cardsHTML=apprentis.map(a=>{
    ...
    const myErrors=(allErrors[a.name]||[]).sort((x,y)=>y.count-x.count);
    const myErrorsTop=myErrors.slice(0,5);
    const safeNameSolo=a.name.replace(/'/g,"\\'");
    ...
    return `
      <div class="stat-card">
        ...
        ${myErrors.length?`
          <div class="stat-weak" onclick="toggleStatErrors('err-${a.name.replace(/[^a-zA-Z0-9]/g,'_')}')" style="cursor:pointer">
            <div class="stat-weak-title" style="display:flex;justify-content:space-between;align-items:center">
              Points faibles <span style="font-size:10px;color:rgba(200,223,204,.4)">${myErrors.length} végétaux · Cliquer pour voir</span>
            </div>
            ${myErrorsTop.map(e=>`<div class="stat-weak-item"><em style="font-size:12px">${e.latin}</em><span class="stat-weak-count">${e.count}×</span></div>`).join('')}
          </div>
          <div id="err-${a.name.replace(/[^a-zA-Z0-9]/g,'_')}" style="display:none;margin-top:4px">
            <div style="font-size:11px;font-weight:700;color:rgba(200,223,204,.5);margin-bottom:4px;letter-spacing:1px">TOUTES LES ERREURS</div>
            ${myErrors.map(e=>`<div class="stat-weak-item"><em style="font-size:12px">${e.latin}</em><span style="color:rgba(200,223,204,.4);font-size:10px">${e.nom||''}</span><span class="stat-weak-count">${e.count}×</span></div>`).join('')}
          </div>`:''}
      </div>`;
  }).join('');


  // ── Chargement parallèle : scores + abandons + sessions (1 seul round-trip) ──
  let incompletes=[], multiPlayers=[], multiSessionsMeta=[];
  if(cloudOk){
    const [resScores, resAbandoned, resPlayers, resSessions] = await Promise.allSettled([
      // Stats admin : sélectionner uniquement les colonnes nécessaires
      sbGet('floralab_scores','select=name,pct,ok,total,level,date,timeouts&order=pct.desc,ok.desc'),
      sbGet('floralab_abandoned','select=name,pct,ok,total,date,elapsed&order=date.desc&limit=100'),
      sbGet('floralab_session_players','select=player_name,score,total,done,session_code&order=updated_at.desc&limit=200'),
      sbGet('floralab_sessions','select=code,level,created_at,status,peer&order=created_at.desc&limit=50')
    ]);
    // Scores — fusionner cloud + local
    if(resScores.status==='fulfilled'){
      const fromCloud=resScores.value;
      let fromLocal=[];
      try{ fromLocal=JSON.parse(localStorage.getItem(SK_SCORES)||'[]'); }catch(e){}
      const cloudKeys=new Set(fromCloud.map(s=>s.name+'|'+s.date+'|'+s.pct));
      const onlyLocal=fromLocal.filter(s=>!cloudKeys.has((s.name||'')+'|'+(s.date||0)+'|'+(s.pct||0)));
    ...
    if(resAbandoned.status==='fulfilled') incompletes=resAbandoned.value;
    if(resPlayers.status==='fulfilled') multiPlayers=resPlayers.value;
    if(resSessions.status==='fulfilled') multiSessionsMeta=resSessions.value.filter(function(s){return !s.peer;});
  }
  // Fallback localStorage pour les abandons
  ...
  // IMPORTANT: multiBySession doit être déclaré avant multiByName car il est référencé dans le forEach
  ...
  const multiCardsHTML=multiApprentis.map(a=>{
    ...
    const myMultiErrors=(allErrors[a.name]||[]).sort((x,y)=>y.count-x.count).slice(0,5);
    const errBlockM=myMultiErrors.length
      ?`<div class="stat-weak"><div class="stat-weak-title">Points faibles</div>${myMultiErrors.map(e=>`<div class="stat-weak-item"><em style="font-size:12px">${e.latin}</em><span class="stat-weak-count">${e.count}×</span></div>`).join('')}</div>`
      :'';
...
// ══════════════════════════════════════════════════════
//  MOTEUR DE RECHERCHE MULTICRITÈRE PARTAGÉ
//  Utilisé par renderFiches (bibliothèque) et filterAdminList (admin)
// ══════════════════════════════════════════════════════
function buildSearchEngine(q){
  ...
  // Set of all type values (lowercase) for fast lookup
  const TYPE_VALS=new Set(TYPE_OPTIONS.map(t=>t.toLowerCase()));

  function plantMatchesToken(p, tok){
    // Type exact match : si le token est un type connu, chercher UNIQUEMENT dans type
    if(TYPE_VALS.has(tok)){
      const plantType=(p.type||'').toLowerCase();
      // Filtrer les valeurs vides pour éviter que tok.includes('') = true
      return plantType.split(',').map(s=>s.trim()).filter(Boolean).some(t=>t===tok||t.includes(tok)||tok.includes(t));
    }
...
// ══════════════════════════════════════════════════════
//  MES VÉGÉTAUX — Fiches personnelles de l'apprenti
// ══════════════════════════════════════════════════════

// Clé localStorage personnalisée par apprenti
function skMyPlants(name){ return SK_MY_PLANTS + '_' + (name||'').replace(/\s/g,'_'); }

// Générer ou récupérer le code de partage (6 chars alphanumériques)
function getOrCreateShareCode(name){
  const key = SK_SHARE_CODE + '_' + (name||'').replace(/\s/g,'_');
...
// Charger les plantes perso depuis localStorage
function loadMyPlants(name){
  myPlantsOwner = name;
  try{ myPlants = JSON.parse(localStorage.getItem(skMyPlants(name))||'[]'); }
  catch(e){ myPlants=[]; }
}

// Sauvegarder les plantes perso (localStorage)
function saveMyPlants(){
  if(!myPlantsOwner) return;
  try{ localStorage.setItem(skMyPlants(myPlantsOwner), JSON.stringify(myPlants)); }catch(e){}
}

// ── Brouillons admin ──
function loadDrafts(){
  try{ draftPlants=JSON.parse(localStorage.getItem(SK_DRAFTS)||'[]'); }catch(e){ draftPlants=[]; }
}
function saveDrafts(){
  try{ localStorage.setItem(SK_DRAFTS,JSON.stringify(draftPlants)); }catch(e){}
}
function nextDraftId(){
  const used=draftPlants.map(p=>p.id||0);
  let id=90000; while(used.includes(id)) id++; return id;
}
function draftCount(){ return draftPlants.length; }

// ── Export sauvegarde ──
function saveMyPlantsToFile(idsToExport){
  if(!myPlantsOwner) return;
  const plantsToExport = idsToExport
    ? myPlants.filter(p=>idsToExport.includes(p.id))
    : myPlants;
  if(!plantsToExport.length){ showToast('Aucune fiche à sauvegarder.'); return; }
  const data = {
    type: 'chloroquiz_myplants_backup',
    owner: myPlantsOwner,
    date: new Date().toISOString(),
    count: plantsToExport.length,
    plants: plantsToExport
  };
  const blob = new Blob([JSON.stringify(data, null, 2)], {type:'application/json'});
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  const safeName = myPlantsOwner.replace(/[^a-zA-Z0-9]/g,'_').toLowerCase();
  const suffix = idsToExport ? '_selection_'+plantsToExport.length+'_fiches' : '_toutes_fiches';
  // Nom clair pour retrouver dans Téléchargements
  a.href=url;
  a.download='Mes_vegetaux_ChloroQuiz_'+safeName+suffix+'_'+new Date().toISOString().slice(0,10)+'.json';
  a.click();
  URL.revokeObjectURL(url);
  showToast('💾 '+plantsToExport.length+' fiche(s) exportée(s) — dossier : Téléchargements');
}

// ── Import/restauration ──
function restoreMyPlantsFromFile(input){
  const file = input.files[0]; if(!file) return;
  const reader = new FileReader();
  reader.onload = function(e){
    try{
      const data = JSON.parse(e.target.result);
      if(data.type !== 'chloroquiz_myplants_backup' || !Array.isArray(data.plants)){
        alert('❌ Fichier invalide — utilise uniquement un fichier de sauvegarde ChloroQuiz.');
        return;
      }
      ...
      const msg = `Restaurer ${n} fiche(s) de "${owner}" (sauvegarde du ${date}) ?

⚠️ Cela remplacera toutes tes fiches actuelles.`;
      if(!confirm(msg)) return;
      myPlants = data.plants;
      saveMyPlants();
      renderMyPlants();
      showToast('✅ '+n+' fiche(s) restaurée(s) !');
    }catch(ex){
      alert('❌ Impossible de lire le fichier : '+ex.message);
    }
  };
...
// Prochain ID local (négatif pour éviter collision avec catalogue admin)
function nextMyPlantId(){
  const used = myPlants.map(p=>p.id||0);
  let id = -1; while(used.includes(id)) id--;
  return id;
}

// Compter les partages en attente pour l'apprenti connecté
async function countPendingShares(shareCode){
  if(!cloudOk||!shareCode) return 0;
  try{
    const rows = await sbGet('floralab_plant_shares',
      `recipient_code=eq.${encodeURIComponent(shareCode)}&status=eq.pending&select=id`);
    return rows.length;
  }catch(e){ return 0; }
}

// ── Page principale Mes végétaux ──
async function renderMyPlants(){
  const c = document.getElementById('page-myplants');
  if(!c) return;

  // Demander le nom si pas encore identifié
  if(!myPlantsOwner || !qName){
    c.innerHTML=`
      <div class="pg-header"><div class="pg-title">🌱 Mes <em>végétaux</em></div></div>
      <div style="max-width:480px;margin:40px auto;padding:0 16px;text-align:center">
        <div style="font-size:14px;color:rgba(200,223,204,.6);margin-bottom:20px">
          Pour accéder à tes fiches personnelles, commence par saisir ton prénom (le même que pour le quiz).
        </div>
        <div style="display:flex;gap:10px;margin-bottom:12px">
          <input id="myveg-name" placeholder="Prénom…" maxlength="30" autocomplete="off"
            style="flex:3;padding:14px 16px;background:rgba(19,48,29,.7);border:1px solid rgba(74,184,112,.3);
            border-radius:14px;color:var(--cr);font-family:'DM Sans',sans-serif;font-size:17px;outline:none"
            oninput="var v=this.value;if(v)this.value=v.charAt(0).toUpperCase()+v.slice(1).toLowerCase()"
            onkeydown="if(event.key==='Enter')document.getElementById('myveg-init').focus()"/>
          <input id="myveg-init" placeholder="AB" maxlength="2" autocomplete="off"
            style="flex:1;padding:14px 10px;background:rgba(19,48,29,.7);border:1px solid rgba(74,184,112,.3);
            border-radius:14px;color:var(--cr);font-family:'DM Sans',sans-serif;font-size:17px;text-align:center;
            text-transform:uppercase;outline:none"
            onkeydown="if(event.key==='Enter')confirmMyVegName()"/>
        </div>
        <div style="font-size:11px;color:rgba(200,223,204,.35);margin-bottom:16px">2 premières lettres de ton nom (ex: Dupont → DU)</div>
        <button onclick="confirmMyVegName()"
          style="width:100%;padding:14px;background:linear-gradient(135deg,#1e5030,#2d7a4a);border:none;
          border-radius:14px;color:white;font-family:'DM Sans',sans-serif;font-size:16px;font-weight:700;cursor:pointer">
          Accéder à mes végétaux →
        </button>
      </div>`;
    setTimeout(()=>{ const el=document.getElementById('myveg-name'); if(el)el.focus(); },100);
    return;
  }

  loadMyPlants(myPlantsOwner);
  const shareCode = getOrCreateShareCode(myPlantsOwner);
  // Ne pas await pour ne pas bloquer le rendu si connexion perdue
  // Le badge sera mis à jour en arrière-plan
  const pending = 0; // sera mis à jour async ci-dessous

  c.innerHTML=`
    <div class="pg-header">
      <div class="pg-title">🌱 Mes <em>végétaux</em></div>
      <div class="pg-sub" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">
        <span style="color:rgba(200,223,204,.5)">Fiches de <strong style="color:var(--cr)">${myPlantsOwner}</strong> — stockées sur cet appareil</span>
        <span style="background:rgba(19,48,29,.6);border:1px solid rgba(74,184,112,.2);border-radius:10px;padding:5px 12px;font-size:12px;cursor:pointer"
          onclick="copyShareCode('${shareCode}')" title="Cliquer pour copier">
          🔗 Code : <strong style="color:var(--g4);letter-spacing:2px">${shareCode}</strong>
        </span>
      </div>
    </div>

    <div id="myplants-pending-zone"></div>

    <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:14px">
      <button onclick="openMyPlantForm(null)"
        style="padding:10px 20px;background:linear-gradient(135deg,#1e5030,#2d7a4a);border:none;border-radius:12px;
        color:white;font-family:'DM Sans',sans-serif;font-size:14px;font-weight:700;cursor:pointer">
        + Nouvelle fiche
      </button>
      <button onclick="openReceivePlantModal()"
        style="padding:10px 18px;background:rgba(74,184,112,.1);border:1px solid rgba(74,184,112,.25);border-radius:12px;
        color:var(--g4);font-family:'DM Sans',sans-serif;font-size:13px;font-weight:600;cursor:pointer">
        📥 Recevoir par code
      </button>
      <button onclick="saveMyPlantsToFile()"
        style="padding:10px 18px;background:rgba(232,160,32,.1);border:1px solid rgba(232,160,32,.25);border-radius:12px;color:var(--am2);font-family:'DM Sans',sans-serif;font-size:13px;font-weight:600;cursor:pointer">
        💾 Sauvegarder mes fiches
      </button>
      <button onclick="document.getElementById('import-myplants-file').click()"
        style="padding:10px 14px;background:rgba(74,184,112,.06);border:1px solid rgba(74,184,112,.15);border-radius:12px;color:rgba(200,223,204,.6);font-family:'DM Sans',sans-serif;font-size:12px;cursor:pointer">
        📂 Restaurer
      </button>
      <input type="file" id="import-myplants-file" accept=".json" style="display:none" onchange="restoreMyPlantsFromFile(this)"/>
      <button onclick="showPage('home');myPlantsOwner=null;"
        style="padding:10px 14px;background:rgba(255,100,100,.06);border:1px solid rgba(255,100,100,.15);border-radius:12px;
        color:#ff9999;font-family:'DM Sans',sans-serif;font-size:12px;cursor:pointer">
        ↩ Changer d'apprenti
      </button>
    </div>

    <div id="myplants-grid" class="fiches-grid">
      ${myPlants.length===0
        ? '<div class="empty" style="grid-column:1/-1"><span class="empty-icon">🌱</span><p>Aucune fiche personnelle pour l\'instant.</p></div>'
        : myPlants.map(p=>myPlantCard(p)).join('')}
    </div>`;

  // Mise à jour async du badge de fiches reçues (ne bloque pas le rendu si hors ligne)
  countPendingShares(shareCode).then(n=>{
    if(n<=0) return;
    const zone = document.getElementById('myplants-pending-zone');
    if(!zone) return;
    zone.innerHTML = `<div onclick="openPendingShares()" style="margin:0 0 14px;padding:12px 16px;background:rgba(232,160,32,.1);border:1px solid rgba(232,160,32,.3);border-radius:12px;cursor:pointer;display:flex;align-items:center;gap:10px">
      <span style="font-size:20px">📬</span>
      <div style="flex:1"><div style="font-weight:700;color:var(--am2)">${n} fiche(s) reçue(s) en attente</div>
      <div style="font-size:11px;color:rgba(232,160,32,.6)">Appuyer pour accepter ou refuser</div></div>
      <span style="color:var(--am2)">→</span>
    </div>`;
  }).catch(()=>{});
}

function confirmMyVegName(){
  const nameEl=document.getElementById('myveg-name');
  const initEl=document.getElementById('myveg-init');
  if(!nameEl||!initEl) return;
  const name=nameEl.value.trim();
  const init=initEl.value.trim().toUpperCase().replace(/[^A-Z]/g,'');
  if(!name){ nameEl.style.borderColor='#ff7070'; nameEl.focus(); return; }
  if(init.length<2){ initEl.style.borderColor='#ff7070'; initEl.focus(); return; }
  const full=(name.charAt(0).toUpperCase()+name.slice(1).toLowerCase())+' '+init+'.';
  myPlantsOwner=full;
  // Also sync with qName for quiz usage
  if(!qName) qName=full;
  renderMyPlants();
}
...
function showToast(msg){
  ...
  t.textContent=msg;
  t.style.cssText='position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:rgba(74,184,112,.9);color:#0b1e12;padding:10px 20px;border-radius:20px;font-family:"DM Sans",sans-serif;font-size:13px;font-weight:700;z-index:9999;transition:opacity .4s';
  document.body.appendChild(t);
  setTimeout(()=>{ t.style.opacity='0'; setTimeout(()=>t.remove(),400); },2000);
}

// ── Card mini pour une plante perso ──
function myPlantCard(p){
  return `<div class="fiche" onclick="openMyPlantModal(${p.id})">
    ...
    <div class="fiche-body">
      <div class="fiche-latin">${italLatin(p.latin)}</div>
      <div class="fiche-common">${firstNom(p.nom)||''}</div>
      <div class="fiche-tags">
        ${p.type?`<span class="ftag" style="background:rgba(74,184,112,.1);color:var(--g4)">${p.type}</span>`:''}
        ${p.feuillage?`<span class="ftag" style="background:rgba(100,160,255,.1);color:#a0c4ff">${p.feuillage}</span>`:''}
      </div>
      <div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap">
        <button onclick="event.stopPropagation();openMyPlantForm(${p.id})"
          style="padding:4px 10px;background:rgba(74,184,112,.1);border:1px solid rgba(74,184,112,.2);border-radius:8px;color:var(--g4);font-size:11px;cursor:pointer">✏️ Modifier</button>
        <button onclick="event.stopPropagation();sharePlantModal(${p.id})"
          style="padding:4px 10px;background:rgba(232,160,32,.1);border:1px solid rgba(232,160,32,.25);border-radius:8px;color:var(--am2);font-size:11px;cursor:pointer">📤 Partager</button>
        <button onclick="event.stopPropagation();deleteMyPlant(${p.id})"
          style="padding:4px 10px;background:rgba(255,80,80,.06);border:1px solid rgba(255,80,80,.15);border-radius:8px;color:#ff9999;font-size:11px;cursor:pointer">🗑</button>
      </div>
    </div>
  </div>`;
}
...
// ── Modal fiche simple ──
function openMyPlantModal(id){
  const p=myPlants.find(x=>x.id===id); if(!p) return;
  const root=document.getElementById('overlay-root');
  root.innerHTML=`<div class="modal-bg" onclick="if(event.target===this)closeModal()">
    <div class="modal">
      <div style="padding:20px 52px 0 20px;position:relative">
        <button class="modal-close" onclick="closeModal()">✕</button>
        ${p.famille?`<div class="modal-famille">${p.famille}</div>`:''}
        <div class="modal-latin">${italLatin(p.latin)}</div>
        <div class="modal-common">${p.nom||''}</div>
      </div>
      ${p.photo?`<img src="${p.photo}" class="modal-img" style="object-fit:contain;background:#071610" onerror="this.style.display='none'" loading="lazy"/>`:''}
      <div class="modal-grid" style="padding:0 20px 20px">
        ${[['Famille',p.famille],['Type',p.type],['Feuillage',p.feuillage],['Exposition',p.exposition],
           ['Rusticité',p.rusticite],['Hauteur',p.hauteurAdulte],['Largeur',p.largeurAdulte],
           ['Floraison',p.periodeFloraison],['Couleur fleurs',p.couleurFleurs],['pH',p.ph],
           ['Humidité',p.humidite],['Usage',p.usageAmenagement],['Description',p.description]]
          .filter(([,v])=>v).map(([l,v])=>`<div style="background:rgba(19,48,29,.6);border:1px solid rgba(74,184,112,.12);border-radius:10px;padding:8px 10px;display:flex;flex-direction:column"><div style="font-size:9px;letter-spacing:1.5px;text-transform:uppercase;color:rgba(200,223,204,.55);margin-bottom:3px">${l}</div><div style="font-size:12px;color:var(--cr);font-weight:600">${v}</div></div>`).join('')}
      </div>
      <div style="padding:0 20px 20px;display:flex;gap:8px;flex-wrap:wrap">
        <button onclick="closeModal();openMyPlantForm(${p.id})"
          style="flex:1;padding:10px;background:rgba(74,184,112,.1);border:1px solid rgba(74,184,112,.25);border-radius:10px;color:var(--g4);font-family:'DM Sans',sans-serif;font-size:13px;font-weight:600;cursor:pointer">✏️ Modifier</button>
        <button onclick="closeModal();sharePlantModal(${p.id})"
          style="flex:1;padding:10px;background:rgba(232,160,32,.1);border:1px solid rgba(232,160,32,.25);border-radius:10px;color:var(--am2);font-family:'DM Sans',sans-serif;font-size:13px;font-weight:600;cursor:pointer">📤 Partager</button>
      </div>
    </div>
  </div>`;
}

// ── Formulaire ajout/modif plante perso (même formulaire qu'admin) ──
function openMyPlantForm(id){
  const existing = id!==null ? myPlants.find(x=>x.id===id) : null;
  editingId = id!==null ? id : null;
  _myPlantMode = true;

  if(existing){
    // Injecter temporairement la plante dans plants[] pour que openForm() la trouve
    const wasInPlants = plants.some(p=>p.id===existing.id);
    if(!wasInPlants) plants.push(existing);
    openForm(existing.id);
    // Retirer APRÈS le setTimeout de previewUrl (80ms) + marge
    if(!wasInPlants) setTimeout(function(){
...
var _myPlantMode = false; // flag pour savePlantForm
var _myPlantSelecting = false; // mode sélection multiple
var _myPlantSelected = new Set(); // IDs sélectionnés
window._myPlantSelected = _myPlantSelected; // exposé globalement pour les handlers inline

function toggleMyPlantSelectMode(){
  _myPlantSelecting = !_myPlantSelecting;
  _myPlantSelected.clear();
  renderMyPlants();
}

function toggleMyPlantSelect(id, e){
  e.stopPropagation();
  if(_myPlantSelected.has(id)) _myPlantSelected.delete(id);
  else _myPlantSelected.add(id);
  // Mettre à jour la carte visuellement sans re-render complet
  const card = document.getElementById('myp-card-'+id);
  if(card){
    card.style.borderColor = _myPlantSelected.has(id) ? 'var(--g4)' : '';
    card.style.background  = _myPlantSelected.has(id) ? 'rgba(74,184,112,.15)' : '';
    const cb = document.getElementById('myp-cb-'+id);
    if(cb) cb.textContent = _myPlantSelected.has(id) ? '✅' : '☐';
  }
...
function _updateSelectBar(){
  const bar = document.getElementById('myp-select-bar');
  if(!bar) return;
  const n = _myPlantSelected.size;
  bar.innerHTML = n > 0
    ? '<span style="color:var(--g4);font-weight:700">'+n+' fiche(s) sélectionnée(s)</span>'
      +'<button onclick="saveMyPlantsToFile([...window._myPlantSelected])" style="padding:7px 14px;background:rgba(232,160,32,.15);border:1px solid rgba(232,160,32,.35);border-radius:10px;color:var(--am2);font-size:13px;font-weight:600;cursor:pointer">💾 Exporter</button>'
      +'<button onclick="exportSelectionPDF()" style="padding:7px 14px;background:rgba(74,184,112,.12);border:1px solid rgba(74,184,112,.3);border-radius:10px;color:var(--g4);font-size:13px;font-weight:600;cursor:pointer">📄 PDF</button>'
      +'<button onclick="_myPlantSelected.clear();renderMyPlants()" style="padding:7px 12px;background:rgba(255,80,80,.08);border:1px solid rgba(255,80,80,.2);border-radius:10px;color:#ff9999;font-size:12px;cursor:pointer">✕ Tout désélect.</button>'
    : '<span style="color:rgba(200,223,204,.45);font-size:12px">Cliquer sur les fiches pour les sélectionner</span>';
}

async function exportSelectionPDF(){
  const ids = [..._myPlantSelected];
  if(!ids.length){ showToast('Aucune fiche sélectionnée.'); return; }
  for(const id of ids){
    await exportFichePDF(id);
    await new Promise(r=>setTimeout(r,300));
  }
  showToast('📄 '+ids.length+' PDF exporté(s)');
}

// ── Supprimer ──
function deleteMyPlant(id){
  if(!confirm('Supprimer cette fiche de tes végétaux personnels ?')) return;
...
function sharePlantModal(id){
  ...
  root.innerHTML=`<div class="modal-bg" onclick="if(event.target===this)closeModal()">
    <div class="modal" style="max-width:420px">
      <div style="padding:20px 52px 0 20px;position:relative">
        <button class="modal-close" onclick="closeModal()">✕</button>
        <div style="font-family:var(--disp);font-size:18px;font-weight:900;color:var(--cr)">📤 Partager — <em style="font-style:italic;color:var(--am2)">${italLatin(p.latin)}</em></div>
        <div style="font-size:12px;color:rgba(200,223,204,.45);margin-top:4px">Entrer le code de partage du destinataire</div>
      </div>
      <div style="padding:20px">
        <input id="share-dest-code" placeholder="Code du destinataire (ex: AB3X7K)" maxlength="6"
          style="width:100%;box-sizing:border-box;padding:14px 16px;background:rgba(19,48,29,.7);border:1px solid rgba(74,184,112,.3);
          border-radius:12px;color:var(--cr);font-family:'DM Sans',sans-serif;font-size:20px;letter-spacing:4px;
          text-transform:uppercase;outline:none;text-align:center;margin-bottom:14px"
          oninput="this.value=this.value.toUpperCase().replace(/[^A-Z0-9]/g,'')"/>
        <div style="font-size:11px;color:rgba(200,223,204,.3);text-align:center;margin-bottom:16px">
          Le destinataire devra accepter la fiche avant qu'elle soit ajoutée à ses végétaux.
        </div>
        <div style="display:flex;gap:8px">
          <button onclick="closeModal()"
            style="flex:1;padding:12px;background:rgba(200,223,204,.06);border:1px solid rgba(200,223,204,.1);border-radius:10px;color:rgba(200,223,204,.6);font-family:'DM Sans',sans-serif;font-size:13px;cursor:pointer">Annuler</button>
          <button onclick="sendPlantShare(${id})"
            style="flex:2;padding:12px;background:linear-gradient(135deg,#1e5030,#2d7a4a);border:none;border-radius:10px;color:white;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:700;cursor:pointer">📤 Envoyer</button>
        </div>
        <div style="margin-top:12px;padding:10px 14px;background:rgba(19,48,29,.4);border-radius:10px">
          <div style="font-size:11px;color:rgba(200,223,204,.35);margin-bottom:6px">Ou exporter pour envoyer manuellement (WhatsApp, email…)</div>
          <button onclick="exportMyPlantJSON(${id})"
            style="width:100%;padding:9px;background:rgba(232,160,32,.08);border:1px solid rgba(232,160,32,.2);border-radius:8px;color:var(--am2);font-family:'DM Sans',sans-serif;font-size:12px;cursor:pointer">📁 Exporter en fichier</button>
        </div>
      </div>
    </div>
  </div>`;
  setTimeout(()=>{ const el=document.getElementById('share-dest-code'); if(el)el.focus(); },100);
}

async function sendPlantShare(id){
  const p=myPlants.find(x=>x.id===id); if(!p) return;
  const destCode=(document.getElementById('share-dest-code')||{}).value||'';
  if(destCode.length!==6){ alert('⚠️ Code invalide — 6 caractères alphanumériques.'); return; }
  if(!cloudOk){ alert('⚠️ Connexion cloud requise pour le partage direct.'); return; }
  const myCode=getOrCreateShareCode(myPlantsOwner);
  try{
    await sbPost('floralab_plant_shares',{
      sender_name: myPlantsOwner,
      sender_share_code: myCode,
      recipient_code: destCode.toUpperCase(),
      plant: p,
      status: 'pending'
    },'return=minimal');
    closeModal();
    showToast('✅ Fiche envoyée ! En attente d\'acceptation.');
  }catch(e){ alert('❌ Erreur d\'envoi : '+e.message); }
}

function exportMyPlantJSON(id){
  const p=myPlants.find(x=>x.id===id); if(!p) return;
  const blob=new Blob([JSON.stringify({type:'chloroquiz_plant',sender:myPlantsOwner,plant:p,date:new Date().toISOString()})],{type:'application/json'});
  const url=URL.createObjectURL(blob);
  const a=document.createElement('a');
  a.href=url; a.download=(p.latin||'plante').replace(/\s/g,'_').toLowerCase()+'.cqp.json'; a.click();
  URL.revokeObjectURL(url);
  showToast('Fichier exporté — envoie-le à ton destinataire');
}

// ── Recevoir par import de fichier JSON ──
function openReceivePlantModal(){
  const root=document.getElementById('overlay-root');
  root.innerHTML=`<div class="modal-bg" onclick="if(event.target===this)closeModal()">
    <div class="modal" style="max-width:400px">
      <div style="padding:20px 52px 0 20px;position:relative">
        <button class="modal-close" onclick="closeModal()">✕</button>
        <div style="font-family:var(--disp);font-size:18px;font-weight:900;color:var(--cr)">📥 Recevoir une fiche</div>
      </div>
      <div style="padding:20px">
        <div style="font-size:13px;color:rgba(200,223,204,.6);margin-bottom:16px">
          Si tu as reçu un fichier <strong>.cqp.json</strong> par WhatsApp ou email, charge-le ici.
        </div>
        <button onclick="document.getElementById('import-plant-file').click()"
          style="width:100%;padding:14px;background:rgba(74,184,112,.1);border:1px solid rgba(74,184,112,.25);border-radius:12px;color:var(--g4);font-family:'DM Sans',sans-serif;font-size:14px;font-weight:600;cursor:pointer">
          📂 Choisir le fichier .cqp.json
        </button>
        <input type="file" id="import-plant-file" accept=".json,.cqp.json" style="display:none" onchange="importPlantFromFile(this)"/>
        <div style="margin-top:16px;font-size:12px;color:rgba(200,223,204,.3);text-align:center">
          Pour les partages directs via le code, vérifie les notifications en haut de la page.
        </div>
      </div>
    </div>
  </div>`;
...
// (importPlantFromFile définie plus bas)

// (showImportPlantPreview remplacée par showImportPlantPreviewStored)
function showImportPlantPreviewStored(plant, sender){
  _pendingImportPlant=plant;
  closeModal();
  const root=document.getElementById('overlay-root');
  root.innerHTML=`<div class="modal-bg" onclick="if(event.target===this)closeModal()">
    <div class="modal" style="max-width:440px">
      <div style="padding:20px 52px 0 20px;position:relative">
        <button class="modal-close" onclick="closeModal()">✕</button>
        <div style="font-family:var(--disp);font-size:16px;font-weight:900;color:var(--cr)">📬 Fiche reçue de <em style="color:var(--am2)">${sender}</em></div>
      </div>
      <div style="padding:16px 20px">
        <div style="background:rgba(19,48,29,.6);border:1px solid rgba(74,184,112,.15);border-radius:12px;padding:14px;margin-bottom:16px">
          <div style="font-style:italic;font-size:16px;color:var(--cr);margin-bottom:4px">${plant.latin}</div>
          <div style="font-size:13px;color:rgba(200,223,204,.6)">${plant.nom||''}</div>
          <div style="font-size:11px;color:rgba(200,223,204,.35);margin-top:6px">${[plant.type,plant.famille,plant.feuillage].filter(Boolean).join(' · ')}</div>
        </div>
        <div style="font-size:12px;color:rgba(200,223,204,.45);margin-bottom:14px">Veux-tu ajouter cette fiche à tes végétaux personnels ?</div>
        <div style="display:flex;gap:8px">
          <button onclick="closeModal()"
            style="flex:1;padding:12px;background:rgba(255,80,80,.06);border:1px solid rgba(255,80,80,.15);border-radius:10px;color:#ff9999;font-family:'DM Sans',sans-serif;font-size:13px;cursor:pointer">✕ Refuser</button>
          <button onclick="acceptStoredImport()"
            style="flex:2;padding:12px;background:linear-gradient(135deg,#1e5030,#2d7a4a);border:none;border-radius:10px;color:white;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:700;cursor:pointer">✅ Accepter et ajouter</button>
        </div>
      </div>
    </div>
  </div>`;
...
function importPlantFromFile(input){
  const file=input.files[0]; if(!file) return;
  const reader=new FileReader();
  reader.onload=function(e){
    try{
      const data=JSON.parse(e.target.result);
      if(data.type!=='chloroquiz_plant'||!data.plant||!data.plant.latin){
        alert('❌ Fichier invalide.'); return;
      }
      showImportPlantPreviewStored(data.plant, data.sender||'inconnu');
    }catch(ex){ alert('❌ Fichier illisible : '+ex.message); }
  };
  reader.readAsText(file,'UTF-8');
}

function acceptStoredImport(){
  if(!_pendingImportPlant) return;
  addPlantToMyCollection(_pendingImportPlant);
  _pendingImportPlant=null;
  closeModal();
}

function addPlantToMyCollection(plant){
  const newP={...plant, id:nextMyPlantId()};
  myPlants.push(newP);
  saveMyPlants();
  showToast('✅ Fiche ajoutée à tes végétaux !');
  renderMyPlants();
}

// ── Fiches en attente depuis Supabase ──
async function openPendingShares(){
  if(!cloudOk){ alert('Connexion cloud requise.'); return; }
  const shareCode=getOrCreateShareCode(myPlantsOwner);
  let rows=[];
  try{
    rows=await sbGet('floralab_plant_shares',
      `recipient_code=eq.${encodeURIComponent(shareCode)}&status=eq.pending&order=created_at.asc`);
  }catch(e){ alert('Erreur : '+e.message); return; }
  if(!rows.length){ showToast('Aucune fiche en attente.'); return; }
  ...
  root.innerHTML=`<div class="modal-bg" onclick="if(event.target===this)closeModal()">
    <div class="modal" style="max-width:500px">
      <div style="padding:20px 52px 0 20px;position:relative">
        <button class="modal-close" onclick="closeModal()">✕</button>
        <div style="font-family:var(--disp);font-size:18px;font-weight:900;color:var(--cr)">📬 Fiches reçues (${rows.length})</div>
      </div>
      <div style="padding:16px 20px;max-height:60vh;overflow-y:auto">
        ${rows.map(row=>`
          <div style="background:rgba(19,48,29,.6);border:1px solid rgba(74,184,112,.12);border-radius:12px;padding:14px;margin-bottom:10px">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
              <div>
                <div style="font-style:italic;font-size:15px;color:var(--cr)">${row.plant.latin}</div>
                <div style="font-size:12px;color:rgba(200,223,204,.5)">${row.plant.nom||''} · de <strong>${row.sender_name}</strong></div>
              </div>
            </div>
            <div style="display:flex;gap:8px">
              <button onclick="acceptShareRow(${row.id})"
                style="flex:1;padding:8px;background:rgba(74,184,112,.15);border:1px solid rgba(74,184,112,.3);border-radius:8px;color:var(--g4);font-family:'DM Sans',sans-serif;font-size:12px;font-weight:600;cursor:pointer">✅ Accepter</button>
              <button onclick="rejectShareRow(${row.id})"
                style="flex:1;padding:8px;background:rgba(255,80,80,.06);border:1px solid rgba(255,80,80,.15);border-radius:8px;color:#ff9999;font-family:'DM Sans',sans-serif;font-size:12px;cursor:pointer">✕ Refuser</button>
            </div>
          </div>`).join('')}
      </div>
    </div>
...
async function acceptShareRow(shareId){
  try{
    const rows=await sbGet('floralab_plant_shares',`id=eq.${shareId}&select=*`);
    if(!rows.length) return;
    addPlantToMyCollection(rows[0].plant);
    await sbPatch('floralab_plant_shares',`id=eq.${shareId}`,{status:'accepted'});
    // Refresh pending list
    const btn=document.querySelector(`[onclick="acceptShareRow(${shareId})"]`);
    if(btn) btn.closest('div[style*="border-radius:12px"]').remove();
  }catch(e){ alert('Erreur : '+e.message); }
}

async function rejectShareRow(shareId){
  try{
    await sbPatch('floralab_plant_shares',`id=eq.${shareId}`,{status:'rejected'});
    const btn=document.querySelector(`[onclick="rejectShareRow(${shareId})"]`);
    if(btn) btn.closest('div[style*="border-radius:12px"]').remove();
    showToast('Fiche refusée.');
  }catch(e){ alert('Erreur : '+e.message); }
}
...
function renderFiches(){
  ...
  const TYPE_VALS_S=new Set(TYPE_OPTIONS.map(t=>t.toLowerCase()));
  function plantMatchesToken(p, tok){
    // Type exact match
    if(TYPE_VALS_S.has(tok)){
      const plantType=(p.type||'').toLowerCase();
      // Filtrer les valeurs vides pour éviter que tok.includes('') = true
      return plantType.split(',').map(s=>s.trim()).filter(Boolean).some(t=>t===tok||t.includes(tok)||tok.includes(t));
    }
...
<head>
<meta charset="UTF-8"/>
<title>Fiche — ${italLatin(p.latin)}</title>
<style>
...
<body>
  <div class="header">
    ...
    <div class="latin">${italLatin(p.latin)}</div>
...
function openModal(id){
  ...
  // Mobile : mini-carrousel à hauteur fixe pour éviter que les photos empilées
  // ne fassent déborder le modal et bloquent la détection du swipe horizontal
  const photoHTML = (function(){
    if(!_allPhotosModal.length) return `<div class="modal-img-fb">${LEAF_SVG}</div>`;
    if(_allPhotosModal.length===1)
      return `<img src="${_allPhotosModal[0]}" class="modal-img-full" style="margin-top:8px;max-height:55vw" onerror="this.style.display='none'" loading="lazy"/>`;
    // Plusieurs photos → mini-carrousel à hauteur fixe
    const slides = _allPhotosModal.map(ph=>
      `<div style="min-width:100%;box-sizing:border-box"><img src="${ph}" style="width:100%;height:52vw;max-height:220px;object-fit:contain;background:#071610;display:block" onerror="this.style.display='none'" loading="lazy"/></div>`
    ).join('');
    const dots = _allPhotosModal.map((_,i)=>
      `<span id="mpd-${p.id}-${i}" onclick="mpGoSlide(${p.id},${i})" style="display:inline-block;width:7px;height:7px;border-radius:50%;background:${i===0?'var(--g4)':'rgba(200,223,204,.25)'};cursor:pointer;transition:background .2s;margin:0 3px"></span>`
    ).join('');
    return `<div style="position:relative;overflow:hidden;margin-top:8px;touch-action:pan-y">
      <div id="mpt-${p.id}" style="display:flex;transition:transform .28s ease;will-change:transform">${slides}</div>
      <div style="text-align:center;padding:6px 0 2px">${dots}</div>
    </div>`;
  })();
  ...
  if(isMobile){
    ...
    const page2=`
      <div class="mswipe-page" style="overflow-y:auto">
        <div style="padding:14px 52px 16px 16px">
          <button class="modal-close" onclick="closeModal()">✕</button>
          ${p.famille?`<div class="modal-famille">${p.famille}</div>`:''}
          <div class="modal-latin" style="font-size:18px;margin-bottom:12px">${italLatin(p.latin)}</div>
    ...
    initModalSwipe();
    // Swipe sur le mini-carrousel photos (page 1)
    var mpTrack = document.getElementById('mpt-'+p.id);
    if(mpTrack && _allPhotosModal.length > 1){
      var mpStart=0, mpMoved=false, mpIsH=null, mpId=p.id;
      var mpLen = _allPhotosModal.length;
      mpTrack.addEventListener('touchstart',function(e){
        mpStart=e.touches[0].clientX; mpMoved=false; mpIsH=null;
      },{passive:true});
      mpTrack.addEventListener('touchmove',function(e){
        var dx=e.touches[0].clientX-mpStart;
        var dy=e.touches[0].clientY-e.touches[0].clientY; // toujours 0
        var adx=Math.abs(e.touches[0].clientX-mpStart);
        // On détecte l'axe au bout de 8px
        if(mpIsH===null && adx>8) mpIsH=true;
        if(mpIsH){ e.stopPropagation(); mpMoved=true; }
      },{passive:true});
      mpTrack.addEventListener('touchend',function(e){
        if(!mpMoved||!mpIsH) return;
        var dx=e.changedTouches[0].clientX-mpStart;
        var cur=_mpIdx[mpId]||0;
        if(dx<-30 && cur<mpLen-1) mpGoSlide(mpId, cur+1);
        else if(dx>30 && cur>0)   mpGoSlide(mpId, cur-1);
        mpIsH=null; mpMoved=false;
      },{passive:true});
    }
  } else {
...
var _modalIdx=0;
// Mini-carrousel photos (modal mobile, plusieurs photos)
var _mpIdx={};
function mpGoSlide(plantId, idx){
  _mpIdx[plantId]=idx;
  var track=document.getElementById('mpt-'+plantId);
  if(track) track.style.transform='translateX(-'+idx*100+'%)';
  // Mettre à jour les dots
  var i=0;
  while(true){
    var dot=document.getElementById('mpd-'+plantId+'-'+i);
    if(!dot) break;
    dot.style.background = i===idx ? 'var(--g4)' : 'rgba(200,223,204,.25)';
    i++;
  }
}
...
function initModalSwipe(){
  ...
  // Touch — distingue scroll vertical (laisser passer) vs swipe horizontal (intercepter)
  var startY=0, isHoriz=null;
  track.addEventListener('touchstart',function(e){
    startX=e.touches[0].clientX;
    startY=e.touches[0].clientY;
    moved=false; isHoriz=null;
  },{passive:true});
  track.addEventListener('touchmove',function(e){
    ...
    if(isHoriz){
      e.preventDefault(); // bloquer le scroll vertical pendant un swipe horizontal
      moved=true;
    }
  },{passive:false});
  track.addEventListener('touchend',function(e){
    if(moved&&isHoriz) handleSwipe(e.changedTouches[0].clientX-startX);
    isHoriz=null; moved=false;
  },{passive:true});
  // Souris
...
function showFicheComplete(id){
  ...
  const secMorpho = [
    ...
    row('Disposition des feuilles', p.dispositionFeuilles),
...
function switchAdminTab(tab){
  ['plantes','cloud','mdp','plantnet','multi','glossaire','users'].forEach(t=>{
  ...
  if(tab==='plantnet') renderPlantnetTab();
  if(tab==='multi') renderMultiTab();
  if(tab==='glossaire') renderGlossList();
  if(tab==='users') renderUsersTab();
  // Sur mobile : scroll vers le contenu (sidebar au-dessus du contenu)
  if(window.innerWidth <= 700){
    const content = document.querySelector('.admin-content');
    if(content) setTimeout(function(){ content.scrollIntoView({behavior:'smooth', block:'start'}); }, 50);
  }
}
// ══════════════════════════════════════════════════════
...
function _showCodeToast(nom, prenom, code){
  const d=document.createElement('div');
  d.style.cssText='position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);'+
    'background:#1a3a28;border:2px solid #4ab870;color:#c8dfc4;padding:28px 36px;border-radius:18px;'+
    'z-index:99999;text-align:center;box-shadow:0 8px 40px rgba(0,0,0,.6);min-width:280px;max-width:90vw';
  d.innerHTML=`<div style="font-size:13px;color:rgba(200,223,204,.6);margin-bottom:8px">Compte créé pour</div>
    <div style="font-size:18px;font-weight:700;margin-bottom:12px">${_fmtPre(prenom)} ${_fmtNom(nom)}</div>
    <div style="font-size:11px;color:rgba(200,223,204,.5);margin-bottom:6px;text-transform:uppercase;letter-spacing:1px">Code de connexion</div>
    <div style="font-size:32px;font-weight:900;letter-spacing:4px;color:#4ab870;font-family:monospace;margin-bottom:16px">${code}</div>
    <div style="font-size:11px;color:rgba(200,223,204,.4);margin-bottom:18px">Communiquer ce code à l'utilisateur</div>
    <button onclick="navigator.clipboard&&navigator.clipboard.writeText('${code}');this.textContent='✅ Copié !';setTimeout(()=>this.textContent='📋 Copier le code',1500)"
      style="padding:8px 20px;background:rgba(74,184,112,.15);border:1px solid rgba(74,184,112,.4);border-radius:10px;color:#4ab870;cursor:pointer;font-size:13px;font-weight:600;margin-right:8px">📋 Copier le code</button>
    <button onclick="this.closest('div[style]').remove()"
      style="padding:8px 20px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:10px;color:rgba(200,223,204,.7);cursor:pointer;font-size:13px">Fermer</button>`;
...
async function renderUsersTab(){
  ...
  if(!tab) return;
  tab.innerHTML='<div style="color:rgba(200,223,204,.5);padding:20px;text-align:center">Chargement…</div>';
  let users=[], requests=[];
  try{
    if(cloudOk && window.SUPA_URL && window.SUPA_KEY){
      const hdr={'apikey':window.SUPA_KEY,'Authorization':'Bearer '+window.SUPA_KEY};
      const [ru,rr]=await Promise.all([
        fetch(window.SUPA_URL+'/rest/v1/floralab_users?order=nom.asc&select=*',{headers:hdr}),
        fetch(window.SUPA_URL+'/rest/v1/floralab_requests?statut=eq.pending&order=created_at.desc&select=*',{headers:hdr})
      ]);
      if(ru.ok) users=await ru.json();
      if(rr.ok) requests=await rr.json();
      // Mettre en cache
      try{ localStorage.setItem(SK_USERS_CACHE, JSON.stringify(users)); }catch(e){}
    } else {
      try{ users=JSON.parse(localStorage.getItem(SK_USERS_CACHE)||'[]'); }catch(e){}
    }
  }catch(e){ cqErr('renderUsersTab',e); }

  const ROLES={admin:'👑 Admin',formateur:'🎓 Formateur',apprenti:'🌱 Apprenti',amateur:'🌿 Amateur'};
  const roleColors={admin:'#e8a020',formateur:'#7ab8f5',apprenti:'#4ab870',amateur:'#a0d878'};

  tab.innerHTML=`
    <div style="padding:4px 0 18px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:8px">
        <h3 style="font-size:16px;font-weight:700;color:#c8dfc4;margin:0">👥 Utilisateurs</h3>
        <button onclick="openCreateUserForm()"
          style="padding:8px 16px;background:linear-gradient(135deg,#1e5030,#2d7a4a);border:none;border-radius:10px;color:#c8dfc4;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:700;cursor:pointer">
      ...
      <div style="margin-bottom:20px">
        <div style="font-size:11px;text-transform:uppercase;letter-spacing:1px;color:#e8a020;margin-bottom:10px;display:flex;align-items:center;gap:6px">
          🔔 Demandes en attente (${requests.length})
        </div>
        <div style="display:flex;flex-direction:column;gap:8px">
          ${requests.map(r=>`
          <div style="background:rgba(232,160,32,.06);border:1px solid rgba(232,160,32,.2);border-radius:12px;padding:12px 14px;display:flex;align-items:center;gap:10px;flex-wrap:wrap">
            <div style="flex:1;min-width:160px">
              <div style="font-weight:700;color:#c8dfc4;font-size:14px">${_fmtPre(r.prenom||'')} ${_fmtNom(r.nom||'')}</div>
              <div style="font-size:11px;color:rgba(200,223,204,.5);margin-top:2px">
                ${ROLES[r.role]||r.role}${r.formateur_nom?' · Formateur demandé : '+r.formateur_nom:''}
                ${r.mail?' · '+r.mail:''}
              </div>
            </div>
            <div style="display:flex;gap:6px">
              <button onclick="acceptRequest('${r.id}','${(r.nom||'').replace(/'/g,"\\'")}','${(r.prenom||'').replace(/'/g,"\\'")}','${r.role||"apprenti"}')"
                style="padding:6px 14px;background:rgba(74,184,112,.15);border:1px solid rgba(74,184,112,.4);border-radius:8px;color:#4ab870;cursor:pointer;font-size:12px;font-weight:700">✅ Valider</button>
              <button onclick="rejectRequest('${r.id}')"
                style="padding:6px 12px;background:rgba(220,60,60,.1);border:1px solid rgba(220,60,60,.3);border-radius:8px;color:#e07070;cursor:pointer;font-size:12px">✕ Refuser</button>
            </div>
          </div>`).join('')}
        </div>
      </div>` : ''}

      <div style="font-size:11px;text-transform:uppercase;letter-spacing:1px;color:rgba(200,223,204,.4);margin-bottom:10px">
        Comptes actifs (${users.filter(u=>u.actif).length} / ${users.length})
      </div>
      <div style="display:flex;flex-direction:column;gap:6px">
        ${users.map(u=>`
        <div style="background:rgba(255,255,255,.03);border:1px solid rgba(74,184,112,.1);border-radius:12px;padding:10px 14px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;opacity:${u.actif?1:0.45}">
          <div style="flex:1;min-width:140px">
            <div style="font-weight:700;color:#c8dfc4;font-size:14px">${_fmtPre(u.prenom||'')} ${_fmtNom(u.nom||'')}</div>
            <div style="font-size:11px;color:rgba(200,223,204,.4);margin-top:2px;font-family:monospace;letter-spacing:1px">${u.code||'—'}</div>
          </div>
          <span style="font-size:11px;background:rgba(74,184,112,.1);border:1px solid rgba(74,184,112,.2);border-radius:20px;padding:2px 10px;color:${roleColors[u.role]||'#c8dfc4'}">${ROLES[u.role]||u.role}</span>
          <div style="display:flex;gap:5px">
            <button onclick="regenCode('${u.id}')" title="Régénérer le code"
              style="padding:5px 10px;background:rgba(74,184,112,.08);border:1px solid rgba(74,184,112,.2);border-radius:8px;color:#4ab870;cursor:pointer;font-size:12px">🔑</button>
            <button onclick="toggleUserActif('${u.id}',${u.actif})"
              style="padding:5px 10px;background:${u.actif?'rgba(220,60,60,.08)':'rgba(74,184,112,.08)'};border:1px solid ${u.actif?'rgba(220,60,60,.25)':'rgba(74,184,112,.25)'};border-radius:8px;color:${u.actif?'#e07070':'#4ab870'};cursor:pointer;font-size:12px">
...
async function acceptRequest(reqId, nom, prenom, roleReq){
  const code=_genCode(8);
  try{
    const hdr={'apikey':window.SUPA_KEY,'Authorization':'Bearer '+window.SUPA_KEY,'Content-Type':'application/json','Prefer':'return=minimal'};
    // Créer l'utilisateur
    await fetch(window.SUPA_URL+'/rest/v1/floralab_users',{method:'POST',headers:hdr,
      body:JSON.stringify({nom:_fmtNom(nom),prenom:_fmtPre(prenom),code,role:roleReq||'apprenti',actif:true})});
    // Mettre la demande en accepted
    await fetch(window.SUPA_URL+'/rest/v1/floralab_requests?id=eq.'+reqId,{method:'PATCH',headers:hdr,
      body:JSON.stringify({statut:'accepted'})});
    _showCodeToast(nom, prenom, code);
    renderUsersTab();
...
async function toggleUserActif(userId, actif){
  const msg = actif ? 'Désactiver cet utilisateur (ne pourra plus se connecter) ?' : 'Réactiver cet utilisateur ?';
  if(!confirm(msg)) return;
  try{
    const hdr={'apikey':window.SUPA_KEY,'Authorization':'Bearer '+window.SUPA_KEY,'Content-Type':'application/json','Prefer':'return=minimal'};
    await fetch(window.SUPA_URL+'/rest/v1/floralab_users?id=eq.'+userId,{method:'PATCH',headers:hdr,
      body:JSON.stringify({actif:!actif})});
    renderUsersTab();
  }catch(e){ alert('Erreur : '+e.message); }
}

async function regenCode(userId){
  if(!confirm('Régénérer le code de cet utilisateur ? L\'ancien code ne fonctionnera plus.')) return;
  const newCode=_genCode(8);
  try{
    const hdr={'apikey':window.SUPA_KEY,'Authorization':'Bearer '+window.SUPA_KEY,'Content-Type':'application/json','Prefer':'return=minimal'};
    // Patch user
    await fetch(window.SUPA_URL+'/rest/v1/floralab_users?id=eq.'+userId,{method:'PATCH',headers:hdr,
      body:JSON.stringify({code:newCode})});
    // Récupérer le nom pour le toast
    const res=await fetch(window.SUPA_URL+'/rest/v1/floralab_users?id=eq.'+userId+'&select=nom,prenom',{headers:hdr});
    if(res.ok){ const rows=await res.json(); if(rows.length) _showCodeToast(rows[0].nom,rows[0].prenom,newCode); }
    renderUsersTab();
  }catch(e){ alert('Erreur : '+e.message); }
}
...
function openCreateUserForm(){
  const d=document.createElement('div');
  d.id='create-user-overlay';
  d.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.75);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px';
  d.innerHTML=`
    <div style="background:#0f2a1c;border:1px solid rgba(74,184,112,.3);border-radius:20px;padding:28px;max-width:400px;width:100%">
      <h3 style="color:#c8dfc4;margin:0 0 20px;font-size:16px">Créer un utilisateur</h3>
      <div style="display:flex;flex-direction:column;gap:12px">
        <input id="cu-nom" placeholder="NOM (ex: DUPONT)" maxlength="40"
          style="padding:10px 14px;background:rgba(255,255,255,.05);border:1px solid rgba(74,184,112,.25);border-radius:10px;color:#c8dfc4;font-family:'DM Sans',sans-serif;font-size:14px"
          oninput="this.value=this.value.toUpperCase().replace(/[^A-Z0-9\\s\\-]/g,'')"/>
        <input id="cu-prenom" placeholder="Prénom (ex: Jean)" maxlength="40"
          style="padding:10px 14px;background:rgba(255,255,255,.05);border:1px solid rgba(74,184,112,.25);border-radius:10px;color:#c8dfc4;font-family:'DM Sans',sans-serif;font-size:14px"/>
        <input id="cu-mail" placeholder="Email (optionnel)" type="email" maxlength="80"
          style="padding:10px 14px;background:rgba(255,255,255,.05);border:1px solid rgba(74,184,112,.25);border-radius:10px;color:#c8dfc4;font-family:'DM Sans',sans-serif;font-size:14px"/>
        <select id="cu-role"
          style="padding:10px 14px;background:#0f2a1c;border:1px solid rgba(74,184,112,.25);border-radius:10px;color:#c8dfc4;font-family:'DM Sans',sans-serif;font-size:14px">
          <option value="amateur">🌿 Amateur</option>
          <option value="apprenti" selected>🌱 Apprenti</option>
          <option value="formateur">🎓 Formateur</option>
          <option value="admin">👑 Admin</option>
        </select>
        <div id="cu-err" style="font-size:12px;color:#e07070;min-height:16px"></div>
        <div style="display:flex;gap:8px;margin-top:4px">
          <button onclick="confirmCreateUser()"
            style="flex:1;padding:11px;background:linear-gradient(135deg,#1e5030,#2d7a4a);border:none;border-radius:10px;color:#c8dfc4;font-family:'DM Sans',sans-serif;font-size:14px;font-weight:700;cursor:pointer">
            Créer & afficher le code
          </button>
          <button onclick="document.getElementById('create-user-overlay').remove()"
            style="padding:11px 16px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:10px;color:rgba(200,223,204,.6);cursor:pointer;font-size:13px">
            Annuler
          </button>
        </div>
      </div>
...
async function confirmCreateUser(){
  const nom=(_fmtNom(document.getElementById('cu-nom').value)||'').trim();
  const prenom=(_fmtPre(document.getElementById('cu-prenom').value)||'').trim();
  const mail=(document.getElementById('cu-mail').value||'').trim();
  const role=document.getElementById('cu-role').value;
  const err=document.getElementById('cu-err');
  if(!nom){ err.textContent='Nom requis'; return; }
  if(!prenom){ err.textContent='Prénom requis'; return; }
  const code=_genCode(8);
  try{
    if(cloudOk && window.SUPA_URL && window.SUPA_KEY){
      const hdr={'apikey':window.SUPA_KEY,'Authorization':'Bearer '+window.SUPA_KEY,'Content-Type':'application/json','Prefer':'return=minimal'};
      const res=await fetch(window.SUPA_URL+'/rest/v1/floralab_users',{method:'POST',headers:hdr,
        body:JSON.stringify({nom,prenom,mail:mail||null,code,role,actif:true})});
      if(!res.ok){ const t=await res.text(); err.textContent='Erreur: '+t; return; }
    } else {
      // Local cache only
      try{
        const c=JSON.parse(localStorage.getItem(SK_USERS_CACHE)||'[]');
        c.push({id:'local-'+Date.now(),nom,prenom,mail:mail||null,code,role,actif:true});
        localStorage.setItem(SK_USERS_CACHE,JSON.stringify(c));
      }catch(e){}
    }
    document.getElementById('create-user-overlay').remove();
    _showCodeToast(nom, prenom, code);
    renderUsersTab();
...
// ══════════════════════════════════════════════════════
//  MESSAGERIE — floralab_messages
// ══════════════════════════════════════════════════════
var msgPollInterval = null;
var _msgOpenConv = null; // {toId, toNom, toPrenom, toRole, isBroadcast}

function _startMsgPoll(){
  if(msgPollInterval) return;
  msgPollInterval = setInterval(()=>{ if(cloudOk) _updateMsgBadge(); }, 12000);
  _updateMsgBadge();
}

async function _updateMsgBadge(){
  if(!cloudOk||!window.SUPA_URL||!window.SUPA_KEY||!currentUser) return;
  try{
    const hdr={'apikey':window.SUPA_KEY,'Authorization':'Bearer '+window.SUPA_KEY};
    // Messages directs non lus
    let url = window.SUPA_URL+'/rest/v1/floralab_messages?to_id=eq.'+encodeURIComponent(currentUser.id)+'&select=id,read_by';
    const r=await fetch(url,{headers:hdr});
    let msgs = r.ok ? await r.json() : [];
    // Broadcast admin→tous
    if(role==='apprenti'||role==='formateur'||role==='admin'||role==='amateur'){
      const rb=await fetch(window.SUPA_URL+'/rest/v1/floralab_messages?is_broadcast=eq.true&select=id,read_by,formateur_id,to_role',{headers:hdr});
      if(rb.ok){
        const bmsgs=await rb.json();
        bmsgs.forEach(m=>{
          // Broadcast admin→tous: visible par tous
          if(m.to_role==='all') msgs.push(m);
          // Broadcast formateur→ses apprentis
          else if(m.to_role==='apprentis' && m.formateur_id && role==='apprenti'){
            // Vérifier si je suis l'apprenti de ce formateur via SK_USERS_CACHE
            try{
              const cache=JSON.parse(localStorage.getItem(SK_USERS_CACHE)||'[]');
              const me=cache.find(u=>u.id===currentUser.id);
              if(me && me.formateur_id===m.formateur_id) msgs.push(m);
            }catch(e){}
          }
          else if(m.to_role==='apprentis' && m.formateur_id===currentUser.id) msgs.push(m); // formateur voit ses propres broadcasts
        });
      }
    }
    const unread=msgs.filter(m=>{
      const rb=Array.isArray(m.read_by)?m.read_by:(typeof m.read_by==='string'?JSON.parse(m.read_by||'[]'):[]);
      return !rb.includes(currentUser.id);
    }).length;
    // Mettre à jour les badges
    ['msg-badge-top','msg-badge-bn'].forEach(bid=>{
      const el=document.getElementById(bid);
      if(el){ el.textContent=unread||''; el.style.display=unread?'inline-flex':'none'; }
    });
    // Si on est sur la page messages → re-render
    const pg=document.getElementById('page-messages');
    if(pg && pg.style.display!=='none') renderMessages();
  }catch(e){ cqWarn('_updateMsgBadge',e); }
}
...
async function renderMessages(){
  ...
  if(!cloudOk||!window.SUPA_URL||!window.SUPA_KEY){
    c.innerHTML='<div style="padding:40px;text-align:center;color:rgba(200,223,204,.4)">☁️ Connexion Supabase requise pour la messagerie</div>';
    return;
  }
  const hdr={'apikey':window.SUPA_KEY,'Authorization':'Bearer '+window.SUPA_KEY};
  try{
    // Charger tous mes messages (envoyés + reçus)
    const [rSent,rRecv]=await Promise.all([
      fetch(window.SUPA_URL+'/rest/v1/floralab_messages?from_id=eq.'+encodeURIComponent(currentUser.id)+'&select=*&order=created_at.desc',{headers:hdr}),
      fetch(window.SUPA_URL+'/rest/v1/floralab_messages?to_id=eq.'+encodeURIComponent(currentUser.id)+'&select=*&order=created_at.desc',{headers:hdr})
    ]);
    let sent = rSent.ok ? await rSent.json() : [];
    let recv = rRecv.ok ? await rRecv.json() : [];
    // Broadcasts visibles
    const rBc=await fetch(window.SUPA_URL+'/rest/v1/floralab_messages?is_broadcast=eq.true&select=*&order=created_at.desc',{headers:hdr});
    const bcAll=rBc.ok?await rBc.json():[];
    const bcVisible=bcAll.filter(m=>{
      if(m.from_id===currentUser.id) return true;
      if(m.to_role==='all') return true;
      if(m.to_role==='apprentis'&&role==='apprenti'){
        try{ const cache=JSON.parse(localStorage.getItem(SK_USERS_CACHE)||'[]'); const me=cache.find(u=>u.id===currentUser.id); return me&&me.formateur_id===m.formateur_id; }catch(e){ return false; }
      }
      return false;
    });
    // Regrouper en conversations
    const convMap={};
    const addMsg=(m)=>{
      if(m.is_broadcast){ const key='bc_'+m.from_id; if(!convMap[key]) convMap[key]={type:'broadcast',otherId:m.from_id,otherNom:m.from_nom,otherPrenom:m.from_prenom,otherRole:m.from_role,msgs:[],isMine:m.from_id===currentUser.id}; convMap[key].msgs.push(m); return; }
      const otherId=m.from_id===currentUser.id?m.to_id:m.from_id;
      const otherNom=m.from_id===currentUser.id?m.to_nom:m.from_nom;
      const otherPrenom=m.from_id===currentUser.id?m.to_prenom:m.from_prenom;
      const otherRole=m.from_id===currentUser.id?m.to_role:m.from_role;
      const key='1:1_'+[currentUser.id,otherId].sort().join('_');
      if(!convMap[key]) convMap[key]={type:'direct',otherId,otherNom,otherPrenom,otherRole,msgs:[]};
      convMap[key].msgs.push(m);
    };
    [...sent,...recv,...bcVisible].forEach(m=>addMsg(m));
    // Trier chaque conversation par date
    Object.values(convMap).forEach(cv=>{ cv.msgs.sort((a,b)=>new Date(a.created_at)-new Date(b.created_at)); cv.lastMsg=cv.msgs[cv.msgs.length-1]; cv.unread=cv.msgs.filter(m=>{ const rb=Array.isArray(m.read_by)?m.read_by:(typeof m.read_by==='string'?JSON.parse(m.read_by||'[]'):[]);return m.to_id===currentUser.id&&!rb.includes(currentUser.id)||m.to_role&&!rb.includes(currentUser.id); }).length; });
    const convs=Object.values(convMap).sort((a,b)=>new Date(b.lastMsg?.created_at||0)-new Date(a.lastMsg?.created_at||0));
    // Si une conversation est ouverte → afficher la thread
    if(_msgOpenConv){
      const key=_msgOpenConv.isBroadcast?'bc_'+_msgOpenConv.toId:'1:1_'+[currentUser.id,_msgOpenConv.toId].sort().join('_');
      const cv=convMap[key];
      _renderConvThread(c, cv?cv.msgs:[], convs);
      return;
    }
    _renderConvList(c, convs);
  }catch(e){ cqErr('renderMessages',e); c.innerHTML='<div style="padding:30px;color:#e07070">Erreur chargement messages</div>'; }
}

function _renderConvList(c, convs){
  const canNew = role==='admin'||role==='formateur'||role==='apprenti';
  var html=`<div style="display:flex;flex-direction:column;height:100%;min-height:60vh">
    <div style="display:flex;align-items:center;justify-content:space-between;padding:18px 18px 10px">
      <div style="font-family:var(--disp);font-size:20px;font-weight:900;color:var(--cr)">💬 Messages</div>
      ${canNew?`<button onclick="openNewMessageModal()" style="padding:8px 16px;background:linear-gradient(135deg,#1e5030,#2d7a4a);border:none;border-radius:10px;color:#c8dfc4;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:700;cursor:pointer">＋ Nouveau</button>`:''}
    </div>`;
  if(!convs.length){
    html+=`<div style="padding:50px;text-align:center;color:rgba(200,223,204,.35);font-size:14px">Aucun message pour l'instant</div>`;
  } else {
    html+=`<div style="flex:1;overflow-y:auto;padding:0 10px 10px">`;
    convs.forEach(cv=>{
      const last=cv.lastMsg;
      const lastText=(last?.content||'').substring(0,60)+(last?.content?.length>60?'…':'');
      const lastTime=last?_msgFmtTime(last.created_at):'';
      const name=cv.type==='broadcast'?(cv.isMine?`📢 Broadcast → vos apprentis`:`📢 Broadcast de ${cv.otherPrenom} ${cv.otherNom}`):`${_fmtPre(cv.otherPrenom||'')} ${_fmtNom(cv.otherNom||'')}`;
      const roleIcon=cv.type==='broadcast'?'':_roleIcon(cv.otherRole);
      const isUnread=cv.unread>0;
      html+=`<div onclick="_openConv('${cv.otherId}','${(cv.otherNom||'').replace(/'/g,"\'")}','${(cv.otherPrenom||'').replace(/'/g,"\'")}','${cv.otherRole||''}',${cv.type==='broadcast'})"
        style="display:flex;align-items:center;gap:12px;padding:13px 14px;background:rgba(19,48,29,${isUnread?'.6':'.3'});border:1px solid rgba(74,184,112,${isUnread?'.3':'.1'});border-radius:14px;margin-bottom:8px;cursor:pointer;transition:all .2s">
        <div style="font-size:26px;flex-shrink:0">${cv.type==='broadcast'?'📢':roleIcon}</div>
        <div style="flex:1;min-width:0">
          <div style="font-weight:${isUnread?'700':'500'};color:${isUnread?'var(--cr)':'rgba(200,223,204,.8)'};font-size:14px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${name}</div>
          <div style="font-size:12px;color:rgba(200,223,204,.45);margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${lastText}</div>
        </div>
        <div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px;flex-shrink:0">
          <div style="font-size:11px;color:rgba(200,223,204,.35)">${lastTime}</div>
          ${isUnread?`<div style="background:#e8a020;color:#0b1e12;border-radius:50%;min-width:18px;height:18px;font-size:11px;font-weight:900;display:flex;align-items:center;justify-content:center;padding:0 3px">${cv.unread}</div>`:''}
        </div>
      </div>`;
    });
    html+=`</div>`;
...
function _renderConvThread(c, msgs, convs){
  const conv=_msgOpenConv;
  const name=conv.isBroadcast?'📢 Broadcast apprentis':`${_fmtPre(conv.toPrenom||'')} ${_fmtNom(conv.toNom||'')}`;
  var html=`<div style="display:flex;flex-direction:column;height:100%;min-height:70vh">
    <div style="display:flex;align-items:center;gap:12px;padding:14px 16px;border-bottom:1px solid rgba(74,184,112,.12)">
      <button onclick="_closeConv()" style="background:none;border:none;color:rgba(200,223,204,.5);font-size:20px;cursor:pointer;padding:0 6px">←</button>
      <div style="font-size:20px">${conv.isBroadcast?'📢':_roleIcon(conv.toRole)}</div>
      <div>
        <div style="font-weight:700;color:var(--cr);font-size:15px">${name}</div>
        ${conv.isBroadcast?'<div style="font-size:11px;color:rgba(200,223,204,.4)">Message à tous vos apprentis</div>':''}
      </div>
    </div>
    <div id="msg-thread" style="flex:1;overflow-y:auto;padding:14px;display:flex;flex-direction:column;gap:8px">`;
  // Marquer comme lu
  const unreadIds=msgs.filter(m=>m.to_id===currentUser.id&&!(Array.isArray(m.read_by)?m.read_by:(JSON.parse(m.read_by||'[]'))).includes(currentUser.id)).map(m=>m.id);
  if(unreadIds.length) _markMsgsRead(unreadIds);
  if(!msgs.length){
    html+=`<div style="text-align:center;padding:40px;color:rgba(200,223,204,.3);font-size:13px">Commencez la conversation…</div>`;
  } else {
    msgs.forEach(m=>{
      const isMe=m.from_id===currentUser.id;
      const t=_msgFmtTime(m.created_at);
      html+=`<div style="display:flex;flex-direction:column;align-items:${isMe?'flex-end':'flex-start'}">
        <div style="max-width:75%;background:${isMe?'linear-gradient(135deg,#1e5030,#2d7a4a)':'rgba(19,48,29,.7)'};border:1px solid rgba(74,184,112,${isMe?'.4':'.15'});border-radius:${isMe?'18px 18px 4px 18px':'18px 18px 18px 4px'};padding:10px 14px;color:var(--cr);font-size:14px;line-height:1.5">${_escapeMsg(m.content)}</div>
        <div style="font-size:10px;color:rgba(200,223,204,.3);margin-top:3px;padding:0 4px">${isMe?'Vous':(_fmtPre(m.from_prenom||'')+' '+_fmtNom(m.from_nom||'')+' •')} ${t}</div>
      </div>`;
    });
  }
  html+=`</div>
    <div style="padding:12px 14px;border-top:1px solid rgba(74,184,112,.1);display:flex;gap:8px;align-items:flex-end">
      <textarea id="msg-input" placeholder="Votre message…" maxlength="1000"
        style="flex:1;padding:10px 13px;background:rgba(11,30,18,.8);border:1px solid rgba(74,184,112,.25);border-radius:13px;color:var(--cr);font-family:'DM Sans',sans-serif;font-size:14px;resize:none;outline:none;min-height:42px;max-height:120px;line-height:1.4"
        onkeydown="if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();sendCurrentMessage();}"></textarea>
      <button onclick="sendCurrentMessage()" style="padding:10px 18px;background:linear-gradient(135deg,var(--am),var(--am2));border:none;border-radius:12px;color:#0b1e12;font-weight:700;font-size:15px;cursor:pointer;flex-shrink:0">▶</button>
    </div>
  </div>`;
  c.innerHTML=html;
  // Scroll en bas
  setTimeout(()=>{ const t=document.getElementById('msg-thread'); if(t) t.scrollTop=t.scrollHeight; }, 50);
}

function _openConv(toId, toNom, toPrenom, toRole, isBroadcast){
  _msgOpenConv={toId, toNom, toPrenom, toRole, isBroadcast};
  renderMessages();
}
function _closeConv(){ _msgOpenConv=null; renderMessages(); }
...
async function sendCurrentMessage(){
  ...
  if(!content) return;
  if(!currentUser||!window.SUPA_URL||!window.SUPA_KEY){ alert('Connexion requise'); return; }
  const conv=_msgOpenConv;
  if(!conv) return;
  inp.disabled=true;
  try{
    const hdr={'apikey':window.SUPA_KEY,'Authorization':'Bearer '+window.SUPA_KEY,'Content-Type':'application/json','Prefer':'return=minimal'};
    const body={
      from_id:currentUser.id, from_nom:currentUser.nom, from_prenom:currentUser.prenom, from_role:role,
      to_id:conv.isBroadcast?null:conv.toId,
      to_nom:conv.isBroadcast?null:conv.toNom,
      to_prenom:conv.isBroadcast?null:conv.toPrenom,
      to_role:conv.isBroadcast?'apprentis':conv.toRole,
      is_broadcast:conv.isBroadcast,
      formateur_id:conv.isBroadcast?currentUser.id:null,
      content, read_by:JSON.stringify([currentUser.id]),
      created_at:new Date().toISOString()
    };
    const res=await fetch(window.SUPA_URL+'/rest/v1/floralab_messages',{method:'POST',headers:hdr,body:JSON.stringify(body)});
    if(!res.ok){ const t=await res.text(); throw new Error(t); }
    inp.value='';
    await renderMessages();
  }catch(e){ cqErr('sendCurrentMessage',e); alert('Erreur envoi : '+e.message); }
  finally{ inp.disabled=false; if(inp) inp.focus(); }
}

async function _markMsgsRead(ids){
  if(!ids.length||!window.SUPA_URL||!window.SUPA_KEY) return;
  try{
    const hdr={'apikey':window.SUPA_KEY,'Authorization':'Bearer '+window.SUPA_KEY,'Content-Type':'application/json'};
    // Lire les messages pour update le read_by (Supabase ne supporte pas array_append direct facilement)
    for(const id of ids){
      const r=await fetch(window.SUPA_URL+'/rest/v1/floralab_messages?id=eq.'+id+'&select=id,read_by',{headers:hdr});
      if(!r.ok) continue;
      const rows=await r.json();
      if(!rows.length) continue;
      const rb=Array.isArray(rows[0].read_by)?rows[0].read_by:(typeof rows[0].read_by==='string'?JSON.parse(rows[0].read_by||'[]'):[]);
      if(rb.includes(currentUser.id)) continue;
      rb.push(currentUser.id);
      await fetch(window.SUPA_URL+'/rest/v1/floralab_messages?id=eq.'+id,{method:'PATCH',headers:{...hdr,'Prefer':'return=minimal'},body:JSON.stringify({read_by:rb})});
    }
    _updateMsgBadge();
  }catch(e){ cqWarn('_markMsgsRead',e); }
}

function openNewMessageModal(){
  if(!cloudOk||!window.SUPA_URL||!window.SUPA_KEY){ alert('Connexion cloud requise'); return; }
  const d=document.createElement('div');
  d.id='new-msg-overlay';
  d.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.75);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px';
  const isAdmin=role==='admin'; const isFormateur=role==='formateur';
  let destHtml='';
  if(isAdmin){
    destHtml=`<div style="display:flex;flex-direction:column;gap:8px">
      <label style="font-size:12px;color:rgba(200,223,204,.5)">Destinataire</label>
      <select id="nm-type" style="padding:10px;background:#0f2a1c;border:1px solid rgba(74,184,112,.25);border-radius:10px;color:#c8dfc4;font-family:'DM Sans',sans-serif;font-size:14px"
        onchange="document.getElementById('nm-user-wrap').style.display=this.value==='direct'?'':'none'">
        <option value="broadcast-all">📢 Broadcast → tous les utilisateurs</option>
        <option value="direct">💬 Message direct à un utilisateur</option>
      </select>
      <div id="nm-user-wrap" style="display:none">
        <input id="nm-user-search" placeholder="Rechercher un utilisateur…" maxlength="60"
          style="width:100%;padding:10px;background:#0f2a1c;border:1px solid rgba(74,184,112,.25);border-radius:10px;color:#c8dfc4;font-family:'DM Sans',sans-serif;font-size:14px;box-sizing:border-box"
          oninput="_searchMsgUsers(this.value)"/>
        <div id="nm-user-results" style="margin-top:6px;max-height:150px;overflow-y:auto"></div>
        <input type="hidden" id="nm-to-id"/><input type="hidden" id="nm-to-nom"/><input type="hidden" id="nm-to-prenom"/><input type="hidden" id="nm-to-role"/>
      </div>
    </div>`;
  } else if(isFormateur){
    destHtml=`<div style="display:flex;flex-direction:column;gap:8px">
      ...
      <select id="nm-type" style="padding:10px;background:#0f2a1c;border:1px solid rgba(74,184,112,.25);border-radius:10px;color:#c8dfc4;font-family:'DM Sans',sans-serif;font-size:14px"
        onchange="document.getElementById('nm-user-wrap').style.display=this.value==='direct'?'':'none'">
        <option value="broadcast-apprentis">📢 Broadcast → mes apprentis</option>
    ...
    // Apprenti → formateur ou admin seulement
    destHtml=`<div style="display:flex;flex-direction:column;gap:8px">
      <label style="font-size:12px;color:rgba(200,223,204,.5)">Destinataire</label>
      <input id="nm-user-search" placeholder="Rechercher formateur ou admin…" maxlength="60"
        style="width:100%;padding:10px;background:#0f2a1c;border:1px solid rgba(74,184,112,.25);border-radius:10px;color:#c8dfc4;font-family:'DM Sans',sans-serif;font-size:14px;box-sizing:border-box"
        oninput="_searchMsgUsers(this.value,'formateur,admin')"/>
      <div id="nm-user-results" style="max-height:150px;overflow-y:auto"></div>
      <input type="hidden" id="nm-type" value="direct"/>
      <input type="hidden" id="nm-to-id"/><input type="hidden" id="nm-to-nom"/><input type="hidden" id="nm-to-prenom"/><input type="hidden" id="nm-to-role"/>
    </div>`;
  }
  d.innerHTML=`<div style="background:#0f2a1c;border:1px solid rgba(74,184,112,.3);border-radius:20px;padding:28px;max-width:420px;width:100%">
    <h3 style="color:#c8dfc4;margin:0 0 20px;font-size:16px">💬 Nouveau message</h3>
    <div style="display:flex;flex-direction:column;gap:14px">
      ${destHtml}
      <textarea id="nm-content" placeholder="Votre message…" maxlength="1000" rows="4"
        style="padding:11px 14px;background:rgba(11,30,18,.8);border:1px solid rgba(74,184,112,.25);border-radius:11px;color:#c8dfc4;font-family:'DM Sans',sans-serif;font-size:14px;resize:vertical;outline:none"></textarea>
      <div id="nm-err" style="font-size:12px;color:#e07070;min-height:16px"></div>
      <div style="display:flex;gap:8px">
        <button onclick="_sendNewMessage()" style="flex:1;padding:11px;background:linear-gradient(135deg,#1e5030,#2d7a4a);border:none;border-radius:10px;color:#c8dfc4;font-family:'DM Sans',sans-serif;font-size:14px;font-weight:700;cursor:pointer">📨 Envoyer</button>
        <button onclick="document.getElementById('new-msg-overlay').remove()" style="padding:11px 16px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:10px;color:rgba(200,223,204,.6);cursor:pointer;font-size:13px">Annuler</button>
      </div>
    </div>
  </div>`;
  document.body.appendChild(d);
...
async function _searchMsgUsers(q, roles){
  const res=document.getElementById('nm-user-results');
  if(!res||!q||q.length<2){ if(res) res.innerHTML=''; return; }
  if(!window.SUPA_URL||!window.SUPA_KEY) return;
  const hdr={'apikey':window.SUPA_KEY,'Authorization':'Bearer '+window.SUPA_KEY};
  let url=window.SUPA_URL+'/rest/v1/floralab_users?select=id,nom,prenom,role&actif=eq.true&order=nom.asc&limit=10';
  if(roles) url+=`&role=in.(${roles})`;
  try{
    const r=await fetch(url,{headers:hdr});
    if(!r.ok) return;
    const users=await r.json();
    const qLow=q.toLowerCase();
    const filtered=users.filter(u=>(u.nom||'').toLowerCase().includes(qLow)||(u.prenom||'').toLowerCase().includes(qLow));
    if(!filtered.length){ res.innerHTML='<div style="padding:8px;font-size:12px;color:rgba(200,223,204,.4)">Aucun résultat</div>'; return; }
    res.innerHTML=filtered.map(u=>`<div onclick="_selectMsgUser('${u.id}','${(u.nom||'').replace(/'/g,"\'")}','${(u.prenom||'').replace(/'/g,"\'")}','${u.role||''}')"
      style="padding:9px 12px;cursor:pointer;border-radius:8px;display:flex;align-items:center;gap:8px;background:rgba(19,48,29,.5);margin-bottom:4px;transition:background .15s"
      onmouseover="this.style.background='rgba(74,184,112,.15)'" onmouseout="this.style.background='rgba(19,48,29,.5)'">
      <span>${_roleIcon(u.role)}</span>
      <span style="font-size:14px;color:#c8dfc4">${_fmtNom(u.nom||'')} ${_fmtPre(u.prenom||'')}</span>
      <span style="font-size:11px;color:rgba(200,223,204,.4);margin-left:auto">${u.role||''}</span>
    </div>`).join('');
  }catch(e){ cqWarn('_searchMsgUsers',e); }
}

function _selectMsgUser(id, nom, prenom, role){
  document.getElementById('nm-to-id').value=id;
  document.getElementById('nm-to-nom').value=nom;
  document.getElementById('nm-to-prenom').value=prenom;
  document.getElementById('nm-to-role').value=role;
  const search=document.getElementById('nm-user-search');
  if(search) search.value=_fmtNom(nom)+' '+_fmtPre(prenom);
  const res=document.getElementById('nm-user-results');
  if(res) res.innerHTML=`<div style="padding:6px 10px;font-size:12px;color:var(--g4)">✅ Sélectionné : ${_fmtNom(nom)} ${_fmtPre(prenom)}</div>`;
}
...
async function _sendNewMessage(){
  ...
  if(!content){ err.textContent='Message vide'; return; }
  const nmType=(document.getElementById('nm-type')?.value)||'direct';
  let toId=null,toNom=null,toPrenom=null,toRole=null,isBroadcast=false,targetRole=null,formateurId=null;
  if(nmType==='direct'){
    toId=document.getElementById('nm-to-id').value;
    toNom=document.getElementById('nm-to-nom').value;
    toPrenom=document.getElementById('nm-to-prenom').value;
    toRole=document.getElementById('nm-to-role').value;
    if(!toId){ err.textContent='Sélectionnez un destinataire'; return; }
  } else if(nmType==='broadcast-all'){
    isBroadcast=true; targetRole='all'; formateurId=currentUser.id;
  } else if(nmType==='broadcast-apprentis'){
    isBroadcast=true; targetRole='apprentis'; formateurId=currentUser.id;
  }
  if(!window.SUPA_URL||!window.SUPA_KEY){ err.textContent='Connexion cloud requise'; return; }
  try{
    const hdr={'apikey':window.SUPA_KEY,'Authorization':'Bearer '+window.SUPA_KEY,'Content-Type':'application/json','Prefer':'return=minimal'};
    const body={from_id:currentUser.id,from_nom:currentUser.nom,from_prenom:currentUser.prenom,from_role:role,
      to_id:toId,to_nom:toNom,to_prenom:toPrenom,to_role:isBroadcast?targetRole:toRole,
      is_broadcast:isBroadcast,formateur_id:formateurId,content,
      read_by:JSON.stringify([currentUser.id]),created_at:new Date().toISOString()};
    const res=await fetch(window.SUPA_URL+'/rest/v1/floralab_messages',{method:'POST',headers:hdr,body:JSON.stringify(body)});
    if(!res.ok){ const t=await res.text(); throw new Error(t); }
    document.getElementById('new-msg-overlay').remove();
    _msgOpenConv=isBroadcast?{toId:currentUser.id,toNom:currentUser.nom,toPrenom:currentUser.prenom,toRole:'broadcast',isBroadcast:true}:{toId,toNom,toPrenom,toRole,isBroadcast:false};
    await renderMessages();
  }catch(e){ err.textContent='Erreur : '+e.message; }
}

function _escapeMsg(txt){
  if(!txt) return '';
  return txt.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g,'<br>');
}

function _msgFmtTime(iso){
  if(!iso) return '';
  const d=new Date(iso); const now=new Date();
  const diff=now-d;
  if(diff<60000) return 'À l\'instant';
  if(diff<3600000) return Math.floor(diff/60000)+'min';
  if(diff<86400000) return d.getHours()+'h'+d.getMinutes().toString().padStart(2,'0');
  if(diff<604800000) return ['Dim','Lun','Mar','Mer','Jeu','Ven','Sam'][d.getDay()];
  return d.toLocaleDateString('fr-FR',{day:'2-digit',month:'2-digit'});
}

function _roleIcon(r){
  const m={admin:'👑',formateur:'🎓',apprenti:'🌱',amateur:'🌿'};
  return m[r]||'👤';
}

// ══════════════════════════════════════════════════════
//  INSCRIPTION (page publique)
// ══════════════════════════════════════════════════════
function openInscription(){
  const d=document.createElement('div');
  d.id='inscription-overlay';
  d.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px;overflow-y:auto';
  d.innerHTML=`
    <div style="background:#0b1e12;border:1px solid rgba(74,184,112,.3);border-radius:22px;padding:32px;max-width:420px;width:100%;position:relative">
      <button onclick="document.getElementById('inscription-overlay').remove()"
        style="position:absolute;top:14px;right:16px;background:none;border:none;color:rgba(200,223,204,.4);font-size:20px;cursor:pointer">✕</button>
      <div style="text-align:center;margin-bottom:24px">
        <div style="font-size:28px;margin-bottom:8px">🌿</div>
        <div style="font-size:18px;font-weight:700;color:#c8dfc4">Demander un accès</div>
        <div style="font-size:12px;color:rgba(200,223,204,.45);margin-top:4px">Votre demande sera examinée par l'administrateur</div>
      </div>
      <div style="display:flex;flex-direction:column;gap:11px">
        <input id="ins-nom" placeholder="NOM (ex: DUPONT)" maxlength="40"
          style="padding:11px 14px;background:rgba(255,255,255,.05);border:1px solid rgba(74,184,112,.25);border-radius:11px;color:#c8dfc4;font-family:'DM Sans',sans-serif;font-size:14px;outline:none"
          oninput="this.value=this.value.toUpperCase().replace(/[^A-ZÉÈÊËÀÂÙÛÎÏÔŒÇ\\s\\-']/gi,m=>m.normalize('NFD').replace(/[\\u0300-\\u036f]/g,'').toUpperCase())"/>
        <input id="ins-prenom" placeholder="Prénom (ex: Jean)" maxlength="40"
          style="padding:11px 14px;background:rgba(255,255,255,.05);border:1px solid rgba(74,184,112,.25);border-radius:11px;color:#c8dfc4;font-family:'DM Sans',sans-serif;font-size:14px;outline:none"/>
        <input id="ins-mail" placeholder="Adresse email" type="email" maxlength="80"
          style="padding:11px 14px;background:rgba(255,255,255,.05);border:1px solid rgba(74,184,112,.25);border-radius:11px;color:#c8dfc4;font-family:'DM Sans',sans-serif;font-size:14px;outline:none"/>
        <select id="ins-role"
          style="padding:11px 14px;background:#0f2a1c;border:1px solid rgba(74,184,112,.25);border-radius:11px;color:#c8dfc4;font-family:'DM Sans',sans-serif;font-size:14px;outline:none"
          onchange="document.getElementById('ins-formateur-wrap').style.display=this.value==='apprenti'?'':'none'">
          <option value="amateur">🌿 Amateur (grand public)</option>
          <option value="apprenti" selected>🌱 Apprenti (CFA)</option>
          <option value="formateur">🎓 Formateur</option>
        </select>
        <div id="ins-formateur-wrap">
          <input id="ins-formateur" placeholder="Nom de votre formateur" maxlength="60"
            style="padding:11px 14px;background:rgba(255,255,255,.05);border:1px solid rgba(74,184,112,.25);border-radius:11px;color:#c8dfc4;font-family:'DM Sans',sans-serif;font-size:14px;outline:none;width:100%;box-sizing:border-box"/>
        </div>
        <div id="ins-err" style="font-size:12px;color:#e07070;min-height:16px;text-align:center"></div>
        <button onclick="submitInscription()"
          style="padding:13px;background:linear-gradient(135deg,#1e5030,#2d7a4a);border:none;border-radius:12px;color:#c8dfc4;font-family:'DM Sans',sans-serif;font-size:15px;font-weight:700;cursor:pointer;margin-top:4px">
          📨 Envoyer ma demande
        </button>
      </div>
    </div>`;
  document.body.appendChild(d);
  document.getElementById('ins-nom').focus();
}

async function submitInscription(){
  const nom=_fmtNom(document.getElementById('ins-nom').value||'');
  const prenom=_fmtPre(document.getElementById('ins-prenom').value||'');
  const mail=(document.getElementById('ins-mail').value||'').trim();
  const roleIns=document.getElementById('ins-role').value;
  const formateurNom=(document.getElementById('ins-formateur')?.value||'').trim();
  const err=document.getElementById('ins-err');
  if(!nom){ err.textContent='Nom requis'; return; }
  if(!prenom){ err.textContent='Prénom requis'; return; }
  if(!mail){ err.textContent='Email requis'; return; }
  try{
    loadSbConf();
    if(window.SUPA_URL && window.SUPA_KEY){
      const hdr={'apikey':window.SUPA_KEY,'Authorization':'Bearer '+window.SUPA_KEY,'Content-Type':'application/json','Prefer':'return=minimal'};
      const res=await fetch(window.SUPA_URL+'/rest/v1/floralab_requests',{method:'POST',headers:hdr,
        body:JSON.stringify({nom,prenom,mail,role:roleIns,formateur_nom:formateurNom||null,statut:'pending'})});
      if(!res.ok){ err.textContent='Erreur serveur, réessayez'; return; }
    }
    document.getElementById('inscription-overlay').remove();
    // Toast confirmation
    const t=document.createElement('div');
    t.style.cssText='position:fixed;top:20px;left:50%;transform:translateX(-50%);background:#1a4a2a;border:1px solid #4ab870;color:#c8dfc4;padding:14px 24px;border-radius:14px;font-size:14px;z-index:9999;text-align:center;box-shadow:0 4px 20px rgba(0,0,0,.5)';
    t.innerHTML='✅ Demande envoyée !<br><span style="font-size:12px;opacity:.7">L\'administrateur vous contactera par email avec votre code.</span>';
    document.body.appendChild(t);
...
async function resetPasswordsToDefault(){
  if(!confirm('Remettre les mots de passe par défaut ?\n\nNote : changez-les immédiatement après dans Admin > Accès.')) return;
...
function renderAdmin(){
  ...
  list.innerHTML = plantsSorted.map(p=>`
    <div class="admin-row" data-id="${p.id}" data-search="${(p.latin+' '+(p.nom||'')+' '+(p.famille||'')+' '+(p.type||'')+' '+(p.feuillage||'')+' '+(p.exposition||'')+' '+(p.usageAmenagement||'')).toLowerCase()}">
      ...
      <div class="admin-row-info">
        <div class="admin-row-latin" style="font-style:italic;font-family:'Playfair Display',serif">${italLatin(p.latin)}</div>
        <div class="admin-row-common">${p.nom||''}${p.famille?' — '+p.famille:''}</div>
      </div>
      <div class="admin-btns">
        <button class="edit-btn" style="background:rgba(74,184,112,.1);border-color:rgba(74,184,112,.3);color:var(--g4)" onclick="openModal(${p.id})">👁️</button>
...
// ── Toggle erreurs détaillées sur carte apprenti ──
function toggleStatErrors(id){
...
// ── Suppression d'un joueur multi (toutes ses participations) ──
async function deleteMultiPlayer(name){
  if(!confirm(`Supprimer toutes les participations multi de "${name}" ?\nAction irréversible.`)) return;
  try{
    if(cloudOk){
      await sbDelete('floralab_session_players','player_name=eq.'+encodeURIComponent(name));
    }
  }catch(e){ alert('Erreur lors de la suppression : '+e.message); return; }
  // Aussi effacer les erreurs localStorage
  try{
    let allErrors=JSON.parse(localStorage.getItem('chloroquiz_errors_v1')||'{}');
    delete allErrors[name];
    localStorage.setItem('chloroquiz_errors_v1',JSON.stringify(allErrors));
  }catch(e){}
  renderStats();
}
...
function filterAdminList(q){
  ...
  if(!q||!q.trim()){ rows.forEach(r=>r.style.display=''); return; }
  const matchFn = buildSearchEngine(q);
  // Associer chaque row à sa plante via data-id
  rows.forEach(r=>{
    const id = parseInt(r.dataset.id||r.dataset.plantId||'0');
    const p = plants.find(x=>x.id===id);
    if(p){
      r.style.display = matchFn(p) ? '' : 'none';
    } else {
      // Fallback: recherche textuelle simple sur data-search
      r.style.display = (r.dataset.search||'').includes(q.trim().toLowerCase()) ? '' : 'none';
    }
  });
}
    ...
    <div class="form-bg" onclick="if(event.target===this)document.getElementById('overlay-root').innerHTML=''">
      <div class="form-card" style="max-width:420px;padding:32px 28px">
        <div style="text-align:center;margin-bottom:22px">
          ...
          <div style="font-size:14px;font-style:italic;color:var(--g4);margin-bottom:4px">${italLatin(p.latin)}</div>
...
var FORM_GL={
  ...
  vivace_herb:{t:'Vivace herbacée',d:'Plante non ligneuse dont les parties aériennes meurent en hiver mais dont la souche (rhizome, bulbe, tubercule) survit et régénère chaque printemps.',e:'Hosta, Pennisetum, Salvia.'},
  disposition_feuilles:{t:'Disposition des feuilles',d:'Organisation des feuilles sur la tige. Caractère diagnostique important en identification botanique.'},
  rosette:{t:'Rosette',d:'Feuilles groupées à la base de la plante, étalées en cercle au ras du sol.',e:'Bellis perennis, Pittosporum tobira.'},
  alternes:{t:'Alternes',d:'Une seule feuille par nœud, disposées en alternance de part et d\'autre de la tige.',e:'Rosa, Prunus, Carpinus.'},
  opposees:{t:'Opposées',d:'Deux feuilles par nœud, face à face de chaque côté de la tige, dans le même plan.',e:'Viburnum, Syringa, Buddleja.'},
  opposees_decussees:{t:'Opposées décussées',d:'Feuilles opposées dont chaque paire est perpendiculaire à la précédente, formant 4 rangées verticales.',e:'Lavandula, Salvia, Thymus.'},
  verticillees:{t:'Verticillées',d:'Trois feuilles ou plus par nœud, disposées en couronne autour de la tige.',e:'Galium odoratum, Nerium oleander.'},
  decussees:{t:'Décussées',d:'Voir Opposées décussées. Chaque paire de feuilles tourne de 90° par rapport à la paire précédente.',e:'Thuja, Cupressus.'},
  helicoidale:{t:'Hélicoïdale',d:'Feuilles insérées en spirale autour de la tige, formant une hélice régulière. Angle de divergence constant (souvent 137,5°).',e:'Pinus, Picea, Abies.'},
  rubanee:{t:'Rubanée',d:'Longue, étroite et plate comme un ruban, plus large que linéaire. Bords parallèles sur une grande longueur.',e:'Cortaderia selloana, Iris, Agapanthus.'},
...
function openForm(id){
  ...
  // Photos
  // Filtrer les photos : seulement base64 ou Storage CDN (pas les vieilles URLs externes)
  var _storBase = (typeof sbConf!=='undefined'&&sbConf) ? sbConf.url+'/storage/v1/object/public/chloroquiz-photos/' : '';
  function _validPhoto(v){ return v && (v.startsWith('data:') || (_storBase && v.startsWith(_storBase))); }
  const photoHTML=[1,2,3,4].map(n=>{
    const sfx=n===1?'':String(n);
    const key='photo'+(n===1?'':String(n));
    const raw=p?p[key]||'':'';
    const val=_validPhoto(raw)?raw:''; // ← filtrer ici avant injection HTML
    const req=n===1?' *':'';
    return '<div>'
      ...
      +'<div class="photo-zone-text" id="photo-zone-text'+sfx+'">'+(val?'Cliquer pour changer':'Uploader une photo')+'</div>'
      +'<input type="file" id="f-photo-file'+sfx+'" accept="image/*" style="display:none" onchange="handlePhotoUpload(event,'+n+')"/>'
      +'</div>'
      // Champ URL supprimé — upload uniquement (photos depuis appareil)
      +'</div>';
  }).join('');
  ...
  document.getElementById('overlay-root').innerHTML=glossOv+
    ...
    +(!id
      ? '<div style="margin-bottom:12px">'
        +'<button onclick="openPlantnetModal()" style="width:100%;padding:11px 16px;background:linear-gradient(135deg,rgba(34,197,94,.15),rgba(22,163,74,.08));border:1px solid rgba(34,197,94,.35);border-radius:12px;color:#4ab870;font-size:14px;font-weight:700;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px">'
        +'<span style="font-size:18px">🔍</span> Identifier la plante avec Pl@ntNet'
        +'</button>'
        +'<div style="font-size:10px;color:rgba(200,223,204,.3);text-align:center;margin-top:4px">Powered by Pl@ntNet · Usage éducatif non commercial</div>'
        +'</div>'
      : '')
    +'<div class="form-actions" style="margin-bottom:14px">'
    +'<button class="cancel-btn" onclick="closeForm()">Annuler</button>'
    +'<button class="save-btn" onclick="savePlantForm()">✅ Enregistrer</button>'+'<button class="save-btn" id="save-draft-btn" onclick="savePlantAsDraft()" style="background:rgba(232,160,32,.15);border:1px solid rgba(232,160,32,.3);color:var(--am2)">📋 Brouillon</button>'
    +'</div>'
    +'<div class="form-grid">'
    ...
    +[['Annuelle','annuelle'],['Bisannuelle','annuelle'],['Vivace','vivace'],['Vivace herbacée','vivace_herb'],['Bulbeuse',''],['Graminée ornementale','graminee'],
    ...
    +'</div>'
    +'<div class="fg full"><label class="fl">Disposition des feuilles '+G('disposition_feuilles')+'</label>'
    +'<input type="hidden" id="f-dispositionFeuilles" value="'+v('dispositionFeuilles')+'"/>'
    +'<div class="type-chips" id="disposition-chips">'
    +[['Rosette','rosette'],['Alternes','alternes'],['Opposées','opposees'],['Opposées décussées','opposees_decussees'],['Verticillées','verticillees'],['Décussées','decussees'],['Hélicoïdale','helicoidale']
    ].map(function(item){
      return '<span class="type-chip '+cs1('dispositionFeuilles',item[0])+'" data-val="'+item[0]+'" onclick="setSFChip(this,\'f-dispositionFeuilles\',\'disposition-chips\',\'dispositionFeuilles\')">'+item[0]+' '+G(item[1])+'</span>';
    }).join('')+'</div>'
    +'<div class="type-display" id="dispositionFeuilles">'+(v('dispositionFeuilles')||'—')+'</div>'
    +'</div>'
    ...
    +[['Aciculaire','aciculaire'],['Linéaire','lineaire'],['Rubanée','rubanee'],['Lancéolé','lanceole'],['Oblancéolé','oblanceole'],['Ensiforme','ensiforme'],
    ...
    // PHOTOS & NOTES
    +'<div class="fg-section-title">📷 Photos & notes</div>'    +'<div class="fg full">'    +'<button type="button" onclick="document.getElementById(\'f-photos-multi\').click()" style="width:100%;padding:13px;background:rgba(74,184,112,.08);border:2px dashed rgba(74,184,112,.35);border-radius:14px;color:var(--g4);font-family:\'DM Sans\',sans-serif;font-size:14px;font-weight:700;cursor:pointer;margin-bottom:12px">'    +'📷 Sélectionner jusqu\'à 4 photos en une fois'    +'</button>'    +'<input type="file" id="f-photos-multi" accept="image/*" multiple style="display:none" onchange="handleMultiPhotoUpload(event)"/>'    +'<div style="font-size:11px;color:rgba(200,223,204,.35);margin-bottom:10px;text-align:center">Les photos se répartissent automatiquement · ou ajouter individuellement ci-dessous</div>'    +'<label class="fl">Photos (1 à 4) — compressées automatiquement en WebP</label>'    +'<div class="photo-grid" style="grid-template-columns:1fr 1fr">'+photoHTML+'</div></div>'
    ...
    +'<button class="save-btn" onclick="savePlantAsDraft()" style="background:rgba(232,160,32,.15);border:1px solid rgba(232,160,32,.3);color:var(--am2)">📋 Brouillon</button>'
    +'</div></div></div>';
  // Afficher les photos existantes dans les zones de preview à l'ouverture
  // Uniquement les vraies photos : base64 OU Storage CDN (pas les anciennes URLs cassées)
  setTimeout(function(){
    var pp = id ? (plants.find(function(x){return x.id===id;})||{}) : {};
    var storBase = (typeof sbConf!=='undefined'&&sbConf) ? sbConf.url+'/storage/v1/object/public/chloroquiz-photos/' : '';
    [['photo',''],['photo2','2'],['photo3','3'],['photo4','4']].forEach(function(pair){
      var src = pp[pair[0]]||'';
      if(!src) return;
      // Ignorer les URLs externes qui ne sont pas notre CDN (anciennes URLs Wikimedia etc.)
      var isB64     = src.startsWith('data:');
      var isCDN     = storBase && src.startsWith(storBase);
      var isValid   = isB64 || isCDN;
      if(!isValid){
        // Nettoyer silencieusement la valeur invalide
        console.log('[CQ] Photo ignorée (URL externe cassée) pour '+pp.latin+':', src.substring(0,60));
        return;
      }
      var sfx = pair[1];
      var zone = document.getElementById('photo-zone'+sfx);
      var icon = document.getElementById('photo-zone-icon'+sfx); if(icon) icon.remove();
      var prev = document.getElementById('photo-preview'+sfx);
      if(!prev && zone){ prev=document.createElement('img'); prev.id='photo-preview'+sfx; prev.className='photo-preview'; zone.insertBefore(prev,zone.firstChild); }
      if(prev){
        prev.onerror = function(){
          // Image impossible à charger → nettoyer la zone
          this.removeAttribute('src');
          this.style.display='none';
          if(zone) zone.classList.remove('has-photo');
          var txt2=document.getElementById('photo-zone-text'+pair[1]);
          if(txt2) txt2.textContent='Uploader une photo';
          // Effacer la valeur invalide en mémoire et dans Supabase
          var pid = id;
          var mp = plants.find(function(x){return x.id===pid;});
          if(mp){
            var key = pair[0];
            console.log('[CQ] Photo invalide effacée: '+mp.latin+' ('+key+') src='+src.substring(0,80));
            mp[key]='';
          }
        };
        // Forcer le rechargement en ajoutant un timestamp si CDN (évite le cache)
        if(isCDN) prev.src = src + '?t=' + Date.now();
        else prev.src = src;
      }
      if(zone) zone.classList.add('has-photo');
      var txt=document.getElementById('photo-zone-text'+sfx);
      if(txt) txt.textContent='Cliquer pour changer';
    });
  },80);
}
...
function handlePhotoUpload(e, n=1){
  ...
  reader.onload=ev=>{
    ...
    img.onload=()=>{
      ...
      // Afficher preview immédiatement en base64
      prev.src=b64;
      zone.classList.add('has-photo');
      const txt=document.getElementById('photo-zone-text'+sfx);
      if(txt){ const kb=Math.round(b64.length*0.75/1024); txt.textContent='⬆️ Upload…'; }
      // Stocker en base64 localement d'abord
      if(n===1) formPhotoB64=b64;
      else if(n===2) formPhoto2B64=b64;
      else if(n===3) formPhoto3B64=b64;
      else if(n===4) formPhoto4B64=b64;
      // Si cloud dispo → uploader vers Storage et remplacer par URL
      if(cloudOk){
        // Récupérer le latin de la plante en cours d'édition
        var latin = (document.getElementById('f-latin')||{}).value||'plante_'+Date.now();
        uploadPhotoToStorage(b64, latin, n).then(function(url){
          if(url && !url.startsWith('data:')){
            if(n===1) formPhotoB64=url;
            else if(n===2) formPhoto2B64=url;
            else if(n===3) formPhoto3B64=url;
            else if(n===4) formPhoto4B64=url;
            if(txt) txt.textContent='✅ Storage CDN';
          } else {
            const kb=Math.round(b64.length*0.75/1024);
            if(txt) txt.textContent='✅ '+kb+' Ko (local)';
          }
        });
      } else {
        const kb=Math.round(b64.length*0.75/1024);
...
// ── Sélection multiple de photos (jusqu'à 4 en une fois) ──
async function handleMultiPhotoUpload(e){
  const files = Array.from(e.target.files).slice(0,4); // max 4
  if(!files.length) return;
  // Trouver les slots libres (sans photo)
  const slots = [1,2,3,4].filter(n=>{
    const sfx = n===1?'':String(n);
    const urlVal = (document.getElementById('f-photo-url'+sfx)||{}).value||'';
    const prevEl = document.getElementById('photo-preview'+sfx);
    const b64s = [formPhotoB64,formPhoto2B64,formPhoto3B64,formPhoto4B64][n-1];
    return !urlVal && !b64s && !prevEl;
  });
  // Si tous les slots sont pleins, utiliser tous les slots dans l'ordre
  const targetSlots = slots.length >= files.length ? slots : [1,2,3,4];
  for(let i=0;i<files.length;i++){
    const n = targetSlots[i];
    if(!n) break;
    await compressAndSetPhoto(files[i], n);
  }
  // Réinitialiser l'input pour permettre une nouvelle sélection
  e.target.value = '';
}

// Compression d'une photo vers un slot donné (1-4)
function compressAndSetPhoto(file, n){
  return new Promise(resolve=>{
    ...
    reader.onload = ev=>{
      ...
      img.onload = ()=>{
        ...
        const icon = document.getElementById('photo-zone-icon'+sfx); if(icon) icon.remove();
        let prev = document.getElementById('photo-preview'+sfx);
        if(!prev){prev=document.createElement('img');prev.id='photo-preview'+sfx;prev.className='photo-preview';if(zone)zone.insertBefore(prev,zone.firstChild);}
        prev.src=b64;
        if(zone) zone.classList.add('has-photo');
        const txt = document.getElementById('photo-zone-text'+sfx);
        if(txt) txt.textContent='⬆️ Upload…';
        // Stocker base64 localement d'abord
        if(n===1) formPhotoB64=b64;
        else if(n===2) formPhoto2B64=b64;
        else if(n===3) formPhoto3B64=b64;
        else if(n===4) formPhoto4B64=b64;
        // Upload vers Storage si cloud dispo
        if(cloudOk){
          var lat2 = (document.getElementById('f-latin')||{}).value||'plante_'+Date.now();
          uploadPhotoToStorage(b64, lat2, n).then(function(url2){
            if(url2 && !url2.startsWith('data:')){
              if(n===1) formPhotoB64=url2;
              else if(n===2) formPhoto2B64=url2;
              else if(n===3) formPhoto3B64=url2;
              else if(n===4) formPhoto4B64=url2;
              if(txt) txt.textContent='✅ Storage CDN';
            } else {
              if(txt){ const kb2=Math.round(b64.length*0.75/1024); txt.textContent='✅ '+kb2+' Ko'; }
            }
          });
        } else {
          if(txt){ const kb=Math.round(b64.length*0.75/1024); txt.textContent='✅ '+kb+' Ko'; }
        }
        resolve();
      };
      img.src = ev.target.result;
...
// ══════════════════════════════════════════════════════
//  IMPORT / EXPORT CSV — Catalogue végétaux
// ══════════════════════════════════════════════════════

// Colonnes du modèle CSV (ordre fixe)
const CSV_COLUMNS = [
  {key:'latin',           label:'Nom botanique *',      hint:'Rosa canina  (Genre Espece  -  hybride: Rosa x canina  -  cultivar: Prunus avium Burlat)'},
  {key:'nom',             label:'Nom commun',            hint:'Rosier des haies'},
  {key:'famille',         label:'Famille botanique',     hint:'Rosaceae  (se termine en -aceae)'},
  {key:'classe',          label:'Classe',                hint:'Dicotylédone | Monocotylédone | Gymnosperme | Bryophyte'},
  {key:'type',            label:'Type végétal',          hint:'Arbuste | Arbre | Vivace | Annuelle | Conifère | Bambou | Fougère | Sous-arbrisseau | Arbrisseau | Rosier | Bulbeuse | Liane / Grimpante | Couvre-sol | Graminée ornementale | Arbre fruitier | Aquatique / Berge | Plante aromatique | Spontanée | Indigène | Horticole'},
  {key:'feuillage',       label:'Feuillage',             hint:'Caduc | Persistant | Marcescent | Semi-persistant'},
  {key:'rusticite',       label:'Rusticité',             hint:'-20 / -15°C  (valeur unique ou plage: -15 / -10°C)'},
  {key:'exposition',      label:'Exposition',            hint:'Soleil | Ombre | Mi-ombre | Ombre – Mi-ombre | Mi-ombre – Soleil | Ombre – Mi-ombre – Soleil'},
  {key:'hauteurAdulte',   label:'Hauteur adulte',        hint:'1 m – 3 m  (utiliser des valeurs TAILLES: 0.1 à 0.9 par 0.1 / 1 à 40 par 1 / ex: 0.3 m – 0.6 m  ou  2 m – 4 m)'},
  {key:'largeurAdulte',   label:'Largeur adulte',        hint:'1 m – 2 m  (même format que hauteur)'},
  {key:'port',            label:'Port',                  hint:'Érigé | Étalé / Rampant | Fastigié / Colonnaire | Arrondi | Pleureur | Buissonnant'},
  {key:'vitesseCroissance',label:'Vitesse croissance',   hint:'Lente | Moyenne | Rapide | Très rapide'},
  {key:'ph',              label:'pH du sol',             hint:'Acidophile (4 – 5,5) | Légèrement acide (5,5 – 6,5) | Neutre (6,5 – 7,2) | Légèrement basique (7,2 – 7,8) | Calcicole (> 7,8) | Indifférent  (multi: séparer par virgule)'},
  {key:'humidite',        label:'Humidité sol',          hint:'Sec | Frais | Humide | Détrempé / Aquatique  (multi: séparer par virgule)'},
  {key:'structureSol',    label:'Structure sol',         hint:'Drainant | Équilibré | Lourd / Argileux | Indifférent  (multi: séparer par virgule)'},
  {key:'periodeFloraison',label:'Période floraison',     hint:'Mois séparés par  –  (tiret long): Janvier | Février | Mars | Avril | Mai | Juin | Juillet | Août | Septembre | Octobre | Novembre | Décembre  — ex: Mai – Juillet  ou  Mars – Avril'},
  {key:'couleurFleurs',   label:'Couleur fleurs',        hint:'Rouge | Blanc | Rose | Jaune | Violet | Bleu | Orange | Rose pâle | etc. (multi: séparer par virgule)'},
  {key:'typeInflorescence',label:'Type inflorescence',   hint:'Grappe | Ombelle | Épi | Capitule | Panicule | Corymbe | Chaton | Cyme | Glomérule'},
  {key:'parfum',          label:'Parfum',                hint:'Oui | Non'},
  {key:'interetOrnemental',label:'Intérêt ornemental',   hint:'Floraison décorative | Feuillage persistant | Feuillage décoratif | Port architectural | Fruits & baies décoratifs | Floraison parfumée | Silhouette hivernale | Feuillage automnal  (multi: virgule)'},
  {key:'autresInterets',  label:'Autres intérêts',       hint:'Médicinale | Feuillage aromatique | Comestible / Fruitière | Fixatrice d\'azote | Résistante à la pollution  (multi: virgule)'},
  {key:'usageAmenagement',label:'Usage aménagement',     hint:'Haie libre | Haie taillée | Haie champêtre | Massif | Bordure | Couvre-sol | Talus | Rocaille | Bac / Jardinière | Plante isolée (sujet) | Sous-bois | Brise-vent | Grimpante | Prairie fleurie | Alignement | Arbre isolé  (multi: virgule)'},
  {key:'biodiversite',    label:'Biodiversité',          hint:'Mellifère | Plante hôte insectes | Nectarifère | Gîte hivernal | Baies pour oiseaux  (multi: virgule)'},
  {key:'couleurLimbe',    label:'Couleur limbe',         hint:'Vert foncé | Vert moyen | Vert clair | Vert glauque | Vert bleuté | Gris-vert | Argenté | Pourpre | Panaché crème'},
  {key:'formeLimbe',      label:'Forme limbe',           hint:'Ovale | Lancéolé | Elliptique | Cordiforme | Lobé | Pennatiséqué | Linéaire | Palmé | Hastiforme'},
  {key:'texture',         label:'Texture feuillage',     hint:'Coriace | Soyeux | Rugueux | Lisse | Glabre | Tomenteux | Pubescent'},
  {key:'reproduction',    label:'Reproduction',          hint:'Hermaphrodite | Monoïque | Dioïque | Polygame'},
  {key:'pollinisation',   label:'Pollinisation',         hint:'Entomogame | Anémogame | Ornithogame | Autogame'},
  {key:'typeTaille',      label:'Type taille',           hint:'Aucune (port naturel) | Taille de formation | Taille de maintien | Recépage | Haie / Topiaire  (multi: virgule)'},
  ...
  {key:'photo4',          label:'URL Photo 4',           hint:''},
];

function downloadCSVTemplate(){
  const esc = v => '"' + String(v||'').replace(/"/g,'""') + '"';
  const header = CSV_COLUMNS.map(col=>esc(col.label)).join(';');
  const hints  = CSV_COLUMNS.map(col=>esc(col.hint)).join(';');
  const rows   = plants.length
    ? plants.map(p=>CSV_COLUMNS.map(col=>esc(p[col.key]||'')).join(';')).join('\n')
    : '';
  const content = [header, hints, rows].filter(Boolean).join('\n');

  // Encoder en UTF-8 avec BOM (EF BB BF) intégré dans les bytes bruts
  // Le BOM dans les bytes (pas en texte) est reconnu par Excel, WPS, Google Sheets
  const encoder = new TextEncoder();
  const contentBytes = encoder.encode(content);
  const bom = new Uint8Array([0xEF, 0xBB, 0xBF]);
  const withBom = new Uint8Array(bom.length + contentBytes.length);
  withBom.set(bom, 0);
  withBom.set(contentBytes, bom.length);

  const blob = new Blob([withBom], {type:'text/csv;charset=utf-8'});
  const url  = URL.createObjectURL(blob);
  const a    = document.createElement('a');
  a.href=url; a.download='chloroquiz_catalogue.csv'; a.click();
...
function triggerCSVImport(){
  ...
  input.onchange = e => parseCSVFile(e.target.files[0]);
  input.click();
}

// ── Normalisation des champs CSV à format contrôlé ──
function normalizeCSVPlant(p){
  const MOIS_FULL=['Janvier','Février','Mars','Avril','Mai','Juin','Juillet','Août','Septembre','Octobre','Novembre','Décembre'];

  // Période de floraison : accepte "Mai, Juin, Juillet" ou "mai-juillet" → "Mai – Juin – Juillet"
  if(p.periodeFloraison){
    const raw=p.periodeFloraison.trim();
    // Séparer sur , ou - ou – ou /
    const parts=raw.split(/\s*[,;\/\-–]\s*/).map(m=>m.trim()).filter(Boolean);
    const valid=parts.map(m=>{
      const cap=m.charAt(0).toUpperCase()+m.slice(1).toLowerCase();
      return MOIS_FULL.find(mo=>mo.toLowerCase()===cap.toLowerCase()||mo.toLowerCase().startsWith(cap.toLowerCase().slice(0,4)))||null;
    }).filter(Boolean);
    if(valid.length){
      // Trier par ordre calendaire
      valid.sort((a,b)=>MOIS_FULL.indexOf(a)-MOIS_FULL.indexOf(b));
      p.periodeFloraison=valid.join(' – ');
    }
  }

  // Hauteur/Largeur adulte : normaliser le tiret en ' – ' et les unités
  function normDim(val){
    if(!val) return val;
    val=val.trim().replace(/,/g,'.');
    // Convertir cm → m si valeur > 100 cm ou si unité cm présente
    const isCm=/cm/i.test(val);
    // Format plage : "1-3 m" ou "1 – 3m" ou "100-300 cm"
    const rangeM=val.match(/([\d.]+)\s*[–\-]\s*([\d.]+)\s*m?/i);
    if(rangeM){
      let lo=parseFloat(rangeM[1]), hi=parseFloat(rangeM[2]);
      if(isCm){ lo=lo/100; hi=hi/100; }
      // Arrondir au palier TAILLES le plus proche
      function snapTaille(v){
        if(v<0.1) return '<0.1 m';
        if(v>40) return '>40 m';
        if(v<1) return (Math.round(v*10)/10).toFixed(1)+' m';
        return Math.round(v)+' m';
      }
      return snapTaille(lo)+' – '+snapTaille(hi);
    }
    // Valeur unique
    const singleM=val.match(/([\d.]+)\s*m?/i);
    if(singleM){
      let v=parseFloat(singleM[1]);
      if(isCm) v=v/100;
      if(v<0.1) return '<0.1 m';
      if(v>40) return '>40 m';
      if(v<1) return (Math.round(v*10)/10).toFixed(1)+' m';
  ...
  if(p.hauteurAdulte) p.hauteurAdulte=normDim(p.hauteurAdulte);
  if(p.largeurAdulte) p.largeurAdulte=normDim(p.largeurAdulte);

  // Rusticité : normaliser -20/-15°C → "-20 / -15°C"
  if(p.rusticite){
    const rv=p.rusticite.trim().replace(/\s+/g,' ');
    // Format "< -30°C" ou "<-30°C"
    if(/^</.test(rv)){ p.rusticite='< -30°C'; }
    else {
      const rm=rv.match(/([-\d]+)\s*[\/\-–,]\s*([-\d]+)\s*°?C?/i);
      if(rm){
        let a=parseInt(rm[1]), b=parseInt(rm[2]);
        if(a>b){const t=a;a=b;b=t;} // lo < hi (négatifs → lo est plus négatif)
        if(b>a) p.rusticite=a+' / '+b+'°C';
        else p.rusticite=a+'°C';
      } else {
        // Valeur unique "-15°C"
        const rs=rv.match(/([-\d]+)/);
        if(rs) p.rusticite=rs[1]+'°C';
      }
    }
  }

  // Feuillage : capitaliser
  if(p.feuillage){
    const fmap={'caduc':'Caduc','persistant':'Persistant','marcescent':'Marcescent','semi-persistant':'Semi-persistant','semi persistant':'Semi-persistant'};
    const flo=p.feuillage.trim().toLowerCase();
    p.feuillage=fmap[flo]||(p.feuillage.charAt(0).toUpperCase()+p.feuillage.slice(1).toLowerCase());
  }

  // Exposition : capitaliser première lettre, tirets longs
  if(p.exposition){
    p.exposition=p.exposition.trim()
      .replace(/\s*-\s*/g,' – ')  // tirets courts → longs
...
function parseCSVFile(file){
  ...
  reader.onload = function(e){
    try{
      ...
      // Ligne 1 = headers, ignorer ligne 2 (hints) si elle ressemble à des exemples
      const headers = parseLine(lines[0]).map(h=>h.replace(/^"+|"+$/g,''));
      // Mapper les headers vers les clés CSV_COLUMNS
      const keyMap = {};
      headers.forEach((h,i)=>{
        const col = CSV_COLUMNS.find(c=>c.label.toLowerCase()===h.toLowerCase()||c.key.toLowerCase()===h.toLowerCase());
        if(col) keyMap[i]=col.key;
      });

      // Détecter si ligne 2 est une ligne de hints (contient 'Rosaceae', '|' etc.)
      let dataStart = 1;
      if(lines.length>2){
        const l2=parseLine(lines[1]);
        const looksLikeHint = l2.some(v=>v.includes('|')||v==='*'||v.toLowerCase().includes('ex :')||v.toLowerCase().includes('hint'));
        if(looksLikeHint) dataStart=2;
      }
      ...
      for(let i=dataStart;i<lines.length;i++){
        ...
        plant.latin=normalizeLatin(plant.latin.trim());
        if(plant.famille) plant.famille=normalizeFamille(plant.famille);
        // Normaliser les champs à format contrôlé
        plant=normalizeCSVPlant(plant);
        imported.push(plant);
      }

      if(!imported.length){ alert('❌ Aucune ligne valide trouvée (colonne "Nom botanique *" obligatoire).'); return; }
...
function showCSVPreview(imported){
  // Catégoriser : nouveau / doublon
  const latinSet = new Set(plants.map(p=>(p.latin||'').toLowerCase()));
  const rows = imported.map(p=>({
    plant:p,
    status: latinSet.has((p.latin||'').toLowerCase()) ? 'doublon' : 'nouveau'
  }));

  const nouveaux = rows.filter(r=>r.status==='nouveau').length;
  const doublons = rows.filter(r=>r.status==='doublon').length;

  const tableRows = rows.map((r,i)=>`
    <tr id="csv-row-${i}" style="border-bottom:1px solid rgba(74,184,112,.08)">
      <td style="padding:6px 8px;font-style:italic;font-size:12px;color:var(--cr)">${r.plant.latin}</td>
      <td style="padding:6px 8px;font-size:12px;color:rgba(200,223,204,.6)">${r.plant.nom||'—'}</td>
      <td style="padding:6px 8px;font-size:12px;color:rgba(200,223,204,.5)">${r.plant.famille||'—'}</td>
      <td style="padding:6px 8px;text-align:center">
        ${r.status==='nouveau'
          ? '<span style="background:rgba(74,184,112,.15);color:var(--g4);border:1px solid rgba(74,184,112,.3);border-radius:20px;padding:2px 10px;font-size:10px;font-weight:700">✨ Nouveau</span>'
          : '<select id="csv-action-'+i+'" style="background:rgba(19,48,29,.8);border:1px solid rgba(232,160,32,.3);border-radius:8px;color:var(--am2);font-size:11px;padding:3px 8px;font-family:\'DM Sans\',sans-serif"><option value="update">✏️ Mettre à jour</option><option value="skip">⏭ Ignorer</option></select>'}
      </td>
    </tr>`).join('');
  ...
  document.getElementById('overlay-root').innerHTML=`
    <div class="modal-bg" onclick="if(event.target===this)closeModal()">
      <div class="modal" style="max-width:680px">
        <div style="padding:20px 52px 12px 20px;position:relative">
          <button class="modal-close" onclick="closeModal()">✕</button>
          <div style="font-family:var(--disp);font-size:20px;font-weight:900;color:var(--cr)">📤 Import CSV — Prévisualisation</div>
          <div style="font-size:13px;color:rgba(200,223,204,.5);margin-top:4px">
            ${imported.length} ligne(s) détectée(s) &nbsp;·&nbsp;
            <span style="color:var(--g4)">✨ ${nouveaux} nouvelle(s)</span> &nbsp;·&nbsp;
            <span style="color:var(--am2)">⚠️ ${doublons} doublon(s)</span>
          </div>
        </div>
        <div style="overflow-y:auto;max-height:50vh;padding:0 20px">
          <table style="width:100%;border-collapse:collapse">
            <thead><tr style="font-size:10px;text-transform:uppercase;letter-spacing:1px;color:rgba(200,223,204,.35)">
              <th style="padding:6px 8px;text-align:left">Nom botanique</th>
              <th style="padding:6px 8px;text-align:left">Nom commun</th>
        ...
        <div style="padding:16px 20px;display:flex;gap:10px;justify-content:flex-end;border-top:1px solid rgba(74,184,112,.1)">
          ...
          <button onclick="confirmCSVImport(window._csvImportRows)" style="padding:10px 22px;background:linear-gradient(135deg,#1e5030,#2d7a4a);border:none;border-radius:10px;color:white;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:700;cursor:pointer">✅ Valider l'import (${nouveaux} nouveau${nouveaux>1?'x':''} + ${doublons} doublon${doublons>1?'s':''})</button>
        </div>
      </div>
    </div>`;
}
...
async function confirmCSVImport(rows){
  // ⚠️ Lire les selects AVANT closeModal (closeModal supprime le DOM)
  const toProcess = rows.map((r,i)=>{
    if(r.status==='nouveau') return {plant:r.plant, action:'add'};
    const sel = document.getElementById('csv-action-'+i);
    const action = sel ? sel.value : 'skip'; // lit le select pendant qu'il existe encore
    return {plant:r.plant, action};
  }).filter(r=>r.action!=='skip');

  closeModal(); // maintenant on peut fermer
  if(!toProcess.length){ alert('Aucune opération sélectionnée.\nPour les doublons, choisissez "✏️ Mettre à jour" dans la colonne Action.'); return; }

  // ── Afficher un indicateur de progression ──
  var progDiv = document.createElement('div');
  progDiv.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:#0d2b1a;border:1px solid rgba(74,184,112,.3);border-radius:14px;padding:24px 32px;z-index:99999;color:#4ab870;font-size:15px;font-weight:700;text-align:center';
  progDiv.innerHTML = '⏳ Import en cours…';
  document.body.appendChild(progDiv);

  var added = 0, updated = 0;

  // ── Calculer le maxId UNE SEULE FOIS avant la boucle ──
  var baseId = plants.reduce(function(m,p){ return Math.max(m, Number(p.id)||0); }, 0);

  for(var ti=0; ti<toProcess.length; ti++){
    var action = toProcess[ti].action;
    var plant  = toProcess[ti].plant;
    if(action==='add'){
      baseId++;
      plant.id = baseId;
      plants.push(plant);
      ...
      var idx = plants.findIndex(function(p){ return (p.latin||'').toLowerCase()===(plant.latin||'').toLowerCase(); });
      if(idx>=0){
        plant.id = plants[idx].id; // conserver l'ID existant
        // Conserver les photos existantes si le CSV n'en fournit pas
        if(!plant.photo)  plant.photo  = plants[idx].photo  || '';
        if(!plant.photo2) plant.photo2 = plants[idx].photo2 || '';
        if(!plant.photo3) plant.photo3 = plants[idx].photo3 || '';
        if(!plant.photo4) plant.photo4 = plants[idx].photo4 || '';
        plants[idx] = plant;
        updated++;
      }
    }
  ...
  progDiv.innerHTML = '💾 Sauvegarde en cours… (' + plants.length + ' végétaux)';

  // ── Sauvegarde : local immédiatement, puis cloud ──
  localSavePlants();
  var cloudSaved = false;
  if(cloudOk){
    try{
      // Données texte sans photos (évite les timeouts)
      const plantsNoPhotos = plants.map(p=>({...p,photo:'',photo2:'',photo3:'',photo4:''}));
      await sbPost('floralab_data',{key:'plants',value:plantsNoPhotos,updated_at:new Date().toISOString()},'resolution=merge-duplicates');
      // Photos séparées dans floralab_photos
      await savePhotosCloud();
      cloudSaved = true;
      flashSaved();
    } catch(e){
      cqErr('CSV cloud save error:', e);
    }
  }

  document.body.removeChild(progDiv);

  // ── Recharger depuis la source pour afficher l'état réel ──
  if(cloudOk && cloudSaved){
    try{ await loadPlants(true); } catch(e){} // forcer rechargement après import
  }
  renderAdmin();

  var msg = '✅ Import terminé !\n✨ ' + added + ' ajoutée(s)   ✏️ ' + updated + ' mise(s) à jour\n📦 Catalogue total : ' + plants.length + ' végétaux';
  if(cloudOk && !cloudSaved) msg += '\n⚠️ Sauvegarde locale uniquement (vérifier connexion cloud)';
  alert(msg);
}


// ── Extract plant data from form (shared by admin + myplant modes) ──
function buildPlantFromForm(){
  function v(id){ return (document.getElementById('f-'+id)||{}).value||''; }
  const type_chips=Array.from(document.querySelectorAll('#type-chips-grid .type-chip.selected')).map(c=>c.dataset.val||c.textContent.trim()).filter(Boolean);
  return {
    nom:v('nom').trim(),
    famille:normalizeFamille(v('famille')),
    type:type_chips.join(','),
    feuillage:v('feuillage'),
    rusticite:v('rusticite'),
    exposition:v('exposition'),
    ph:v('ph'),
    resistanceSech:(document.getElementById('f-resistanceSech')||{}).value||'',
    hauteurAdulte:v('hauteurAdulte'),
    largeurAdulte:v('largeurAdulte'),
    periodeFloraison:v('periodeFloraison').trim(),
    couleurFleurs:v('couleurFleurs'),
    interetOrnemental:v('interetOrnemental'),
    autresInterets:v('autresInterets'),
    usageAmenagement:v('usageAmenagement'),
    ...
    dispositionFeuilles:(document.getElementById('f-dispositionFeuilles')||{}).value||'',
    ...
    photo:(()=>{
      if(formPhotoB64) return formPhotoB64;
      const prev=document.getElementById('photo-preview');
      var s=prev&&prev.src; var sb=(typeof sbConf!=='undefined'&&sbConf)?sbConf.url+'/storage/v1/object/public/chloroquiz-photos/':'';
      return (s&&(s.startsWith('data:')||(sb&&s.startsWith(sb))))?s:'';
    })(),
    photo2:(()=>{
      if(formPhoto2B64) return formPhoto2B64;
      const prev=document.getElementById('photo-preview2');
      var s=prev&&prev.src; var sb=(typeof sbConf!=='undefined'&&sbConf)?sbConf.url+'/storage/v1/object/public/chloroquiz-photos/':'';
      return (s&&(s.startsWith('data:')||(sb&&s.startsWith(sb))))?s:'';
    })(),
    photo3:(()=>{
      if(formPhoto3B64) return formPhoto3B64;
      const prev=document.getElementById('photo-preview3');
      var s=prev&&prev.src; var sb=(typeof sbConf!=='undefined'&&sbConf)?sbConf.url+'/storage/v1/object/public/chloroquiz-photos/':'';
      return (s&&(s.startsWith('data:')||(sb&&s.startsWith(sb))))?s:'';
    })(),
    photo4:(()=>{
      if(formPhoto4B64) return formPhoto4B64;
      const prev=document.getElementById('photo-preview4');
      var s=prev&&prev.src; var sb=(typeof sbConf!=='undefined'&&sbConf)?sbConf.url+'/storage/v1/object/public/chloroquiz-photos/':'';
      return (s&&(s.startsWith('data:')||(sb&&s.startsWith(sb))))?s:'';
    })(),
  };
...
// ── Enregistrer comme brouillon (sans Supabase) ──
function savePlantAsDraft(){
  const latin=normalizeLatin((document.getElementById('f-latin')||{}).value||'');
  if(!latin){ alert('Le nom botanique est obligatoire pour créer un brouillon.'); return; }
  const plant=buildPlantFromForm();
  plant.latin=latin;
  if(editingId && draftPlants.some(p=>p.id===editingId)){
    const idx=draftPlants.findIndex(p=>p.id===editingId);
    draftPlants[idx]={...draftPlants[idx],...plant};
  } else {
    plant.id=nextDraftId();
    draftPlants.push(plant);
  }
  saveDrafts();
  closeForm();
  renderAdmin();
  showToast('📋 Brouillon enregistré — '+latin);
}

// ── Publier un brouillon vers Supabase ──
async function publishDraft(draftId){
  const dp=draftPlants.find(p=>p.id===draftId); if(!dp) return;
  if(!confirm('Publier "'+dp.latin+'" dans le catalogue principal ?')) return;
  try{
    // Assigner un vrai ID
    const maxId=plants.reduce((m,p)=>Math.max(m,p.id||0),0);
    const plant={...dp, id:maxId+1};
    plants.push(plant);
    await savePlants();
    draftPlants=draftPlants.filter(p=>p.id!==draftId);
    saveDrafts();
    renderAdmin();
    showToast('✅ '+dp.latin+' publié dans le catalogue !');
...
function deleteDraft(draftId){
  const dp=draftPlants.find(p=>p.id===draftId); if(!dp) return;
  if(!confirm('Supprimer le brouillon "'+dp.latin+'" ?')) return;
  draftPlants=draftPlants.filter(p=>p.id!==draftId);
  saveDrafts();
  renderAdmin();
}
...
function editDraft(draftId){
  ...
  editingId=draftId;
  _myPlantMode=false;
  // Inject temporarily into plants for openForm to find
  const wasIn=plants.some(p=>p.id===dp.id);
  if(!wasIn) plants.push(dp);
  openForm(dp.id);
  if(!wasIn) plants=plants.filter(p=>p.id!==dp.id);
  // After form opens, intercept save to update draft instead of plants
  // → patch savePlantForm to detect draftId range (>= 90000)
}

async function savePlantForm(){
  // Mode brouillon admin : mise à jour du brouillon (pas Supabase)
  if(!_myPlantMode && editingId && editingId>=90000){
    const latin=normalizeLatin((document.getElementById('f-latin')||{}).value||'');
    if(!latin){ alert('Le nom botanique est obligatoire.'); return; }
    const plant=buildPlantFromForm(); plant.latin=latin; plant.id=editingId;
    const idx=draftPlants.findIndex(p=>p.id===editingId);
    if(idx>=0) draftPlants[idx]={...draftPlants[idx],...plant};
    else draftPlants.push(plant);
    saveDrafts(); editingId=null; closeForm(); renderAdmin();
    showToast('📋 Brouillon mis à jour — '+latin);
    return;
  }
  // Mode Mes végétaux : sauvegarder localement au lieu du cloud
  if(_myPlantMode){
    const latin=normalizeLatin((document.getElementById('f-latin')||{}).value||'');
    if(!latin){ alert('Le nom botanique est obligatoire.'); return; }
    const plant=buildPlantFromForm();
    plant.latin=latin;
    if(editingId!==null && myPlants.some(p=>p.id===editingId)){
      // Mise à jour de la plante existante
      const idx=myPlants.findIndex(p=>p.id===editingId);
      plant.id=editingId;
      myPlants[idx]={...myPlants[idx],...plant};
    } else {
      plant.id=nextMyPlantId();
      myPlants.push(plant);
    }
    saveMyPlants();
    _myPlantMode=false;
  ...
  // Lecture des photos : nouvelle upload → preview CDN/b64 → existante valide en base
  const _sb = (typeof sbConf!=='undefined'&&sbConf)?sbConf.url+'/storage/v1/object/public/chloroquiz-photos/':'';
  const _ps = function(s){
    const e=document.getElementById('photo-preview'+s);
    const src=e&&e.src;
    return (src&&(src.startsWith('data:')||(_sb&&src.startsWith(_sb))))?src:'';
  };
  const existing  = (editingId ? (plants.find(x=>x.id==editingId)||{}) : {});
  // Pour existing, ne garder que les vraies valeurs (CDN ou base64)
  const _ev = function(v){ return (v&&(v.startsWith('data:')||(_sb&&v.startsWith(_sb))))?v:''; };
  const photo     = formPhotoB64  || _ps('')  || _ev(existing.photo)  || '';
  const photo2    = formPhoto2B64 || _ps('2') || _ev(existing.photo2) || '';
  const photo3    = formPhoto3B64 || _ps('3') || _ev(existing.photo3) || '';
  const photo4    = formPhoto4B64 || _ps('4') || _ev(existing.photo4) || '';
...
function closeForm(){
  _myPlantMode=false; document.getElementById('overlay-root').innerHTML=''; editingId=null; formPhotoB64=null; formPhoto2B64=null; }
...
// ══════════════════════════════════════════════════════
//  SESSION ENTRE APPRENTIS (peer-to-peer)
// ══════════════════════════════════════════════════════
var peerSessionCode = null;
var peerPollInterval = null;

async function openPeerSessionModal(){
  if(!cloudOk){ alert('Connexion internet requise.'); return; }
  const root = document.getElementById('overlay-root');
  root.innerHTML = `
    <div class="form-bg" onclick="if(event.target===this)this.innerHTML=''">
      <div class="form-card" style="max-width:420px">
        <div class="form-title">🎮 Jouer entre apprentis</div>
        <div style="font-size:13px;color:rgba(200,223,204,.5);margin-bottom:20px;text-align:center">
          Créez une partie privée ou rejoignez celle d'un ami
        </div>
        <div style="display:flex;gap:10px;flex-direction:column">
          <button class="save-btn" onclick="createPeerSession()" style="padding:16px;font-size:16px">
            🚀 Créer une partie
          </button>
          <div style="text-align:center;color:rgba(200,223,204,.3);font-size:12px">— ou —</div>
          <div style="display:flex;gap:8px">
            <input class="fi" id="peer-join-code" maxlength="4" placeholder="Code 4 chiffres"
              style="text-align:center;font-size:22px;font-weight:700;letter-spacing:4px"
              oninput="this.value=this.value.replace(/[^0-9]/g,'')"/>
            <button class="save-btn" onclick="joinPeerSession()" style="padding:12px 20px;white-space:nowrap">Rejoindre →</button>
          </div>
        </div>
        <button class="cancel-btn" onclick="document.getElementById('overlay-root').innerHTML=''" style="width:100%;margin-top:12px">Annuler</button>
      </div>
    </div>`;
  setTimeout(()=>{ const el=document.getElementById('peer-join-code'); if(el)el.focus(); },100);
}

async function createPeerSession(){
  document.getElementById('overlay-root').innerHTML='';
  if(!cloudOk){ alert('Connexion requise.'); return; }
  // Demander nom
  const root = document.getElementById('overlay-root');
  root.innerHTML = `
    <div class="form-bg" onclick="if(event.target===this)this.innerHTML=''">
      <div class="form-card" style="max-width:360px">
        <div class="form-title">🚀 Créer une partie</div>
        <div style="margin-bottom:12px">
          <label class="fl">Ton prénom</label>
          <input class="fi" id="peer-name-input" placeholder="Prénom…" maxlength="20" autocomplete="off"/>
        </div>
        <div style="margin-bottom:16px">
          <label class="fl">2 premières lettres du nom</label>
          <input class="fi" id="peer-init-input" placeholder="AB" maxlength="2" style="text-transform:uppercase"/>
        </div>
        <div style="margin-bottom:16px">
          <label class="fl">Niveau</label>
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            ${[0,1,2,3,4].map(n=>{
              const lbl={0:'👀 Découverte',1:'🌱 Débutant',2:'🎓 Intermédiaire',3:'🔬 Expert',4:'🏗️ Technique'}[n];
              return '<button type="button" class="peer-lv-btn" data-lv="'+n+'" onclick="selectPeerLevel('+n+')" style="flex:1;min-width:80px;padding:8px;border:1px solid rgba(74,184,112,.3);border-radius:10px;background:rgba(74,184,112,.06);color:rgba(200,223,204,.7);font-size:11px;cursor:pointer;transition:all .15s">'+lbl+'</button>';
            }).join('')}
          </div>
          <input type="hidden" id="peer-level-val" value="1"/>
        </div>
        <div style="display:flex;gap:10px">
          <button class="cancel-btn" onclick="openPeerSessionModal()">← Retour</button>
          <button class="save-btn" onclick="confirmCreatePeerSession()">Créer ▶</button>
        </div>
      </div>
    </div>`;
  setTimeout(()=>{
    selectPeerLevel(1);
    const el=document.getElementById('peer-name-input'); if(el)el.focus();
  },100);
}

function selectPeerLevel(n){
  document.querySelectorAll('.peer-lv-btn').forEach(function(b){
    var sel = parseInt(b.dataset.lv)===n;
    b.style.background = sel?'rgba(74,184,112,.25)':'rgba(74,184,112,.06)';
    b.style.borderColor = sel?'var(--g4)':'rgba(74,184,112,.3)';
    b.style.color = sel?'var(--g4)':'rgba(200,223,204,.7)';
    b.style.fontWeight = sel?'700':'400';
  });
  var hid=document.getElementById('peer-level-val'); if(hid) hid.value=n;
}

async function confirmCreatePeerSession(){
  const nameEl=document.getElementById('peer-name-input');
  const initEl=document.getElementById('peer-init-input');
  const levelEl=document.getElementById('peer-level-val');
  const name=(nameEl&&nameEl.value||'').trim();
  const init=(initEl&&initEl.value||'').toUpperCase().replace(/[^A-Z]/g,'');
  const level=parseInt(levelEl&&levelEl.value||'1');
  if(!name){ if(nameEl)nameEl.style.borderColor='#ff7070'; return; }
  if(init.length<2){ if(initEl)initEl.style.borderColor='#ff7070'; return; }
  const fullName=name.charAt(0).toUpperCase()+name.slice(1).toLowerCase()+' '+init+'.';
  document.getElementById('overlay-root').innerHTML='';

  // Générer code et questions
  var code = String(Math.floor(1000+Math.random()*9000));
  var pool = plants.filter(function(p){ return p.photo||p.photo2||p.photo3||p.photo4; });
  if(pool.length<4){ alert('Il faut au moins 4 plantes avec photos pour jouer.'); return; }
  var qs = generateSessionQuestions(pool, level, 20);
  // Strip base64 photos
  var qsLight = qs.map(function(q){
    return Object.assign({},q,{
      plantPhoto:  (q.plantPhoto  &&!q.plantPhoto.startsWith('data:'))  ? q.plantPhoto  : '',
      plantPhoto2: (q.plantPhoto2 &&!q.plantPhoto2.startsWith('data:')) ? q.plantPhoto2 : '',
      plantPhoto3: (q.plantPhoto3 &&!q.plantPhoto3.startsWith('data:')) ? q.plantPhoto3 : '',
      plantPhoto4: (q.plantPhoto4 &&!q.plantPhoto4.startsWith('data:')) ? q.plantPhoto4 : '',
    });
  });

  try{
    // Créer la session avec peer:true pour la cacher aux stats admin
    await sbPost('floralab_sessions',{
      code:code, level:level, questions:qsLight,
      host:fullName, status:'waiting',
      created_at:new Date().toISOString(),
      peer:true  // flag pour masquer des stats admin
    },'resolution=merge-duplicates');
    // S'enregistrer comme 1er joueur
    await sbPost('floralab_session_players',{
      session_code:code, player_name:fullName,
      score:0, total:0, current_q:0, done:false
    },'on_conflict=player_name,session_code&resolution=ignore-duplicates');
  } catch(e){ alert('Erreur : '+e.message); return; }

  peerSessionCode = code;
  showPeerLobby(code, fullName, qs, level, true);
}

async function joinPeerSession(){
  const codeEl=document.getElementById('peer-join-code');
  const code=(codeEl&&codeEl.value||'').trim();
  if(code.length!==4){ if(codeEl)codeEl.style.borderColor='#ff7070'; return; }
  document.getElementById('overlay-root').innerHTML='';

  try{
    const rows=await sbGet('floralab_sessions','code=eq.'+code+'&select=*');
    if(!rows.length){ alert('Code invalide.'); openPeerSessionModal(); return; }
    const session=rows[0];
    if(session.status==='ended'){ alert('Cette partie est terminee.'); return; }
    ...
    root.innerHTML=`
      <div class="form-bg" onclick="if(event.target===this)this.innerHTML=''">
        <div class="form-card" style="max-width:360px">
          <div class="form-title">🎮 Rejoindre — Code ${code}</div>
          <div style="margin-bottom:12px">
            <label class="fl">Ton prénom</label>
            <input class="fi" id="peer-join-name" placeholder="Prénom…" maxlength="20" autocomplete="off"/>
          </div>
          <div style="margin-bottom:16px">
            <label class="fl">2 premières lettres du nom</label>
            <input class="fi" id="peer-join-init" placeholder="AB" maxlength="2" style="text-transform:uppercase"/>
          </div>
          <div style="display:flex;gap:10px">
            <button class="cancel-btn" onclick="openPeerSessionModal()">← Retour</button>
            <button class="save-btn" onclick="confirmJoinPeerSession('${code}')">Rejoindre ▶</button>
          </div>
        </div>
      </div>`;
    setTimeout(()=>{ const el=document.getElementById('peer-join-name'); if(el)el.focus(); },100);
  } catch(e){ alert('Erreur : '+e.message); }
}

async function confirmJoinPeerSession(code){
  const nameEl=document.getElementById('peer-join-name');
  const initEl=document.getElementById('peer-join-init');
  const name=(nameEl&&nameEl.value||'').trim();
  const init=(initEl&&initEl.value||'').toUpperCase().replace(/[^A-Z]/g,'');
  if(!name){ if(nameEl)nameEl.style.borderColor='#ff7070'; return; }
  if(init.length<2){ if(initEl)initEl.style.borderColor='#ff7070'; return; }
  ...
  try{
    ...
    if(session.status==='playing'){
      startSessionQuiz(code, fullName, null, null);
    } else {
      showPeerLobby(code, fullName, session.questions, session.level, false);
    }
  } catch(e){ alert('Erreur : '+e.message); }
}

function showPeerLobby(code, playerName, questions, level, isHost){
  const c=document.getElementById('quiz-container');
  showPage('quiz');
  peerSessionCode=code;
  ...
  function render(players){
    c.innerHTML=`
      <div class="session-quiz-wrap" style="text-align:center;padding:20px 0">
        <div style="font-family:var(--disp);font-size:22px;font-weight:900;color:var(--cr);margin-bottom:4px">🎮 Partie privee</div>
        <div style="font-size:13px;color:rgba(200,223,204,.4);margin-bottom:20px">Partage ce code a tes amis</div>
        <div style="font-size:52px;font-weight:900;color:var(--g4);letter-spacing:6px;background:rgba(74,184,112,.08);border:2px solid rgba(74,184,112,.25);border-radius:16px;padding:16px 24px;display:inline-block;margin-bottom:20px">${code}</div>
        <div style="font-size:13px;color:rgba(200,223,204,.4);margin-bottom:16px">
          Niv. ${level} · ${(questions&&questions.length)||20} questions
        </div>
        <div style="margin-bottom:20px">
          ${players.map(function(r){
            return '<div style="padding:8px 16px;background:rgba(74,184,112,.07);border-radius:10px;margin-bottom:6px;font-size:14px;color:var(--cr)">'+
              (r.player_name===playerName?'<span style="color:var(--g4)">● </span>':'')+r.player_name+'</div>';
          }).join('')}
        </div>
        ${isHost?`<button class="save-btn" onclick="startPeerSessionNow('${code}','${playerName}',${JSON.stringify(questions)},${level})"
        ...
        </button>`:'<div style="color:rgba(200,223,204,.4);font-size:13px">En attente du createur…</div>'}
        <button onclick="cancelPeerSession('${code}')" style="background:none;border:1px solid rgba(255,80,80,.25);border-radius:10px;color:#ff9999;padding:8px 16px;cursor:pointer;font-size:12px;margin-top:8px">
          Annuler
        </button>
      </div>`;
  }
  ...
  var poll = async function(){
    try{
      const rows=await sbGet('floralab_session_players','session_code=eq.'+code+'&select=*');
      render(rows);
      if(!isHost){
        const sess=await sbGet('floralab_sessions','code=eq.'+code+'&select=status,questions,level');
        if(sess.length&&sess[0].status==='playing'){
          clearInterval(peerPollInterval);
          startSessionQuiz(code, playerName, null, null);
        }
        if(sess.length&&sess[0].status==='ended'){
          clearInterval(peerPollInterval);
          c.innerHTML='<div style="text-align:center;padding:40px;color:rgba(200,223,204,.4)">Partie annulee.</div>';
        }
      }
    } catch(e){}
  };
  poll();
  peerPollInterval = setInterval(poll, 4000);
}

async function startPeerSessionNow(code, playerName, questions, level){
  if(peerPollInterval){ clearInterval(peerPollInterval); peerPollInterval=null; }
  try{
    await sbPost('floralab_sessions',{code:code, status:'playing'},'resolution=merge-duplicates');
    startSessionQuiz(code, playerName, questions, level);
  } catch(e){ alert('Erreur : '+e.message); }
}

async function cancelPeerSession(code){
  if(peerPollInterval){ clearInterval(peerPollInterval); peerPollInterval=null; }
  try{ await sbPost('floralab_sessions',{code:code, status:'ended'},'resolution=merge-duplicates'); }catch(e){}
  showPage('home');
}
...
function renderStep1(){
  ...
  const items = plants.map(p=>`
    <div class="psel-item ${multiSelectedIds.has(p.id)?'selected':''}" onclick="togglePlantSel(${p.id},this)">
      ...
      <div>
        <div class="psel-item-latin">${italLatin(p.latin)}</div>
  ...
  c.innerHTML=`
    ...
    <div style="margin-bottom:14px">
      ...
      <div style="display:flex;gap:8px">
        ${[0,1,2,3,4].map(n=>{
          const labels={0:'Découverte',1:'Débutant',2:'Intermédiaire',3:'Expert',4:'Technique'};
          const icons ={0:'👀',1:'🌱',2:'🎓',3:'🔬',4:'🏗️'};
          const descs ={0:'Photo → nom commun',1:'Photo+base',2:'Sol+usage',3:'Tous critères',4:'Chantier'};
          const sel = n===multiLevel;
          return '<button class="level-card lv'+(n||'0')+' '+(sel?'selected':'')+'" id="mlc-'+n+'" onclick="selectMultiLevel('+n+')" style="flex:1;min-height:0;padding:8px;border-radius:12px'+(n===0?';border:1px solid rgba(100,200,255,.3);background:rgba(100,200,255,.05)':'')+'">'
            +'<span class="level-badge-tag" '+(n===0?'style="background:rgba(100,200,255,.2);color:#7ab8f5"':'')+'>'+labels[n]+'</span>'
            +'<span class="level-icon" style="font-size:18px">'+icons[n]+'</span>'
            +'<div class="level-title" style="font-size:10px;'+(n===0?'color:#7ab8f5':'')+'">N'+n+'</div>'
            +'</button>';
        }).join('')}
      </div>
    </div>
    <div class="field-filter-wrap">
      ...
      <div class="field-chips" id="field-chips-wrap">
        ${(MULTI_ALL_FIELDS||[]).filter(f=>multiLevel===0||f.levels.includes(multiLevel)).map(f=>`
...
function selectMultiLevel(n){
  ...
  multiLevel=n; // accepte 0 (Découverte) à 4 (Technique)
  [1,2,3].forEach(i=>{
...
// ══════════════════════════════════════════════════════
function goToStep2(){
  const pool = plants.filter(p=>multiSelectedIds.has(p.id) && (p.photo||p.photo2||p.photo3||p.photo4));
...
function renderStep2(){
  ...
  const cards = multiQuestions.map((q,i)=>{
    ...
    const isPhotoType = q.type==='photo'||q.type==='dictee'||q.type==='n3';
    // Sélecteur de photos si plusieurs photos disponibles
    const allPhotos = [q.plantPhoto,q.plantPhoto2,q.plantPhoto3,q.plantPhoto4].filter(Boolean);
    const currentPhoto = q.selectedPhoto || q.plantPhoto || '';
    const photoSelector = (isPhotoType && allPhotos.length > 1) ? `
      <div style="margin:8px 0 4px">
        <div style="font-size:10px;letter-spacing:1.2px;text-transform:uppercase;color:rgba(200,223,204,.4);margin-bottom:6px">📷 Photo affichée aux apprentis</div>
        <div style="display:flex;gap:6px;flex-wrap:wrap">
          ${allPhotos.map((ph,pi)=>`
            <div class="photo-thumb-sel" onclick="selectQuestionPhoto(${i},${pi})"
              style="cursor:pointer;border-radius:8px;overflow:hidden;border:2px solid ${ph===currentPhoto?'var(--g4)':'rgba(74,184,112,.15)'};transition:border .2s;flex-shrink:0">
              <img src="${ph}" style="width:56px;height:56px;object-fit:cover;display:block" onerror="this.parentElement.style.display='none'" loading="lazy"/>
              <div class="photo-thumb-lbl" style="text-align:center;font-size:9px;color:${ph===currentPhoto?'var(--g4)':'rgba(200,223,204,.3)'};padding:2px 0">${ph===currentPhoto?'✓ Choisie':'Photo '+(pi+1)}</div>
            </div>`).join('')}
        </div>
      </div>` : (isPhotoType && allPhotos.length===1 ? `
      <div style="margin:8px 0 4px">
        <img src="${currentPhoto}" style="height:60px;border-radius:8px;object-fit:cover" onerror="this.style.display='none'" loading="lazy"/>
        <div style="font-size:10px;color:rgba(200,223,204,.35);margin-top:2px">1 seule photo disponible</div>
      </div>` : '');
...
// ── Sélection de la photo affichée pour une question ──
function selectQuestionPhoto(qIdx, photoIdx){
  const q = multiQuestions[qIdx];
  if(!q) return;
  const allPhotos = [q.plantPhoto, q.plantPhoto2, q.plantPhoto3, q.plantPhoto4].filter(Boolean);
  if(photoIdx < allPhotos.length){
    q.selectedPhoto = allPhotos[photoIdx];
  }
  // Rafraîchir uniquement la carte sans re-render toute l'étape 2
  const card = document.getElementById('qrev-'+qIdx);
  if(card){
    // Re-générer le sélecteur photo de cette carte
    const currentPhoto = q.selectedPhoto || q.plantPhoto || '';
    const photoThumbs = card.querySelectorAll('[data-photoidx]');
    // Mise à jour visuelle des bordures et labels
    const thumbContainers = card.querySelectorAll('.photo-thumb-sel');
    thumbContainers.forEach((el, pi) => {
      const ph = allPhotos[pi];
      el.style.border = ph===currentPhoto ? '2px solid var(--g4)' : '2px solid rgba(74,184,112,.15)';
      const lbl = el.querySelector('.photo-thumb-lbl');
      if(lbl){ lbl.textContent = ph===currentPhoto ? '✓ Choisie' : 'Photo '+(pi+1); lbl.style.color = ph===currentPhoto ? 'var(--g4)' : 'rgba(200,223,204,.3)'; }
    });
    // Fallback si pas de classes : re-render complet
    if(!thumbContainers.length) renderStep2();
  } else renderStep2();
}
...
function generateOneQuestion(pool, level, usedPlantIds){
  ...
  // Niveau 0 : photo uniquement, correct = nom commun (comme quiz solo N0)
  if(level===0){
    return buildPhotoQuestion_N0(p, pool);
  }

  if(type==='dictee'){
    return buildDicteeQuestion(p, level);
  } else if(type==='photo'){
...
function buildDicteeQuestion(p, level){
  // N0, N1, N2 : uniquement Genre + espèce + hybride (×)
  // Pas de cultivar ('NomCultivar') ni de trinôme (var./subsp./f.)
  let target = p.latin;
  const lvl = level || qLevel;
  if(lvl <= 2){
    // Supprimer le cultivar : Prunus avium 'Burlat' → Prunus avium
    target = target.replace(/\s+'[^']*'/g, '').replace(/\s+"[^"]*"/g, '');
    // Supprimer les rangs infraspécifiques : var. / subsp. / f. + ce qui suit
    target = target.replace(/\s+(var\.|subsp\.|f\.|cv\.).*$/i, '').trim();
  }
  return {
    type:'dictee', plantId:p.id, plantLatin:p.latin, plantNom:p.nom,
    plantPhoto:p.photo||'', plantPhoto2:p.photo2||'', plantPhoto3:p.photo3||'', plantPhoto4:p.photo4||'',
    question:'Quel est le nom latin de ce végétal ?',
    correct:target,  // ← simplifié pour N1/N2
    correctFull:p.latin, // ← nom complet conservé pour affichage
    options:[],
...
function buildPhotoQuestion(p, pool){
  // pool déjà filtré (au moins 1 photo) en amont
...
// ── Question N0 : Photo → trouver le nom COMMUN (Découverte) ──
function buildPhotoQuestion_N0(p, pool){
  const withNom = pool.filter(x=>x.id!==p.id && x.nom);
  const wrongs = shuffle(withNom).slice(0,3);
  if(wrongs.length<3) return buildPhotoQuestion(p, pool); // fallback N1
  const nom = (p.nom||'').split(/[,;/]/)[0].trim();
  if(!nom) return buildPhotoQuestion(p, pool);
  const options = shuffle([nom, ...wrongs.map(x=>(x.nom||'').split(/[,;/]/)[0].trim())]);
  return {
    type:'photo', plantId:p.id, plantLatin:p.latin, plantNom:p.nom,
    plantPhoto:p.photo||'', plantPhoto2:p.photo2||'', plantPhoto3:p.photo3||'', plantPhoto4:p.photo4||'',
    question:'Quel est le nom commun de ce végétal ?',
    correct:nom,
    options:options,
    fieldLabel:'Nom commun',
    status:'pending'
...
function buildKnowledgeQuestion(p, pool, level, fieldKeys){
  ...
  const field = rand(avail);
  let correct = p[field.key];
  let allValidValsM=[correct];
  if(MULTIVALUE_KEYS.includes(field.key)){
    const vals = correct.split(',').map(s=>s.trim()).filter(Boolean);
    correct = rand(vals) || correct;
    allValidValsM=vals;
  }
  const others = [...new Set(pool.filter(x=>x.id!==p.id&&x[field.key]).map(x=>{
    if(MULTIVALUE_KEYS.includes(field.key)){
      const vals=(x[field.key]||'').split(',').map(s=>s.trim()).filter(v=>v&&!allValidValsM.includes(v));
      return vals.length?rand(vals):null;
    }
    return allValidValsM.includes(x[field.key]) ? null : x[field.key];
  }).filter(Boolean))];
  let wrongs = shuffle(others).slice(0,3);
  const fil = field.key==='famille'?shuffle(FILLERS_FAMILLE.filter(f=>!allValidValsM.includes(f))):shuffle((FILLERS[field.key]||[]).filter(f=>!allValidValsM.includes(f)));
  for(const f of fil){ if(wrongs.length>=3)break; if(!allValidValsM.includes(f)&&!wrongs.includes(f))wrongs.push(f); }
  if(wrongs.length<3) return null;
...
async function goToStep3(){
  ...
  // Alléger les questions : retirer les photos base64 (trop lourdes pour Supabase)
  // Les photos sont rechargées depuis floralab_photos au moment du quiz
  const questionsLight = valid.slice(0,20).map(function(q){
    return Object.assign({}, q, {
      plantPhoto:  (q.plantPhoto  && !q.plantPhoto.startsWith('data:'))  ? q.plantPhoto  : '',
      plantPhoto2: (q.plantPhoto2 && !q.plantPhoto2.startsWith('data:')) ? q.plantPhoto2 : '',
      plantPhoto3: (q.plantPhoto3 && !q.plantPhoto3.startsWith('data:')) ? q.plantPhoto3 : '',
...
async function startSessionLobby(code){
  ...
  // Poll toutes les 3s
  if(multiPollInterval) clearInterval(multiPollInterval);
  multiPollInterval = setInterval(()=>pollSessionPlayers(code), 8000);
...
async function renderSessionMonitor(code){
  ...
  multiPollInterval = setInterval(()=>pollMonitor(code), 3000);
  pollMonitor(code);
}
async function pollMonitor(code){
  try{
    ...
    const now = new Date(); const hh=now.getHours().toString().padStart(2,'0'); const mm=now.getMinutes().toString().padStart(2,'0'); const ss=now.getSeconds().toString().padStart(2,'0');
    cnt.textContent = `${rows.length} joueur(s) · ${done} terminé(s) · màj ${hh}:${mm}:${ss}`;
  ...
  sessionPollI = setInterval(async ()=>{
    try{
      ...
      if(rows[0].status==='playing'){
        ...
        // Recharger les questions fraîches (avec level) depuis Supabase au démarrage
        startSessionQuiz(code, playerName, null, null);
      }
    } catch(e){}
  }, 5000);
}
async function startSessionQuiz(code, playerName, questions, level){
  // Si questions manquantes (ex: trop lourdes au moment du join), recharger depuis Supabase
  var qs = questions;
  if(!qs || !qs.length){
    try{
      const rows = await sbGet('floralab_sessions', 'code=eq.'+code+'&select=questions,level');
      if(rows.length){ qs = rows[0].questions; level = level || rows[0].level; }
    } catch(e){}
  }
  if(!qs || !qs.length){
    alert('❌ Impossible de charger les questions. Réessaie de rejoindre la session.');
    showPage('home'); return;
  }
  cqLog('startSessionQuiz — code:', code, '| joueur:', playerName, '| questions:', qs.length);
  // Enrichir les questions avec les photos depuis floralab_photos si manquantes
  if(cloudOk){
    try{
      // Charger seulement les photos des plantes du quiz (pas tout floralab_photos)
      const plantIds = [...new Set(qs.map(function(q){return q.plantId;}).filter(Boolean))];
      const idFilter = 'plant_id=in.('+plantIds.join(',')+')';
      const photoRows = await sbGet('floralab_photos','select=plant_id,photo,photo2,photo3,photo4&'+idFilter);
      qs = qs.map(function(q){
        if(q.plantPhoto) return q; // déjà une photo
        var ph = photoRows.find(function(r){ return r.plant_id === q.plantId; });
        if(ph) return Object.assign({}, q, {
          plantPhoto:  ph.photo  ||'',
          plantPhoto2: ph.photo2 ||'',
          plantPhoto3: ph.photo3 ||'',
          plantPhoto4: ph.photo4 ||''
        });
        return q;
      });
    } catch(e){}
...
async function renderSessionQuestion(code){
  ...
  var photoHtml = '';
  if(isPhoto||isDictee){
    // Photo principale = selectedPhoto si l'admin a fait un choix, sinon plantPhoto
    const mainPhoto = q.selectedPhoto || q.plantPhoto || '';
    // Les autres photos disponibles pour le carrousel (hors celle choisie en principal)
    const otherPhotos = [q.plantPhoto, q.plantPhoto2, q.plantPhoto3, q.plantPhoto4]
      .filter(ph => ph && ph !== mainPhoto);
    // Toutes les photos : principale en premier, puis les autres
    const sessionPhotos = mainPhoto ? [mainPhoto, ...otherPhotos] : otherPhotos;
    if(sessionPhotos.length > 0){
      const slides = sessionPhotos.map(ph=>
        `<div class="qphoto-slide"><img src="${ph}" class="qphoto" onerror="this.style.display='none'" loading="lazy"/></div>`
      ).join('');
      const dots = sessionPhotos.length > 1
        ? `<div class="qphoto-dots">${sessionPhotos.map((_,di)=>`<span class="qphoto-dot${di===0?' active':''}" onclick="goSlide(${di})"></span>`).join('')}</div>`
        : '';
  ...
  const opts = isDictee
    ? (`<div class="dictee-wrap">`
      +`<input id="dictee-input" class="dictee-input" type="text" placeholder="Genre espèce…"
       autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false"
       style="font-style:italic;font-family:Arial,Helvetica,sans-serif;letter-spacing:0.5px"
...
async function pickSessionAnswer(btn, answer, code){
  ...
  // Mettre à jour score dans Supabase (retry x3)
  sessionQIdx++;
  (function(){ var _pD={score:sessionScore.ok,total:sessionScore.total,current_q:sessionQIdx,done:sessionQIdx>=sessionQuestions.length,updated_at:new Date().toISOString()}; var _pF='session_code=eq.'+code+'&player_name=eq.'+encodeURIComponent(sessionPlayerName); (async function(){ for(var _t=0;_t<3;_t++){ try{ await sbPatch('floralab_session_players',_pF,_pD); break; }catch(e){ if(_t===2) cqErr('PATCH score failed:',e); await new Promise(function(r){setTimeout(r,800);}); } } })(); })();
  // Bouton suivant
...
async function submitSessionDictee(code){
  ...
  (function(){ var _pD={score:sessionScore.ok,total:sessionScore.total,current_q:sessionQIdx,done:sessionQIdx>=sessionQuestions.length}; var _pF='session_code=eq.'+code+'&player_name=eq.'+encodeURIComponent(sessionPlayerName); (async function(){ for(var _t=0;_t<3;_t++){ try{ await sbPatch('floralab_session_players',_pF,_pD); break; }catch(e){ if(_t===2) cqErr('PATCH dictee score failed:',e); await new Promise(function(r){setTimeout(r,800);}); } } })(); })();
}


async function onSessionTimeout(code){
  stopTimer(); sessionScore.total++; sessionQIdx++;
  (function(){ var _pD={score:sessionScore.ok,total:sessionScore.total,current_q:sessionQIdx,done:sessionQIdx>=sessionQuestions.length,updated_at:new Date().toISOString()}; var _pF='session_code=eq.'+code+'&player_name=eq.'+encodeURIComponent(sessionPlayerName); (async function(){ for(var _t=0;_t<3;_t++){ try{ await sbPatch('floralab_session_players',_pF,_pD); break; }catch(e){ if(_t===2) cqErr('PATCH timeout score failed:',e); await new Promise(function(r){setTimeout(r,800);}); } } })(); })();
  renderSessionQuestion(code);
}
...
async function renderSessionEnd(code){
  ...
  // S'assurer que done=true est bien enregistré dans Supabase (retry x3)
  var _endFilter = 'session_code=eq.'+code+'&player_name=eq.'+encodeURIComponent(sessionPlayerName);
  var _endData = {score:sessionScore.ok, total:sessionScore.total, current_q:sessionQIdx, done:true, updated_at:new Date().toISOString()};
  for(var _r=0; _r<3; _r++){
    try{ await sbPatch('floralab_session_players', _endFilter, _endData); break; }
    catch(e){ if(_r===2) cqErr('renderSessionEnd patch done failed:', e); await new Promise(function(res){setTimeout(res,1000);}); }
  }
  const c = document.getElementById('quiz-container');
...
// ═══════════════════════════════════════════════════════════════
//  PLANTNET — Identification botanique par photo
// ═══════════════════════════════════════════════════════════════

function openPlantnetModal(){
  const existing = document.getElementById('pnet-overlay');
  if(existing) existing.remove();
  window._pnetOrgan = 'auto';
  window._pnetFiles = [];
  window._pnetResults = [];
  const ORGANS = [
    ['auto','🌿 Auto'],['leaf','🍃 Feuille'],['flower','🌸 Fleur'],
    ['fruit','🍒 Fruit'],['bark','🪵 Écorce']
  ];
  const organBtns = ORGANS.map(function(pair){
    const v = pair[0], l = pair[1];
    const active = v === 'auto';
    const bg  = active ? 'rgba(74,184,112,.25)' : 'rgba(255,255,255,.05)';
    const bd  = active ? 'rgba(74,184,112,.5)'  : 'rgba(255,255,255,.1)';
    const col = active ? '#4ab870' : 'rgba(200,223,204,.5)';
    return '<button onclick="_pnetSetOrgan(\'' + v + '\',this)" data-organ="' + v + '"'
      + ' style="padding:7px 12px;border-radius:20px;font-size:12px;cursor:pointer;transition:all .2s;'
      + 'background:' + bg + ';border:1px solid ' + bd + ';color:' + col + '">'
      + l + '</button>';
  }).join('');

  const notConfigured = !plantnetKey;

  const div = document.createElement('div');
  div.id = 'pnet-overlay';
  div.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.78);z-index:10000;display:flex;align-items:center;justify-content:center;padding:16px;box-sizing:border-box';
  div.innerHTML = ''
    + '<div style="background:#0d2b1a;border:1px solid rgba(74,184,112,.25);border-radius:18px;width:100%;max-width:480px;max-height:90vh;overflow-y:auto;padding:20px;box-sizing:border-box">'
    +   '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">'
    +     '<div style="font-size:18px;font-weight:900;color:#4ab870">🔍 Identification Pl@ntNet</div>'
    +     '<button onclick="closePlantnetModal()" style="background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.15);border-radius:8px;color:rgba(200,223,204,.6);width:32px;height:32px;cursor:pointer;font-size:16px">✕</button>'
    +   '</div>'
    + (notConfigured
      ? '<div style="text-align:center;padding:24px 16px">'
        + '<div style="font-size:36px;margin-bottom:12px">🔑</div>'
        + '<div style="color:rgba(200,223,204,.7);font-size:14px;line-height:1.5">Fonctionnalité non configurée.<br>Contacter le formateur pour activer l\'identification Pl@ntNet.</div>'
        + '</div>'
      : '<div style="margin-bottom:14px">'
        + '<div style="font-size:12px;color:rgba(200,223,204,.45);margin-bottom:8px">📷 Photos (1 à 4 — plusieurs angles améliorent la précision)</div>'
        + '<div id="pnet-photos-grid" style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px"></div>'
        + '<div id="pnet-add-btn" onclick="document.getElementById(\'pnet-file\').click()" ondragover="event.preventDefault()" ondrop="event.preventDefault();_pnetLoadFiles(event.dataTransfer.files)"'
        +   ' style="border:2px dashed rgba(74,184,112,.35);border-radius:12px;padding:14px;text-align:center;cursor:pointer">'
        +   '<div style="color:rgba(200,223,204,.5);font-size:13px">➕ Ajouter une photo (glisser ou cliquer)</div>'
        +   '<input type="file" id="pnet-file" accept="image/*" multiple style="display:none" onchange="_pnetLoadFiles(this.files)"/>'
        + '</div>'
        + '</div>'
        + '<div style="margin-bottom:14px">'
        +   '<div style="font-size:12px;color:rgba(200,223,204,.45);margin-bottom:8px">Partie photographiée</div>'
        +   '<div style="display:flex;flex-wrap:wrap;gap:6px" id="pnet-organs">' + organBtns + '</div>'
        + '</div>'
        + '<button id="pnet-btn" onclick="_pnetRun()"'
        +   ' style="width:100%;padding:13px;background:linear-gradient(135deg,rgba(74,184,112,.2),rgba(34,197,94,.1));border:1px solid rgba(74,184,112,.4);border-radius:12px;color:#4ab870;font-size:15px;font-weight:700;cursor:pointer;margin-bottom:14px">'
        +   '🔍 Identifier la plante'
        + '</button>'
        + '<div id="pnet-results"></div>'
      )
    + '</div>';
  document.body.appendChild(div);
  div.addEventListener('click', function(e){ if(e.target === div) closePlantnetModal(); });
}

function closePlantnetModal(){
  const el = document.getElementById('pnet-overlay');
  if(el) el.remove();
}

function _pnetSetOrgan(val, btn){
  window._pnetOrgan = val;
  document.querySelectorAll('#pnet-organs button').forEach(function(b){
    const active = b.dataset.organ === val;
    b.style.background   = active ? 'rgba(74,184,112,.25)' : 'rgba(255,255,255,.05)';
    b.style.borderColor  = active ? 'rgba(74,184,112,.5)'  : 'rgba(255,255,255,.1)';
    b.style.color        = active ? '#4ab870' : 'rgba(200,223,204,.5)';
  });
}

function _pnetLoadFiles(fileList){
  if(!fileList || !fileList.length) return;
  var files = Array.from(fileList).filter(function(f){ return f.type.startsWith('image/'); });
  if(!files.length) return;
  var current = window._pnetFiles || [];
  files.forEach(function(f){
    if(current.length < 4) current.push(f);
  });
  window._pnetFiles = current;
  _pnetRenderPhotos();
}

function _pnetRenderPhotos(){
  var grid = document.getElementById('pnet-photos-grid');
  var addBtn = document.getElementById('pnet-add-btn');
  if(!grid) return;
  var files = window._pnetFiles || [];
  grid.innerHTML = files.map(function(f, i){
    var url = URL.createObjectURL(f);
    return '<div style="position:relative;border-radius:10px;overflow:hidden;background:rgba(0,0,0,.3);aspect-ratio:1">'
      + '<img src="' + url + '" style="width:100%;height:100%;object-fit:cover"/>'
      + '<button onclick="_pnetRemovePhoto(' + i + ')" style="position:absolute;top:4px;right:4px;width:22px;height:22px;background:rgba(0,0,0,.7);border:none;border-radius:50%;color:#fff;font-size:13px;cursor:pointer;line-height:1;padding:0">✕</button>'
      + '<div style="position:absolute;bottom:0;left:0;right:0;background:rgba(0,0,0,.5);font-size:10px;color:rgba(255,255,255,.7);padding:2px 4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + f.name + '</div>'
      + '</div>';
  }).join('');
  // Masquer le bouton ajout si 4 photos atteintes
  if(addBtn) addBtn.style.display = files.length >= 4 ? 'none' : '';
}

function _pnetRemovePhoto(idx){
  if(!window._pnetFiles) return;
  window._pnetFiles.splice(idx, 1);
  _pnetRenderPhotos();
}

// ── Chargement clé PlantNet depuis Supabase (au login) ──
async function loadPlantnetKey(){
  if(!cloudOk || !sbConf) return;
  try{
    const rows = await sbGet('floralab_config', 'key=eq.plantnet_api_key&select=value');
    if(rows && rows.length && rows[0].value) plantnetKey = rows[0].value.trim().replace(/[\r\n\t]/g,'');
  }catch(e){ /* silencieux */ }
}

// ── Sauvegarde clé PlantNet vers Supabase (admin uniquement) ──
async function savePlantnetKeyToCloud(key){
  if(!cloudOk || !sbConf) return false;
  try{
    await sbPost('floralab_config', {key:'plantnet_api_key', value:key}, 'resolution=merge-duplicates&on_conflict=key');
    return true;
  }catch(e){ return false; }
}

// ── Onglet admin PlantNet ──
function renderPlantnetTab(){
  var el = document.getElementById('plantnet-tab-content');
  if(!el) return;
  var configured = !!plantnetKey;
  var preview = configured ? plantnetKey.slice(0,4)+'...'+plantnetKey.slice(-4) : '';
  var statusBlock = configured
    ? '<div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;padding:10px 14px;background:rgba(74,184,112,.08);border:1px solid rgba(74,184,112,.2);border-radius:10px">'
      + '<span style="font-size:18px">✅</span>'
      + '<span style="color:#4ab870;font-size:13px;font-weight:700">Cle en memoire : ' + preview + '</span>'
      + '</div>'
    : '<div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;padding:10px 14px;background:rgba(232,160,32,.08);border:1px solid rgba(232,160,32,.2);border-radius:10px">'
      + '<span style="font-size:18px">⚠️</span>'
      + '<span style="color:var(--am2);font-size:13px">Aucune cle chargee</span>'
      + '</div>';
  el.innerHTML = '<div style="max-width:520px">'
    + '<div style="background:rgba(255,255,255,.04);border:1px solid rgba(74,184,112,.15);border-radius:14px;padding:20px;margin-bottom:16px">'
    + statusBlock
    + (configured
      ? '<button onclick="_pnetAdminTest()" id="pnet-test-btn" style="width:100%;padding:10px;margin-bottom:14px;background:rgba(74,184,112,.08);border:1px solid rgba(74,184,112,.2);border-radius:10px;color:#4ab870;font-size:13px;cursor:pointer">🧪 Tester la cle active</button>'
      : '')
    + '<div id="pnet-test-result" style="font-size:12px;margin-bottom:10px;min-height:0"></div>'
    + '<label style="font-size:12px;color:rgba(200,223,204,.45);display:block;margin-bottom:6px">'
    + (configured ? '🔄 Modifier la cle API' : '🔑 Saisir la cle API') + ' <em style="font-size:10px">(my.plantnet.org)</em>'
    + '</label>'
    + '<div style="display:flex;gap:8px">'
    + '<input type="password" id="pnet-admin-key" placeholder="2b10..." autocomplete="new-password"'
    + ' style="flex:1;padding:11px 14px;background:rgba(19,48,29,.8);border:1px solid rgba(74,184,112,.25);border-radius:10px;color:var(--cr);font-size:14px;outline:none;font-family:monospace"/>'
    + '<button onclick="_pnetAdminToggleVisible()" title="Afficher/masquer"'
    + ' style="padding:11px 13px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);border-radius:10px;color:rgba(200,223,204,.5);cursor:pointer;font-size:16px">👁</button>'
    + '<button onclick="_pnetAdminSave()" id="pnet-admin-save-btn"'
    + ' style="padding:11px 18px;background:rgba(74,184,112,.15);border:1px solid rgba(74,184,112,.35);border-radius:10px;color:#4ab870;font-size:13px;font-weight:700;cursor:pointer">💾 Enregistrer</button>'
    + '</div>'
    + '<div id="pnet-admin-msg" style="font-size:12px;margin-top:8px;min-height:18px"></div>'
    + '</div>'
    + '<div style="font-size:11px;color:rgba(200,223,204,.25);line-height:1.5">'
    + '🔗 Cle gratuite sur <strong>my.plantnet.org</strong> → Mon compte → API · Usage educatif non commercial.'
    + '</div>'
    + '</div>';
}

async function _pnetAdminTest(){
  var btn = document.getElementById('pnet-test-btn');
  var res = document.getElementById('pnet-test-result');
  if(!plantnetKey){ if(res) res.innerHTML = '<span style="color:#ffb347">Pas de cle en memoire</span>'; return; }
  if(btn){ btn.disabled=true; btn.textContent='⏳ Test en cours…'; }
  try{
    var r = await fetch('https://my-api.plantnet.org/v2/identify/all?lang=fr&api-key='+encodeURIComponent(plantnetKey), {method:'GET'});
    var status = r.status;
    var txt = '';
    try{ var d = await r.json(); txt = JSON.stringify(d).slice(0,80); }catch(e){ txt = r.statusText; }
    var col = (status===200||status===400||status===404) ? '#4ab870' : '#ffb347';
    var msg = status===403 ? '❌ 403 — Cle rejetee (CORS ou cle invalide)' :
              status===400 ? '✅ 400 — Cle acceptee (pas de photo fournie, normal)' :
              status===200 ? '✅ 200 — Cle OK' :
              '⚠️ HTTP ' + status + ' — ' + txt;
    if(res) res.innerHTML = '<div style="padding:8px 10px;background:rgba(0,0,0,.2);border-radius:8px;color:'+col+';font-family:monospace;font-size:11px">'+msg+'</div>';
  }catch(e){
    if(res) res.innerHTML = '<div style="color:#ffb347;font-size:11px">❌ Erreur reseau : '+e.message+'</div>';
  }
  if(btn){ btn.disabled=false; btn.textContent='🧪 Tester la cle active'; }
}

function _pnetAdminToggleVisible(){
  const inp = document.getElementById('pnet-admin-key');
  if(!inp) return;
  inp.type = inp.type === 'password' ? 'text' : 'password';
}

async function _pnetAdminSave(){
  const inp = document.getElementById('pnet-admin-key');
  const msg = document.getElementById('pnet-admin-msg');
  const btn = document.getElementById('pnet-admin-save-btn');
  if(!inp || !inp.value.trim()){
    if(msg){ msg.style.color='#ffb347'; msg.textContent="⚠️ Saisir une clé avant d'enregistrer"; }
    return;
  }
  // Nettoyage agressif : espaces, retours à la ligne, tabulations, caractères invisibles
  const key = inp.value.trim().replace(/[\r\n\t\u200B\u00A0]/g,'');
  if(!key){
    if(msg){ msg.style.color='#ffb347'; msg.textContent='⚠️ Clé invalide après nettoyage'; }
    return;
  }
  if(btn){ btn.disabled=true; btn.textContent='⏳…'; }
  const ok = await savePlantnetKeyToCloud(key);
  if(ok){
    plantnetKey = key;
    inp.value = '';
    // Affiche début et fin de la clé pour vérification visuelle
    const preview = key.length > 8 ? key.slice(0,4)+'…'+key.slice(-4) : '****';
    if(msg){ msg.style.color='#4ab870'; msg.textContent='✅ Clé enregistrée ('+preview+') — active immédiatement'; }
    setTimeout(function(){ renderPlantnetTab(); }, 1800);
  } else {
    if(msg){ msg.style.color='#ffb347'; msg.textContent='❌ Erreur — vérifiez la connexion cloud (SQL Supabase exécuté ?)'; }
    if(btn){ btn.disabled=false; btn.textContent='💾 Enregistrer'; }
  }
}

async function _pnetRun(){
  var files = window._pnetFiles || [];
  var res  = document.getElementById('pnet-results');
  if(!files.length){
    if(res) res.innerHTML = '<div style="color:#ffb347;font-size:13px;text-align:center;padding:12px">📷 Ajoutez au moins une photo</div>';
    return;
  }
  var key = plantnetKey;
  if(!key){
    if(res) res.innerHTML = '<div style="color:#ffb347;font-size:13px;padding:12px;text-align:center">🔑 Cle API non configuree — contacter le formateur</div>';
    return;
  }
  var btn = document.getElementById('pnet-btn');
  if(btn){ btn.disabled = true; btn.textContent = '⏳ Identification en cours…'; }
  if(res) res.innerHTML = '<div style="text-align:center;padding:20px;color:rgba(200,223,204,.4);font-size:13px">⏳ Analyse en cours…</div>';
  try{
    var fd = new FormData();
    files.forEach(function(f){ fd.append('images', f); });
    // Un organe par photo (le même pour toutes, simplification)
    files.forEach(function(){ fd.append('organs', window._pnetOrgan || 'auto'); });
    var r = await fetch('https://my-api.plantnet.org/v2/identify/all?lang=fr&api-key=' + encodeURIComponent(key), {method:'POST', body:fd});
    if(r.status === 403){ throw new Error('bad_key'); }
    if(r.status === 404 || r.status === 400){ throw new Error('no_result'); }
    if(!r.ok){ throw new Error('api_error_'+r.status); }
    var data = await r.json();
    var results = (data.results || []).slice(0, 5);
    if(!results.length){ throw new Error('no_result'); }
    window._pnetResults = results;
    window._pnetSelected = [];
    var html = '<div style="font-size:12px;color:rgba(200,223,204,.35);margin-bottom:6px">' + results.length + ' resultat(s) — cocher pour comparer, cliquer Utiliser pour remplir</div>';
    // Zone de comparaison (apparait quand 2+ cochés)
    html += '<div id="pnet-compare-bar" style="display:none;margin-bottom:10px;padding:10px 12px;background:rgba(74,184,112,.08);border:1px solid rgba(74,184,112,.25);border-radius:10px">'
      + '<div style="font-size:12px;color:#4ab870;margin-bottom:8px">Végétaux sélectionnés pour comparer :</div>'
      + '<div id="pnet-selected-list" style="font-size:11px;font-style:italic;color:#c8dfcc;margin-bottom:8px"></div>'
      + '<div style="display:flex;gap:8px;flex-wrap:wrap">'
      +   '<button onclick="_pnetOpenGemini()" style="padding:7px 13px;background:linear-gradient(135deg,rgba(66,133,244,.2),rgba(52,168,83,.15));border:1px solid rgba(66,133,244,.4);border-radius:9px;color:#7ab8f5;font-size:12px;font-weight:700;cursor:pointer">🤖 Comparer avec IA</button>'
      +   '<button onclick="_pnetOpenGoogle()" style="padding:7px 13px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.15);border-radius:9px;color:rgba(200,223,204,.6);font-size:12px;cursor:pointer">🔍 Google</button>'
      + '</div>'
      + '</div>';
    results.forEach(function(r2, i){
      var sp     = r2.species || {};
      var latin  = sp.scientificNameWithoutAuthor || '';
      var common = (sp.commonNames || [])[0] || '';
      var fam    = (sp.family || {}).scientificNameWithoutAuthor || '';
      var pct    = Math.round((r2.score || 0) * 100);
      var barCol = pct >= 70 ? '#4ab870' : pct >= 40 ? '#f5a623' : '#e84040';
      html += '<div id="pnet-result-' + i + '" style="background:rgba(255,255,255,.04);border:1px solid rgba(74,184,112,.12);border-radius:12px;padding:12px 14px;margin-bottom:8px">'
        + '<div style="display:flex;align-items:flex-start;gap:10px">'
        +   '<input type="checkbox" id="pnet-chk-' + i + '" onchange="_pnetToggleSelect(' + i + ',this)" style="margin-top:3px;width:16px;height:16px;cursor:pointer;accent-color:#4ab870;flex-shrink:0"/>'
        +   '<div style="flex:1">'
        +     '<div style="font-style:italic;color:#c8dfcc;font-size:14px">' + latin + '</div>'
        +     (common ? '<div style="font-size:12px;color:rgba(200,223,204,.6);margin-top:2px">' + common + '</div>' : '')
        +     (fam    ? '<div style="font-size:11px;color:rgba(200,223,204,.35);margin-top:1px">' + fam + '</div>' : '')
        +     '<div style="margin-top:8px;height:4px;background:rgba(255,255,255,.08);border-radius:2px">'
        +       '<div style="height:4px;width:' + pct + '%;background:' + barCol + ';border-radius:2px"></div>'
        +     '</div>'
        +     '<div style="font-size:10px;color:rgba(200,223,204,.35);margin-top:3px">Confiance : ' + pct + '%</div>'
        +   '</div>'
        +   '<div style="display:flex;flex-direction:column;gap:6px;flex-shrink:0">'
        +     '<button onclick="_pnetFill(' + i + ',this)" style="padding:6px 10px;background:rgba(74,184,112,.15);border:1px solid rgba(74,184,112,.35);border-radius:9px;color:#4ab870;font-size:11px;font-weight:700;cursor:pointer;white-space:nowrap">✅ Utiliser</button>'
        +     '<button onclick="_pnetSearch1(' + i + ')" style="padding:6px 10px;background:rgba(66,133,244,.1);border:1px solid rgba(66,133,244,.3);border-radius:9px;color:#7ab8f5;font-size:11px;cursor:pointer;white-space:nowrap">🔍 Chercher</button>'
        +   '</div>'
        + '</div>'
        + '</div>';
    });
    ...
    var preview = plantnetKey ? plantnetKey.slice(0,4)+'...'+plantnetKey.slice(-4) : '-';
    var msg = '❌ Erreur identification';
    if(err.message === 'no_result') msg = '🔍 Aucune plante reconnue — autre photo ou organe ?';
    if(err.message === 'bad_key')   msg = '🔑 Cle rejetee (' + preview + ') — verifier sur my.plantnet.org';
    if(err.message.indexOf('api_error') === 0) msg = '❌ Erreur serveur (' + err.message.split('_')[2] + ')';
    if(res) res.innerHTML = '<div style="color:#ffb347;font-size:13px;padding:12px;text-align:center">' + msg + '</div>';
  }
  if(btn){ btn.disabled = false; btn.textContent = '🔍 Identifier la plante'; }
}

function _pnetToggleSelect(idx, chk){
  if(!window._pnetSelected) window._pnetSelected = [];
  var results = window._pnetResults || [];
  var r2 = results[idx];
  if(!r2) return;
  var latin = (r2.species || {}).scientificNameWithoutAuthor || '';
  if(chk.checked){
    if(window._pnetSelected.indexOf(idx) === -1) window._pnetSelected.push(idx);
  } else {
    window._pnetSelected = window._pnetSelected.filter(function(i){ return i !== idx; });
  }
  // Mettre à jour le style de la carte
  var card = document.getElementById('pnet-result-'+idx);
  if(card) card.style.borderColor = chk.checked ? 'rgba(74,184,112,.5)' : 'rgba(74,184,112,.12)';
  // Mettre à jour la barre de comparaison
  var bar = document.getElementById('pnet-compare-bar');
  var list = document.getElementById('pnet-selected-list');
  if(!bar || !list) return;
  if(window._pnetSelected.length >= 1){
    bar.style.display = 'block';
    var names = window._pnetSelected.map(function(i){
      var sp = (results[i] && results[i].species) ? results[i].species : {};
      return sp.scientificNameWithoutAuthor || '';
    });
...
function _pnetGetLatins(){
  var results = window._pnetResults || [];
  var sel = window._pnetSelected || [];
...
function _pnetSearch1(idx){
  var results = window._pnetResults || [];
  var r2 = results[idx];
  if(!r2) return;
  var latin = (r2.species || {}).scientificNameWithoutAuthor || '';
  if(!latin) return;
  // Ouvre un mini-menu choix Google / Gemini
  var btn = document.querySelector('#pnet-result-'+idx+' button:last-child');
  var existing = document.getElementById('pnet-search-menu');
  if(existing){ existing.remove(); return; }
  var menu = document.createElement('div');
  menu.id = 'pnet-search-menu';
  menu.style.cssText = 'position:fixed;z-index:10001;background:#0d2b1a;border:1px solid rgba(74,184,112,.3);border-radius:12px;padding:8px;min-width:180px;box-shadow:0 4px 20px rgba(0,0,0,.5)';
  var gUrl  = 'https://www.google.com/search?q=' + encodeURIComponent(latin);
  var aiUrl = 'https://www.perplexity.ai/search?q=' + encodeURIComponent('Critères botaniques et visuels permettant d\'identifier avec certitude ' + latin + ' : morphologie des feuilles (forme, bord, nervures, texture, couleur), fleurs (couleur, forme, nombre de pétales, inflorescence), fruits/graines, écorce, port général, caractères distinctifs qui la différencient des espèces proches');
  menu.innerHTML = ''
    + '<div style="font-size:11px;color:rgba(200,223,204,.4);padding:4px 6px 8px">Rechercher <em>' + latin + '</em></div>'
    + '<button onclick="window.open(\'' + gUrl + '\',\'_blank\');document.getElementById(\'pnet-search-menu\').remove()" style="width:100%;padding:8px 10px;background:rgba(255,255,255,.06);border:none;border-radius:8px;color:rgba(200,223,204,.8);font-size:13px;cursor:pointer;text-align:left;margin-bottom:6px">🔍 Google Search</button>'
    + '<button onclick="window.open(\'' + aiUrl + '\',\'_blank\');document.getElementById(\'pnet-search-menu\').remove()" style="width:100%;padding:8px 10px;background:rgba(66,133,244,.1);border:none;border-radius:8px;color:#7ab8f5;font-size:13px;cursor:pointer;text-align:left">🤖 Perplexity (IA)</button>';
...
function _pnetOpenGemini(){
  var latins = _pnetGetLatins();
  if(!latins.length) return;
  var q;
  if(latins.length === 1){
    q = 'Critères botaniques et visuels permettant d\'identifier avec certitude ' + latins[0] + ' : morphologie des feuilles (forme, bord, nervures, texture, couleur), fleurs (couleur, forme, nombre de pétales, inflorescence), fruits/graines, écorce, port général, caractères distinctifs qui la différencient des espèces proches';
  } else {
    q = 'Compare botaniquement et visuellement ' + latins.join(' et ') + ' : quels sont les critères morphologiques qui permettent de les distinguer avec certitude ? (feuilles, fleurs, fruits, écorce, port, caractères distinctifs). Tableau comparatif si possible.';
  }
  window.open('https://www.perplexity.ai/search?q=' + encodeURIComponent(q), '_blank');
}

function _pnetOpenGoogle(){
  var latins = _pnetGetLatins();
  if(!latins.length) return;
  var q = latins.join(' vs ');
  window.open('https://www.google.com/search?q=' + encodeURIComponent(q), '_blank');
}

function _pnetFill(idx, btn){
  const results = window._pnetResults || [];
  const r2 = results[idx];
  if(!r2) return;
  const sp     = r2.species || {};
  const latin  = sp.scientificNameWithoutAuthor || '';
  const common = (sp.commonNames || [])[0] || '';
  const fam    = (sp.family || {}).scientificNameWithoutAuthor || '';
  const fLatin = document.getElementById('f-latin');
  const fNom   = document.getElementById('f-nom');
  const fFam   = document.getElementById('f-famille');
  if(fLatin && latin){ fLatin.value = latin; if(typeof checkLatinField === 'function') checkLatinField(fLatin); }
  if(fNom   && common) fNom.value = common;
  if(fFam   && fam){ fFam.value = fam; if(typeof checkFamilleField === 'function') checkFamilleField(fFam); }
  if(btn){ btn.textContent = '✅ Appliqué !'; btn.style.background = 'rgba(74,184,112,.35)'; }
  setTimeout(function(){ closePlantnetModal(); }, 700);
}
