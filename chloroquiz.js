...
function selectLevel(n){
  ...
  // N0 : cacher les filtres végétaux (non pertinents pour la découverte)
  const filterSection=document.querySelector('.filter-section');
  if(filterSection) filterSection.style.display=n===0?'none':'';
  // Fermer les panels ouverts si on passe en N0
  if(n===0){
    const panel=document.getElementById('filter-chips-panel');
    if(panel){ panel.style.display='none'; panel.dataset.openField=''; }
    ['feuillage','exposition','type','famille','classe','usage','couleur','taille','rusticite'].forEach(k=>{
  ...
  if(qPhase==='photo'){
    if(qPhotoType==='spelling'){
      ...
      const opts=qSQ.options.map(opt=>{
        ...
        return `<button class="${cls}" ${dis} onclick="pickPhotoSpelling(this)" style="font-style:italic;font-family:'Playfair Display',serif;font-size:16px;letter-spacing:0.4px">${opt}</button>`;
      }).join('');
...
function showCelebration(isOk,isTimeout){
  ...
  if(isOk){
    ...
    root.innerHTML=confettiHTML()+`<div class="overlay"><div class="cel-card good"><div class="cel-glow"></div><span class="cel-emoji">🎉</span><div class="cel-title">Félicitations ${qName} !<br>Bonne réponse !</div>${fmtMsgOk}<div class="cel-latin">${italLatin(p.latin)}</div><div class="cel-common">${p.nom}</div><button class="cel-btn" onclick="closeCelebration()">Continuer →</button></div></div>`;
    qDicteeFormatMsg = null;
  } else if(isTimeout){
    root.innerHTML=`<div class="overlay"><div class="cel-card timeout"><span class="cel-emoji">⏰</span><div class="cel-title">Temps écoulé !</div><p style="color:rgba(200,223,204,.45);font-size:13px;margin-bottom:16px">Pas de réponse en 30 secondes</p><div class="wrong-box"><div class="wrong-box-label">La bonne réponse était</div><div class="wrong-box-latin">${italLatin(p.latin)}</div><div class="wrong-box-common">${p.nom}</div></div><button class="cel-btn" onclick="closeCelebration()">Continuer →</button></div></div>`;
  } else {
    ...
    root.innerHTML=`<div class="overlay"><div class="cel-card bad"><span class="cel-emoji">😕</span><div class="cel-title">Pas cette fois…</div><p style="color:rgba(200,223,204,.45);font-size:13px;margin-bottom:12px">Tu avais répondu : <strong style="color:#ff9999">${qPhotoSel}</strong></p>${fmtMsg}<div class="wrong-box"><div class="wrong-box-label">La bonne réponse était</div><div class="wrong-box-latin">${italLatin(p.latin)}</div><div class="wrong-box-common">${p.nom}</div></div><button class="cel-btn" onclick="closeCelebration()">Continuer →</button></div></div>`;
    qDicteeFormatMsg = null;
  }
}
