...
function showFicheComplete(id){
  ...
  function row(label, val){ return val ? `<div class="minfo"><div class="minfo-lbl">${label}</div><div class="minfo-val">${val}</div></div>` : ''; }
  ...
  const photoHTML = allPhotos.length
    ? `<div style="display:grid;grid-template-columns:${allPhotos.length>=3?'1fr 1fr 1fr':'1fr 1fr'};gap:8px;margin:12px 0">`
      + allPhotos.map(ph=>`<img src="${ph}" class="modal-img" style="margin:0;height:160px" onerror="this.style.display='none'" loading="lazy"/>`).join('')
      + '</div>'
    : `<div class="modal-img-fb">${LEAF_SVG}</div>`;
