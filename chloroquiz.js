...
async function savePasswords(newUser, newAdmin){
  ...
  // Sauvegarde cloud avec retry x3
  if(cloudOk){
    var saved=false;
    for(var _r=0;_r<3;_r++){
      try{
        await sbPost('floralab_data',{key:'passwords',value:obj,updated_at:new Date().toISOString()},'resolution=merge-duplicates');
        saved=true; break;
      } catch(e){
        if(_r<2) await new Promise(function(res){setTimeout(res,1000);});
        else console.warn('Cloud save passwords fail after 3 attempts',e);
      }
    }
    return saved;
  }
...
function logout(){
  stopAllPolls(); role=null; cloudOk=false;
  window._plantsSessionLoaded = false; // forcer rechargement au prochain login
  window._photosSessionLoaded = false; // idem photos
  window._scoresLoaded = null;
...
function showPage(p){
  ...
  // Sync bottom nav
  ['home','quiz','fiches','scores','myplants'].forEach(id=>{
  ...
  c.innerHTML=`
    <div class="name-prompt">
      ...
      <div class="quiz-inputs-row">
        <input class="quiz-input-name" id="player-name-input" placeholder="Prénom…" autocomplete="off" maxlength="30"/>
        <input class="quiz-input-init" id="player-initial-input" placeholder="AB" autocomplete="off" maxlength="2"/>
      </div>
...
document.addEventListener('visibilitychange', function(){
  if(document.hidden){
    ...
    if(pageId === 'page-stats' && role === 'admin' && cloudOk){
      if(statsPollInterval) clearInterval(statsPollInterval);
...
function switchAdminTab(tab){
  ['plantes','cloud','mdp','plantnet','multi','glossaire'].forEach(t=>{
...
async function changePasswords(){
  ...
  const ok = await savePasswords(newUser, newAdmin);
  if(!ok && cloudOk){ alert('⚠️ Mots de passe sauvegardés localement seulement.\nLa synchronisation cloud a echoue (egress Supabase).\nReessayez dans quelques minutes.'); }
...
async function showSessionWaitRoom(code, playerName, questions){
  ...
  // Poll pour détecter le démarrage — toutes les 2s pour ne pas rater le signal
  var pollErrors = 0;
  ...
  sessionPollI = setInterval(async ()=>{
    try{
      ...
      pollErrors = 0; // reset erreurs sur succès
      const statusEl = document.getElementById('waitroom-status');
      if(!rows.length||rows[0].status==='ended'){
        clearInterval(sessionPollI);
        c.innerHTML='<div style="text-align:center;padding:40px;color:rgba(200,223,204,.4)">Session terminée.</div>';
        return;
      }
      if(rows[0].status==='playing'){
        clearInterval(sessionPollI);
        if(statusEl) statusEl.textContent = 'Chargement des questions…';
        // Recharger les questions fraîches depuis Supabase
        startSessionQuiz(code, playerName, null, null);
      }
    } catch(e){
      pollErrors++;
      const statusEl = document.getElementById('waitroom-status');
      if(statusEl) statusEl.textContent = 'Reconnexion… ('+(pollErrors)+')';
    }
  }, 2000);
}
...
async function pickSessionAnswer(btn, answer, code){
  ...
  sessionQIdx++;
  // Envoyer mise à jour avec retry automatique
  var _patchData = {score:sessionScore.ok,total:sessionScore.total,current_q:sessionQIdx,done:sessionQIdx>=sessionQuestions.length,updated_at:new Date().toISOString()};
  var _patchFilter = 'session_code=eq.'+code+'&player_name=eq.'+encodeURIComponent(sessionPlayerName);
  (async function(){ for(var _t=0;_t<3;_t++){ try{ await sbPatch('floralab_session_players',_patchFilter,_patchData); break; }catch(e){ if(_t===2) cqErr('PATCH score failed:',e); await new Promise(function(r){setTimeout(r,800);}); } } })();
  // Bouton suivant
...
async function onSessionTimeout(code){
  ...
  var _ptData = {score:sessionScore.ok,total:sessionScore.total,current_q:sessionQIdx,done:sessionQIdx>=sessionQuestions.length,updated_at:new Date().toISOString()};
  var _ptFilter = 'session_code=eq.'+code+'&player_name=eq.'+encodeURIComponent(sessionPlayerName);
  (async function(){ for(var _t=0;_t<3;_t++){ try{ await sbPatch('floralab_session_players',_ptFilter,_ptData); break; }catch(e){ await new Promise(function(r){setTimeout(r,800);}); } } })();
  renderSessionQuestion(code);
}
