...
function renderNamePrompt(c){
  ...
  c.innerHTML=`
    <div class="name-prompt" style="max-width:560px;margin:0 auto;text-align:center;padding:16px 0">
      ...
      <div class="qtype-section" style="margin-top:12px">
        <div style="font-size:10px;text-transform:uppercase;letter-spacing:1.5px;color:rgba(200,223,204,.4);margin-bottom:8px">
          🎯 Types de questions <span style="font-size:9px;color:rgba(200,223,204,.25);text-transform:none;letter-spacing:0">(tout activé par défaut)</span>
        </div>
        <div id="qtype-chips" style="display:flex;gap:6px;flex-wrap:wrap">
          ${[
            {id:'photo',   label:'📸 Photo',        keys:['photo']},
            {id:'morph',   label:'🌿 Morphologie',  keys:['feuillage','port','hauteurAdulte','largeurAdulte','vitesseCroissance','couleurLimbe','formeLimbe','texture']},
            {id:'milieu',  label:'☀️ Milieu & sol',  keys:['exposition','rusticite','ph','humidite','structureSol']},
            {id:'floraison',label:'🌸 Floraison',   keys:['periodeFloraison','couleurFleurs']},
            {id:'botanique',label:'🔬 Botanique',   keys:['famille','classe','reproduction','pollinisation']},
            {id:'usage',   label:'🏡 Usage',         keys:['usageAmenagement','interetOrnemental','autresInterets','biodiversite','typeTaille','frequenceTaille']},
            {id:'type',    label:'🌱 Type végétal',  keys:['type']},
          ].map(g=>`<button id="qtype-${g.id}" data-keys="${g.keys.join(',')}"
            onclick="toggleQType(this)"
            style="padding:6px 13px;background:rgba(74,184,112,.18);border:1px solid rgba(74,184,112,.4);
            border-radius:20px;color:var(--g4);font-family:'DM Sans',sans-serif;font-size:12px;
            font-weight:600;cursor:pointer;transition:all .2s">${g.label}</button>`).join('')}
        </div>
        <div id="qtype-hint" style="font-size:10px;color:rgba(200,223,204,.3);margin-top:5px">
          Cliquer pour désactiver un type · Tout actif = quiz complet selon le niveau
        </div>
      </div>
...
function selectLevel(n){
  ...
  // N0 : cacher toute la section filtres
  const filterSection=document.querySelector('.filter-section');
  if(filterSection) filterSection.style.display=n===0?'none':'';
  if(n===0){
    clearAllFilters();
    // Cacher aussi la section types de questions (non pertinente pour N0)
    const qtSection=document.querySelector('.qtype-section');
    if(qtSection) qtSection.style.display='none';
    const dictToggle=document.querySelector('.dictee-toggle-wrap');
    if(dictToggle) dictToggle.style.display='none';
    return;
  }
  // Réafficher les sections pour N1+
  const qtSection=document.querySelector('.qtype-section');
  if(qtSection) qtSection.style.display='';
  const dictToggle=document.querySelector('.dictee-toggle-wrap');
  if(dictToggle) dictToggle.style.display='';

  // N1+: afficher/cacher les filtres selon le niveau
  // N1: feuillage, exposition, type, couleur, taille, rusticite
  // N2: + usage
  // N3+: + famille, classe
  ...
  allCats.forEach(k=>{
    ...
    if(b){
      ...
      // Si le filtre actif n'est plus visible → le réinitialiser
      if(!show && qFilter[k]){
        qFilter[k]='';
  ...
  // Réinitialiser le panel si le champ ouvert n'est plus visible
  if(panel && panel.dataset.openField && !visible.includes(panel.dataset.openField)){
    panel.style.display='none'; panel.dataset.openField='';
  }
}

// ── Types de questions : activer/désactiver des groupes de champs ──
function toggleQType(btn){
  const keys = btn.dataset.keys.split(',');
  const isActive = btn.style.background.includes('0.18') || btn.style.background.includes('.18');
  if(isActive){
    // Désactiver
    btn.style.background='rgba(74,184,112,.04)';
    btn.style.borderColor='rgba(74,184,112,.15)';
    btn.style.color='rgba(200,223,204,.3)';
    keys.forEach(k=>qActiveFields.add(k));
  } else {
    // Réactiver
    btn.style.background='rgba(74,184,112,.18)';
    btn.style.borderColor='rgba(74,184,112,.4)';
    btn.style.color='var(--g4)';
    keys.forEach(k=>qActiveFields.delete(k));
  }
  // Update hint
  const hint=document.getElementById('qtype-hint');
  if(!hint) return;
  if(qActiveFields.size===0){
    hint.textContent='Tout actif = quiz complet selon le niveau';
    hint.style.color='rgba(200,223,204,.3)';
  } else {
    const disabled=[...document.querySelectorAll('#qtype-chips button')].filter(b=>b.style.color.includes('0.3')).map(b=>b.textContent.trim());
    hint.textContent='Désactivé : '+disabled.join(', ');
    hint.style.color='rgba(255,150,80,.6)';
  }
}
...
function buildKQ(){
  ...
  // Filtrer selon les types de questions actifs (qActiveFields vide = tout actif)
  const avail=fields.filter(f=>p[f.key]&&p[f.key].trim()&&(qActiveFields.size===0||!qActiveFields.has(f.key)));
...
function closeCelebration(){
  ...
  // N0 Découverte : pas d'étape 2 — passer directement à la question suivante
  if(qLevel===0){ nextRound(); return; }
...
function buildKnowledgeQuestion(p, pool, level, fieldKeys){
  ...
  const avail = fields.filter(f=>p[f.key]&&p[f.key].trim()&&(!allowedKeys||allowedKeys.includes(f.key))&&(qActiveFields.size===0||!qActiveFields.has(f.key)));
