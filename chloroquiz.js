...
function launchComparisonFull(id1, id2){
  ...
  const SECTIONS=[
    ...
    {titre:'🍃 Feuillage', keys:[
      {key:'feuillage',label:'Comportement'},{key:'couleurLimbe',label:'Couleur limbe'},
  ...
  function colFull(p, other){
    ...
    SECTIONS.forEach(sec=>{
      ...
      rows.forEach(f=>{
        ...
        html+='<div class="cmp-row" style="'+(bg?'background:'+bg:'')+'"><span class="cmp-lbl">'+f.label+'</span><span class="cmp-val" style="color:'+color+'">'+( v||'—')+'</span></div>';
      });
    });
    return html;
  }
