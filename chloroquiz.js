...
const nextId=()=>Math.max(0,...plants.map(p=>p.id))+1;
function fmtDate(ts){
...
function showCelebration(isOk,isTimeout){
  ...
  const root=document.getElementById('overlay-root');
  if(isOk){
    root.innerHTML=confettiHTML()+`<div class="overlay"><div class="cel-card good"><div class="cel-glow"></div><span class="cel-emoji">🎉</span><div class="cel-title">Félicitations ${qName} !<br>Bonne réponse !</div><div class="cel-latin">${p.latin}</div><div class="cel-common">${p.nom}</div><button class="cel-btn" onclick="closeCelebration()">Continuer →</button></div></div>`;
  } else if(isTimeout){
    ...
    root.innerHTML=`<div class="overlay"><div class="cel-card bad"><span class="cel-emoji">😕</span><div class="cel-title">Pas cette fois…</div><p style="color:rgba(200,223,204,.45);font-size:13px;margin-bottom:16px">Tu avais répondu : <strong style="color:#ff9999">${qPhotoSel}</strong></p><div class="wrong-box"><div class="wrong-box-label">La bonne réponse était</div><div class="wrong-box-latin">${p.latin}</div><div class="wrong-box-common">${p.nom}</div></div><button class="cel-btn" onclick="closeCelebration()">Continuer →</button></div></div>`;
  }
}
...
function submitDictee(){
  ...
  if(!val) return;
  stopTimer();
  var ok = val.toLowerCase() === qRound.correct.latin.toLowerCase();
  if(ok){ qScore.ok++; qStreak++; } else { qStreak=0; hapticWrong(); }
  qScore.total++;
  qPhotoSel = val; // marquer comme répondu
  qHistory.push({latin:qRound.correct.latin, nom:qRound.correct.nom,
...
function renderFiches(){
  ...
  const list=plants.filter(p=>[p.latin,p.nom,p.famille,p.type].some(v=>(v||'').toLowerCase().includes(q)));
  ...
  document.getElementById('overlay-root').innerHTML=`
    <div class="form-bg" onclick="if(event.target===this)closeForm()">
      <div class="form-card">
        ...
        <div class="form-grid">
          <div class="fg full"><label class="fl">Nom latin (botanique) *</label><input class="fi" id="f-latin" placeholder="Lavandula angustifolia" value="${v('latin')}"/></div>
...
async function savePlantForm(){
  const latin = document.getElementById('f-latin').value.trim();
  ...
  try{
    ...
    list.innerHTML = rows.map((r,i)=>{
      ...
      if(r.done){
        ...
        statusHtml = `<span class="mon-status wait">▶ En cours — Q${r.current_q||0} · ${r.score||0} pt(s)</span>`;
      } else {
      ...
      return `<div class="mon-row ${r.done?'mon-done':''}">
        ...
        <div class="mon-right">
          ...
          <div class="mon-q">Q ${r.current_q||0}</div>
        </div>
...
async function submitSessionDictee(code){
  ...
  if(wrap){
    ...
    fb.innerHTML = ok
      ...
      : '❌ La réponse était : <em style="color:var(--g4)">'+q.correct+'</em>';
  ...
  try{
    ...
    lb.innerHTML = rows.map((r,i)=>`
      <div class="session-mini-row ${r.player_name===sessionPlayerName?'me':''}">
        ...
        <span class="session-mini-prog">Q${r.current_q||0}/20</span>
      </div>`).join('');
  } catch(e){}
}
