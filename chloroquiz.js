...
async function repairPhotosCloud(){
  ...
  // Lire le localStorage — il a toutes les photos originales
  let localPlants = [];
  ...
  // Compter les photos récupérables
  const withPhotos = localPlants.filter(p=>p.photo||p.photo2||p.photo3||p.photo4);
  if(!confirm('🔧 Restaurer les photos depuis le cache local vers Supabase ?\n\n'+withPhotos.length+' plante(s) avec photos trouvées en local.\n\nContinuer ?')) return;

  // Fusionner : photos locales → plants en mémoire
  // (le localStorage peut être partiel si on est sur mobile — on complète ce qu'on peut)
  ...
  // Sauvegarder (strip base64, garder URLs)
  try{
    await savePlants();
    renderAdmin();
    alert('✅ Sauvegarde effectuée sur Supabase !\n\n🌿 '+plants.length+' végétaux au total\n📸 '+recovered+' avec photos\n\nSi certaines photos manquent encore, refaire depuis l\'appareil qui les avait (PC ou autre mobile).');
  } catch(e){
