  ...
  if(cloudOk){
    ...
    try{
      ...
      if(rows.length){
        ...
        if(photosRestored > 0){
          console.log('[CQ] '+photosRestored+' photo(s) restaurées depuis localStorage → Supabase');
          // Sauvegarde directe en arrière-plan (sans strip base64)
          setTimeout(async function(){
            try{
              await sbPost('floralab_data',{key:'plants',value:plants,updated_at:new Date().toISOString()},'resolution=merge-duplicates');
              console.log('[CQ] Photos restaurées sauvegardées sur Supabase');
            } catch(e){ console.warn('Photo repair save failed',e); }
          }, 2000);
        }
...
async function savePlants(){
  ...
  if(cloudOk){
    try{
      // Envoi complet vers Supabase (photos incluses)
      // savePlants est appelé rarement (action admin explicite) — pas d'impact egress significatif
      await sbPost('floralab_data',{key:'plants',value:plants,updated_at:new Date().toISOString()},'resolution=merge-duplicates');
      flashSaved();
...
async function confirmCSVImport(rows){
  ...
  if(cloudOk){
    try{
      // Envoi complet (photos incluses) — import CSV = action ponctuelle
      await sbPost('floralab_data',{key:'plants',value:plants,updated_at:new Date().toISOString()},'resolution=merge-duplicates');
      cloudSaved = true;
      flashSaved();
    } catch(e){
