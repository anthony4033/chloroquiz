...
function executeStopQuiz(){
  ...
  // Enregistrer dans les stats incomplètes (localStorage uniquement, pas classement)
  if(qScore.total > 0){
    const elapsed = Math.round((Date.now() - qStartTime) / 1000); // secondes
    const inc = {
      name: qName,
      level: qLevel,
      ok: qScore.ok,
    ...
    try{
      const key = SK_INCOMPLETE;
      let arr = JSON.parse(localStorage.getItem(key) || '[]');
      arr.push(inc);
      // Garder les 200 derniers max
...
function renderStats(){
  ...
  const incompleteHTML=incompletes.length?`
    <div class="stats-collective" style="margin-top:20px;border-color:rgba(255,140,0,.2);background:rgba(255,120,0,.04)">
      <div class="stats-collective-title" style="color:rgba(255,160,60,.85)">⏹️ Quiz interrompus — ${incompletes.length} session(s)</div>
      ${incompletes.slice(0,20).map(inc=>`
        <div class="collective-row">
          <span style="font-size:11px;color:rgba(200,223,204,.5)">${new Date(inc.date).toLocaleDateString('fr-FR')} ${new Date(inc.date).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})}</span>
          ...
          <span style="display:flex;gap:6px;flex-wrap:wrap">
            <span style="background:rgba(255,140,0,.12);border:1px solid rgba(255,140,0,.25);border-radius:6px;padding:2px 7px;font-size:11px;color:#ffb347">⏱ ${fmtElapsed(inc.elapsed)}</span>
            <span style="background:rgba(255,80,80,.1);border:1px solid rgba(255,80,80,.2);border-radius:6px;padding:2px 7px;font-size:11px;color:#ff9999">Non terminé</span>
          </span>
        </div>`).join('')}
      ${incompletes.length>20?`<div style="font-size:11px;color:rgba(200,223,204,.3);margin-top:6px">… et ${incompletes.length-20} autres</div>`:''}
      <button onclick="if(confirm('Effacer les quiz interrompus ?')){ localStorage.removeItem('${SK_INCOMPLETE}'); renderStats(); }"
