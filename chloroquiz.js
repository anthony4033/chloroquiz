  ...
  div.innerHTML = ''
    ...
    + (notConfigured
      ? '<div style="text-align:center;padding:24px 16px">'
        ...
        + '</div>'
      : '<div id="pnet-drop" onclick="document.getElementById(\'pnet-file\').click()"'
        +   ' style="border:2px dashed rgba(74,184,112,.35);border-radius:14px;padding:24px;text-align:center;cursor:pointer;margin-bottom:14px"'
        +   ' ondragover="event.preventDefault();this.style.borderColor=\'#4ab870\'"'
        +   ' ondragleave="this.style.borderColor=\'rgba(74,184,112,.35)\'"'
        +   ' ondrop="event.preventDefault();this.style.borderColor=\'rgba(74,184,112,.35)\';_pnetLoadFile(event.dataTransfer.files[0])">'
        +   '<div id="pnet-preview-wrap" style="display:none;margin-bottom:10px">'
        +     '<img id="pnet-preview" style="max-height:160px;max-width:100%;border-radius:10px;object-fit:contain"/>'
        +   '</div>'
        +   '<div id="pnet-drop-label" style="color:rgba(200,223,204,.5);font-size:14px">📷 Cliquer ou glisser une photo</div>'
        +   '<input type="file" id="pnet-file" accept="image/*" style="display:none" onchange="_pnetLoadFile(this.files[0])"/>'
        + '</div>'
        + '<div style="margin-bottom:14px">'
...
function _pnetLoadFile(file){
  if(!file || !file.type.startsWith('image/')) return;
  window._pnetFile = file;
  const reader = new FileReader();
  reader.onload = function(e){
    const img  = document.getElementById('pnet-preview');
    const wrap = document.getElementById('pnet-preview-wrap');
    const lbl  = document.getElementById('pnet-drop-label');
    if(img)  img.src = e.target.result;
    if(wrap) wrap.style.display = 'block';
    if(lbl)  lbl.textContent = '📷 ' + file.name + ' — cliquer pour changer';
  };
  reader.readAsDataURL(file);
}
...
async function _pnetRun(){
  var file = window._pnetFile;
  var res  = document.getElementById('pnet-results');
  if(!file){
    if(res) res.innerHTML = '<div style="color:#ffb347;font-size:13px;text-align:center;padding:12px">📷 Selectionnez une photo</div>';
    return;
  }
  var key = plantnetKey;
  if(!key){
  ...
  try{
    ...
    fd.append('organs', window._pnetOrgan || 'auto');
