  ...
  if(cloudOk){
    ...
    try{
      ...
      if(rows.length){
        ...
        // Restaurer les photos base64 locales sur les plantes cloud
        plants = cloudPlants.map(cp=>{
          const lp = localPlants.find(p=>p.id===cp.id);
          if(lp){
            if(!cp.photo  && lp.photo  && lp.photo.startsWith('data:'))  cp.photo  = lp.photo;
            if(!cp.photo2 && lp.photo2 && lp.photo2.startsWith('data:')) cp.photo2 = lp.photo2;
            if(!cp.photo3 && lp.photo3 && lp.photo3.startsWith('data:')) cp.photo3 = lp.photo3;
            if(!cp.photo4 && lp.photo4 && lp.photo4.startsWith('data:')) cp.photo4 = lp.photo4;
          }
          return cp;
        });
...
async function savePlants(){
  ...
  if(cloudOk){
    try{
      // Supabase : supprimer les photos base64 (trop lourdes, restent en localStorage)
      // Seules les URLs http sont envoyées au cloud
...
async function confirmCSVImport(rows){
  ...
  if(cloudOk){
    try{
      // Strip base64 avant envoi cloud (même logique que savePlants)
