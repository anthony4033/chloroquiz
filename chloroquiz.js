  ...
  if(cloudOk){
    ...
    try{
      ...
      if(rows.length){
        ...
        // Si des photos ont été restaurées depuis le local, re-sauvegarder vers Supabase
        if(photosRestored > 0){
          console.log('[CQ] Restauration de '+photosRestored+' photo(s) depuis localStorage vers Supabase');
          // Sauvegarde silencieuse en arrière-plan
          setTimeout(async function(){
            try{ await savePlants(); } catch(e){ console.warn('Repair save failed',e); }
          }, 2000);
        }
      } else plants = JSON.parse(JSON.stringify(DEFAULT_PLANTS));
      window._plantsSessionLoaded = true;
      localSavePlants(); // mettre en cache local pour usage offline
...
async function savePlants(){
  ...
  if(cloudOk){
    try{
      // Supabase : supprimer uniquement les photos BASE64 (trop lourdes)
      // Les URLs http sont conservées intactes
      const plantsForCloud = plants.map(p=>({
        ...p,
        photo:  (p.photo  && p.photo.startsWith('data:'))  ? '' : (p.photo  || ''),
        photo2: (p.photo2 && p.photo2.startsWith('data:')) ? '' : (p.photo2 || ''),
        photo3: (p.photo3 && p.photo3.startsWith('data:')) ? '' : (p.photo3 || ''),
        photo4: (p.photo4 && p.photo4.startsWith('data:')) ? '' : (p.photo4 || ''),
      }));
      await sbPost('floralab_data',{key:'plants',value:plantsForCloud,updated_at:new Date().toISOString()},'resolution=merge-duplicates');
      flashSaved();
...
async function repairPhotosCloud(){
  ...
  // Compter les photos récupérables (URL et base64)
  ...
  const msg = '🔧 Restaurer les photos vers Supabase ?\n\n'
    + '📸 '+withPhotos.length+' plante(s) avec photos\n'
    + (withB64.length ? '⚠️ '+withB64.length+' plante(s) avec photos compressées (base64)\n   → envoyées telles quelles cette fois\n' : '')
    + '\nContinuer ?';
  ...
  try{
    // Envoi COMPLET vers Supabase — base64 incluses (réparation ponctuelle)
    await sbPost('floralab_data',{
      key:'plants',
      value:plants, // pas de strip base64 ici
      updated_at:new Date().toISOString()
    },'resolution=merge-duplicates');
    localSavePlants();
    flashSaved();
    renderAdmin();
    alert('✅ Photos restaurées sur Supabase !\n\n📸 '+recovered+' plante(s) avec photos\n\nLes apprentis verront les photos au prochain chargement.');
  } catch(e){
    // Si trop lourd (base64), fallback sans base64
    if(e.message&&e.message.includes('413')){
      alert('⚠️ Fichier trop lourd pour Supabase (photos base64).\n\nConseille : utilise des URLs (liens Wikimedia) plutôt que des photos importées depuis le téléphone pour partager avec les apprentis.');
    } else {
...
async function confirmCSVImport(rows){
  ...
  localSavePlants(); // sauvegarde locale immédiate comme filet de sécurité
  var cloudSaved = false;
  if(cloudOk){
    try{
      // Strip uniquement les photos BASE64, conserver les URLs
      const plantsCloud = plants.map(p=>({...p,
        photo:  (p.photo  && p.photo.startsWith('data:'))  ? '' : (p.photo  || ''),
        photo2: (p.photo2 && p.photo2.startsWith('data:')) ? '' : (p.photo2 || ''),
        photo3: (p.photo3 && p.photo3.startsWith('data:')) ? '' : (p.photo3 || ''),
        photo4: (p.photo4 && p.photo4.startsWith('data:')) ? '' : (p.photo4 || ''),
      }));
      await sbPost('floralab_data',{key:'plants',value:plantsCloud,updated_at:new Date().toISOString()},'resolution=merge-duplicates');
      cloudSaved = true;
      flashSaved();
    } catch(e){
...
async function savePlantForm(){
  ...
  if(!_myPlantMode && editingId && editingId>=90000){
    ...
    saveDrafts(); editingId=null; closeForm(); renderAdminList();
