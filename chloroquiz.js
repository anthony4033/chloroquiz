...
function buildSearchEngine(q){
  ...
  function plantMatchesToken(p, tok){
    ...
    if(TYPE_VALS.has(tok)){
      ...
      return plantType.split(',').map(s=>s.trim()).some(t=>t===tok||t.includes(tok)||tok.includes(t));
    }
...
function sharePlantModal(id){
  ...
  root.innerHTML=`<div class="modal-bg" onclick="if(event.target===this)closeModal()">
    <div class="modal" style="max-width:420px">
      <div style="padding:20px 52px 0 20px;position:relative">
        <button class="modal-close" onclick="closeModal()">✕</button>
        <div style="font-family:var(--disp);font-size:18px;font-weight:900;color:var(--cr)">📤 Partager — <em style="font-style:italic;color:var(--am2)">${p.latin}</em></div>
