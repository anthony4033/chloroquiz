...
function renderPlantnetTab(){
  const el = document.getElementById('plantnet-tab-content');
  if(!el) return;
  const configured = !!plantnetKey;
  el.innerHTML = ''
    ...
    +     '<div style="font-size:14px;color:rgba(200,223,204,.8);line-height:1.6;margin-bottom:16px">'
    +       'La clé API Pl@ntNet permet aux apprentis d\'identifier des plantes par photo.<br>'
    +       'Elle est stockée dans le cloud et <strong style="color:#4ab870">jamais visible par les apprentis</strong>.'
    +     '</div>'
    +     (configured
       ? '<div style="display:flex;align-items:center;gap:8px;margin-bottom:16px;padding:10px 14px;background:rgba(74,184,112,.08);border:1px solid rgba(74,184,112,.2);border-radius:10px">'
         +   '<span style="font-size:18px">✅</span>'
         +   '<span style="color:#4ab870;font-size:13px;font-weight:700">Clé configurée et active</span>'
         + '</div>'
       : '<div style="display:flex;align-items:center;gap:8px;margin-bottom:16px;padding:10px 14px;background:rgba(232,160,32,.08);border:1px solid rgba(232,160,32,.2);border-radius:10px">'
         +   '<span style="font-size:18px">⚠️</span>'
         +   '<span style="color:var(--am2);font-size:13px">Aucune clé configurée — identification désactivée</span>'
         + '</div>'
     )
    +     '<label style="font-size:12px;color:rgba(200,223,204,.45);display:block;margin-bottom:6px">'
    +       (configured ? '🔄 Modifier la clé API' : '🔑 Saisir la clé API') + ' <em style="font-size:10px">(my.plantnet.org)</em>'
    +     '</label>'
    +     '<div style="display:flex;gap:8px">'
    +       '<input type="password" id="pnet-admin-key" placeholder="2b10a…" autocomplete="new-password"'
    ...
    +     '🔗 Clé gratuite sur <strong>my.plantnet.org</strong> → Mon compte → API · '
...
async function _pnetRun(){
  const file = window._pnetFile;
  const res  = document.getElementById('pnet-results');
  if(!file){
    if(res) res.innerHTML = '<div style="color:#ffb347;font-size:13px;text-align:center;padding:12px">📷 Sélectionnez d\'abord une photo</div>';
    return;
  }
  const keyInp = document.getElementById('pnet-key-input');
  const key = plantnetKey || (keyInp ? keyInp.value.trim() : '');
  if(!key){
    if(res) res.innerHTML = '<div style="color:#ffb347;font-size:13px;padding:12px">🔑 Clé API manquante — renseigner votre clé ci-dessous</div>';
    return;
  }
  const btn = document.getElementById('pnet-btn');
  if(btn){ btn.disabled = true; btn.textContent = '⏳ Identification en cours…'; }
  if(res) res.innerHTML = '<div style="text-align:center;padding:20px;color:rgba(200,223,204,.4);font-size:13px">⏳ Analyse de l\'image…</div>';
  try{
    const fd = new FormData();
    fd.append('images', file);
    fd.append('organs', window._pnetOrgan || 'auto');
    const r = await fetch('https://my-api.plantnet.org/v2/identify/all?lang=fr&api-key=' + encodeURIComponent(key), {method:'POST', body:fd});
    if(r.status === 403){ throw new Error('bad_key'); }
    if(r.status === 404){ throw new Error('no_result'); }
    if(!r.ok){ throw new Error('api_error'); }
    const data = await r.json();
    const results = (data.results || []).slice(0, 5);
    if(!results.length){ throw new Error('no_result'); }
    window._pnetResults = results;
    let html = '<div style="font-size:12px;color:rgba(200,223,204,.35);margin-bottom:10px">' + results.length + ' résultat(s) — cliquer sur Utiliser pour pré-remplir le formulaire</div>';
    results.forEach(function(r2, i){
      ...
      const pct    = Math.round((r2.score || 0) * 100);
      const barCol = pct >= 70 ? '#4ab870' : pct >= 40 ? '#f5a623' : '#e84040';
    ...
    let msg = "❌ Erreur lors de l'identification";
    if(err.message === 'no_result') msg = "🔍 Aucune plante reconnue — essayez une autre photo ou un autre organe";
    if(err.message === 'bad_key')   msg = "🔑 Clé API invalide — vérifiez votre clé ci-dessous";
