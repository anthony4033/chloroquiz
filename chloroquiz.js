...
function downloadCSVTemplate(){
  // Ligne 1 : labels lisibles, Ligne 2 : hints/exemples, Ligne 3+ : données existantes
  const esc = v => '"' + String(v||'').replace(/"/g,'""') + '"';
  const header = CSV_COLUMNS.map(c=>esc(c.label)).join(';');
  const hints  = CSV_COLUMNS.map(c=>esc(c.hint)).join(';');
  const rows   = plants.length
    ? plants.map(p=>CSV_COLUMNS.map(col=>esc(p[col.key]||'')).join(';')).join('\n')
    : '';
  const bom='\uFEFF'; // BOM UTF-8 pour Excel
  const content = bom + [header, hints, rows].filter(Boolean).join('\n');
