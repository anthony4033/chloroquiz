...
async function renderScores(){
  ...
  // Recharger si scores vide ou cache expiré
...
async function renderStats(){
  ...
  if(cloudOk){
    const [resScores, resAbandoned, resPlayers, resSessions] = await Promise.allSettled([
      ...
      sbGet('floralab_sessions','select=code,level,created_at,status&order=created_at.desc&limit=50')
    ]);
    ...
    if(resSessions.status==='fulfilled') multiSessionsMeta=resSessions.value;
  }
