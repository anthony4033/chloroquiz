...
function openMyPlantForm(id){
  ...
  // Réutiliser openForm mais avec un flag "myplant"
  editingId = id!==null ? id : null;
  _myPlantMode = true;
  openForm(id!==null ? existing : null);
}
...
function buildPlantFromForm(){
  ...
  return {
    ...
    photo:document.getElementById('f-photo-url') ? document.getElementById('f-photo-url').value.trim() : '',
...
async function savePlantForm(){
  ...
  if(_myPlantMode){
    ...
    plant.latin=latin;
    if(plant.id && myPlants.some(p=>p.id===plant.id)){
      // Mise à jour
      const idx=myPlants.findIndex(p=>p.id===plant.id);
