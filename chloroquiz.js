...
async function confirmCSVImport(rows){
  // Lire les selects pour les doublons
  ...
  for(const {plant,action} of toProcess){
    try{
      if(action==='add'){
        // Assigner un ID unique
        const maxId = plants.reduce((m,p)=>Math.max(m,p.id||0),0);
        plant.id = maxId+1+added+updated;
        if(cloudOk){
          await sbPost('floralab_data', {id:1, value:[...plants, plant]}, 'resolution=merge-duplicates');
          plants.push(plant);
        } else {
          plants.push(plant);
          savePlantsLocal();
        ...
        const idx = plants.findIndex(p=>(p.latin||'').toLowerCase()===(plant.latin||'').toLowerCase());
        if(idx>=0){
          plant.id = plants[idx].id;
          plants[idx]={...plants[idx],...plant};
          if(cloudOk){
            await sbPost('floralab_data',{id:1,value:plants},'resolution=merge-duplicates');
          } else { savePlantsLocal(); }
          updated++;
        }
      }
    } catch(e){ errors++; cqErr('CSV import error:',e); }
  }

  // Sauvegarder en cloud si plusieurs plantes
  if(cloudOk && toProcess.length>1){
    try{ await sbPost('floralab_data',{id:1,value:plants},'resolution=merge-duplicates'); }catch(e){}
  }

  renderAdminList();
  alert(`✅ Import terminé !\n✨ ${added} ajoutée(s)  ✏️ ${updated} mise(s) à jour${errors?' ❌ '+errors+' erreur(s)':''}`);
}
