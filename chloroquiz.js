...
async function loadPhotosCloud(){
  if(!cloudOk) return;
  try{
  ...
  return new Promise(resolve=>{
    ...
    reader.onload = ev=>{
      ...
      img.onload = ()=>{
        ...
        const url = document.getElementById('f-photo-url'+sfx);
      // (champ URL supprimé)
        const zone = document.getElementById('photo-zone'+sfx);
        const icon = document.getElementById('photo-zone-icon'+sfx); if(icon) icon.remove();
        let prev = document.getElementById('photo-preview'+sfx);
...
async function startSessionLobby(code){
  ...
  // Poll toutes les 3s
  if(multiPollInterval) clearInterval(multiPollInterval);
  multiPollInterval = setInterval(()=>pollSessionPlayers(code), 5000);
...
async function renderSessionMonitor(code){
  ...
  multiPollInterval = setInterval(()=>pollMonitor(code), 5000);
  pollMonitor(code);
}
...
async function startSessionQuiz(code, playerName, questions, level){
  ...
  if(cloudOk){
    try{
      const photoRows = await sbGet('floralab_photos','select=plant_id,photo,photo2,photo3,photo4');
