...
// ── Export sauvegarde ──
function saveMyPlantsToFile(){
  if(!myPlantsOwner) return;
  if(!myPlants.length){ showToast('Aucune fiche à sauvegarder.'); return; }
  const data = {
  ...
  a.href=url; a.download='mes_vegetaux_'+safeName+'_'+new Date().toISOString().slice(0,10)+'.json'; a.click();
  URL.revokeObjectURL(url);
  showToast('💾 Sauvegarde téléchargée — garde ce fichier précieusement !');
}
