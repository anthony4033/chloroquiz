...
function downloadCSVTemplate(){
  ...
  const rows   = plants.length
    ...
    : '';
  // Pas de BOM — UTF-8 pur, compatible Android/Google Sheets et Excel
  // Le séparateur ; est reconnu par Excel FR et Google Sheets
  const content = [header, hints, rows].filter(Boolean).join('\n');
  // Encoder en UTF-8 via TextEncoder pour éviter toute corruption
