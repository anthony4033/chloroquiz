...
## v32 — Formulaires enrichis · Couleurs nettoyées · Quiz corrigé (2026-03-19)

### 📋 Formulaires (admin + Mes végétaux) — nouveaux champs
- **Type de végétal** : ajout de `Vivace herbacée` (avec entrée glossaire)
- **Disposition des feuilles** : nouveau champ avec 7 chips — Rosette · Alternes · Opposées · Opposées décussées · Verticillées · Décussées · Hélicoïdale
- **Forme du limbe** : ajout de `Rubanée`
- Le champ "Disposition des feuilles" est affiché dans la fiche modale

### 📖 Glossaire — 10 nouvelles entrées
Vivace herbacée · Disposition des feuilles · Rosette · Alternes · Opposées · Opposées décussées · Verticillées · Décussées · Hélicoïdale · Rubanée
Chaque terme défini avec exemples botaniques

### 🎨 Couleurs de floraison — quasi-doublons supprimés
Supprimés de `COULEURS_FLEURS` : Ivoire (≈ Blanc crème) · Jaune pâle (≈ Jaune) · Saumon (≈ Orange pâle) · Rose vif · Magenta · Rouge vif · Lilas (≈ Mauve) · Violet foncé · Indigo (≈ Bleu foncé) · Bronze (≈ Brun) · Rose & Blanc (doublon Blanc & Rose) · Violet & Blanc (doublon Blanc & Violet)
Supprimés de `COULEURS_LIMBE` : Vert bleuté (≈ Vert glauque) · Vert bronzé (≈ Bronze/Cuivré)

### 🧠 Logique quiz — correction ambiguïté couleurs
Quand une plante a plusieurs couleurs (ex: "Violet, Mauve"), toutes les valeurs valides sont exclues des mauvaises réponses — "Mauve" ne peut plus apparaître comme mauvaise réponse si c'est aussi une couleur de la plante.
Corrigé dans les 3 blocs : N0, N1–N3 solo, multijoueur.

### 🏆 Classement par niveaux (suite v31)
- Emojis des onglets corrigés : 🏆 Tous · 👀 N0 · 🌱 N1 · 🎓 N2 · 🔬 N3 · 🏗️ N4
- Cohérence avec les cartes niveaux de la page d'accueil du quiz
...
## v32 — suite (2026-03-21)

### 🎨 Ergonomie page Quiz (PC)
- Suppression de la media query 768px qui miniaturisait tout
- Titre "Prêt pour le Quiz ?" : 38px, poids 900
- Inputs prénom/initiales : 20px, padding 15px
- Cartes niveaux : padding 18px, icônes 32px, description 13px
- Section "Révision ciblée" encadrée (carte avec fond + bordure)
- Section "Critères supplémentaires" encadrée
- Section "Dictée" encadrée
- Chips filtres : 13px, padding 8px 14px
- Bouton Démarrer : 19px, padding 18px, poids 800
- Layout 2 colonnes : `55fr / 45fr`, gap 48px, max-width none

### 📋 Formulaire fiches végétaux
- `max-width` formulaire : `width:min(960px, calc(100vw-32px))` adaptatif
- Fix photos 2/3/4 jamais sauvegardées (ID mismatch `f-photo2-url` → `f-photo-url2`)
- Auto-preview des URLs photo à l'ouverture du formulaire
- `onblur` ajouté sur les champs URL photo

### 🌿 Quiz — filtre photos
- Plantes avec **au moins 1 photo** (photo1 à 4) éligibles au quiz
- Solo et multijoueur

### 📤 Import CSV — corrections
- Fix `const plant` → `let plant` (Assignment to constant variable)
- Fix lignes CSV enveloppées dans guillemets globaux
- Fix bouton import : `window._csvImportRows` (plus de JSON inline dans onclick)
- Lecture des selects AVANT `closeModal()` (doublons ignorés à tort)
- Default select doublons : "Ignorer" → "✏️ Mettre à jour"
- Mise à jour = écrasement complet (CSV remplace tout, photos conservées si absentes du CSV)
- Sauvegarde : local d'abord + 1 seul appel Supabase à la fin
- Indicateur de progression pendant l'import

### ⚡ Réduction egress Supabase (-80% estimé)
- `loadPlants` : cache session (1 seul chargement par session)
- `savePlants` : strip base64 avant envoi cloud (photos base64 → localStorage uniquement)
- `loadScores` : cache 5 min
- `renderScores` : cache 3 min
- `addScore` : invalidation cache au lieu de rechargement complet
- `mastery` : 1 seul appel Supabase au lieu de batchs de 20
- `abandoned` : localStorage uniquement
- Stats admin : colonnes sélectives seulement
- Import CSV : strip base64 aussi dans confirmCSVImport

### 🔄 Service Worker v32
- Network-first pour HTML et JS (toujours la dernière version)
- Cache-first pour CSS et assets
- `skipWaiting()` + `clients.claim()` pour prise de contrôle immédiate

---

## v33 — Fix photos persistantes + audit bugs (2026-03-23)

### 🐛 Fix critique : photos disparaissent après déconnexion/reconnexion
**Cause** : `savePlants()` stripait les photos base64 avant envoi Supabase, détruisant les photos à chaque sauvegarde. Le `setTimeout(savePlants)` dans `loadPlants` aggravait le problème en re-strippant après restauration locale.

**Corrections :**
- `savePlants()` : envoi complet vers Supabase sans strip base64 (les saves sont rares, l'egress est négligeable)
- `loadPlants` : auto-repair utilise `sbPost` direct (sans strip) au lieu de `savePlants`
- Import CSV : suppression du strip base64 (même logique)
- `logout()` : reset de `_plantsSessionLoaded` et `_scoresLoaded` pour forcer rechargement propre au prochain login
- `repairPhotosCloud` : envoi direct sans strip (était déjà correct)

### 🐛 Fix : `renderAdminList is not defined` (4 occurrences)
- Toutes les occurrences remplacées par `renderAdmin()` (la vraie fonction)

### 🐛 Fix : photos "Mes végétaux" perdues à la modification
- `buildPlantFromForm()` : récupère les base64 affichées dans les previews si URL vide et pas de nouvel upload
- `savePlantForm()` admin : même correction
- `openMyPlantForm()` : retrait de la plante de `plants[]` décalé à 300ms (après le setTimeout de previewUrl)

### 🔒 Sécurité
- Mots de passe par défaut retirés du code source JS visible (`PW_USER = ''`, `PW_ADMIN = ''`)
- Chargés depuis Supabase au login, fallback localStorage, puis défauts en dernier recours

### 📷 Sélection multiple de photos
- Bouton "📷 Sélectionner jusqu'à 4 photos en une fois" dans formulaire admin et Mes végétaux
- Répartition automatique dans les slots libres 1→2→3→4
- Compression WebP identique à l'upload individuel

### 📁 Mes végétaux — export amélioré
- Nom de fichier clair : `Mes_vegetaux_ChloroQuiz_[prénom]_toutes_fiches_[date].json`
- Mode sélection multiple : bouton ☐ Sélectionner, cases à cocher sur les cartes
- Barre d'actions : Exporter sélection (JSON) · PDF · Tout désélectionner

---

## v34 — Table floralab_photos : photos séparées des données (2026-03-24)

### 🐛 Fix critique : timeout Supabase sur photos base64
71 plantes × 2-4 photos WebP compressées (~100Ko chacune) = 5-15 Mo en une seule requête → timeout Supabase (code 57014).

**Solution : table `floralab_photos` dédiée**
- `floralab_data` ne stocke plus les photos (données texte légères)
- `floralab_photos` stocke les photos par `plant_id`, upsert par batch de 5
- `loadPlants()` charge les données puis fusionne les photos
- `savePlants()` sauvegarde données + photos séparément
- `repairPhotosCloud()` utilise `savePhotosCloud()` par batch
- Import CSV : même séparation

### SQL Supabase requis
```sql
create table if not exists floralab_photos (
...
alter table floralab_photos enable row level security;
create policy "public_read_photos"  on floralab_photos for select using (true);
create policy "public_write_photos" on floralab_photos for all using (true) with check (true);
```
