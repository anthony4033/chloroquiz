...
### Corrigé / Amélioré
- **Section "Quiz interrompus" toujours visible** dans les stats admin, même si aucun quiz n'a encore été arrêté (affiche "Aucun quiz interrompu pour le moment").
- **Compteurs globaux** ajoutés dans le sous-titre des stats : "X partie(s) terminée(s) · Y interrompue(s)".
- **Nb interrompus par apprenti** affiché dans chaque carte individuelle (ex. "⏹ 2 quiz interrompu(s) · 8/20 qst avant arrêt").
- `fmtElapsed` sorti de `renderStats` comme fonction standalone (évite redéclaration).
...
### Corrigé
- **Quiz interrompus remontés vers l'admin** : les données étaient sauvegardées uniquement dans le localStorage de l'appareil de l'élève. Maintenant enregistrées dans la table Supabase `floralab_abandoned` (partagée) — visibles par l'admin depuis n'importe quel appareil. Fallback localStorage si hors ligne.
- **Mode dictée sans questions de dictée** : dans `buildPhotoRound`, le type `dictee` était appliqué puis immédiatement écrasé par le tirage `spelling/identify` qui suivait sans condition. Corrigé : si le mode dictée est actif et que le tirage l'exige (1 chance sur 4), le type `dictee` est conservé et les branches `spelling/identify` sont ignorées.

### Action requise côté Supabase
Exécuter ce SQL une seule fois dans l'éditeur Supabase :
```sql
...
### Amélioré
- **Rafraîchissement automatique des stats admin** : la page Statistiques se recharge toutes les 10 secondes tant qu'elle est ouverte. Dès qu'un élève arrête un quiz, l'admin voit l'entrée apparaître sans avoir à quitter et revenir.
- **Indicateur de dernière mise à jour** : l'heure du dernier rafraîchissement (ex. `↻ 17:03:42`) est affichée discrètement dans le sous-titre des stats.
- Le polling s'arrête automatiquement dès que l'admin quitte la page Statistiques (pas de requêtes inutiles en arrière-plan).
...
### Ajouté
- **Impression / PDF du comparatif** : bouton "📄 Imprimer / PDF" dans le header de la modale de comparaison. Génère une page HTML imprimable avec les deux plantes côte à côte, photos incluses, champs identiques surlignés en vert et champs différents en jaune. L'impression s'ouvre automatiquement dans un nouvel onglet.
